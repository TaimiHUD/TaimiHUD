use {
    ...
    std::{borrow::Cow, fs, io::{self, Read}, path::{Path, PathBuf}, ffi::{c_void, OsStr, CStr}},
    taimi_d3d::{
        dx11::{
            prelude::*,
            shader::{InputLayout, ShaderP, ShaderV},
        },
        shader::{ShaderKind, ID3DInclude, ID3DInclude_Impl},
    },
    rustc_hash::{FxHashMap, FxHashSet},
    futures::future::Either,
    windows::{core::{PCSTR, Result as WinResult}, Win32::Foundation},
    std::ptr,
    std::sync::Mutex,
};
...
impl ShaderLoader {
    ...
    pub fn load_bundled(device: &Dx11Device) -> anyhow::Result<Self> {
        ...
        for (path, contents) in shader_description_paths {
            ...
            let data = match contents {
                Cow::Borrowed(c) => str::from_utf8(c).map(Cow::Borrowed)
                    .with_context(context),
                Cow::Owned(c) => String::from_utf8(c).map(Cow::Owned)
                    .with_context(context),
            }?;
            let mut shader_description = ShaderDescription::load_from_str(data.into_owned())
                .with_context(context)?;
    ...
    fn load_from<S>(dir: &ShaderDirectory, shader_descriptions: S, device: &Dx11Device) -> anyhow::Result<Self>
    where
        S: IntoIterator<Item = ShaderDescription>,
    {
        ...
        for mut shader_description in shader_descriptions {
            if shader_description.partial {
                shaders.partial.insert(shader_description.identifier.clone(), shader_description);
                continue
            }
            shader_description.defs.terminate();
            let context = || format!("loading shader {}", shader_description.identifier);
            let bytecode = dir.get_file_contents(&shader_description.path)
            ...
            let res = layout.and_then(|layout|
    ...
    pub fn insert(&mut self, device: &Dx11Device, identifier: String, bytecode: &Blob, vertex_layout: Option<InputLayout>) -> anyhow::Result<()> {
        match vertex_layout {
            Some(layout) => {
                let shader = ShaderV::new_with_bytecode(device, bytecode)?;
                self
                    .vertex
                    .insert(identifier, (shader, layout));
            },
            None => {
    ...
    pub fn load_partial(&mut self, device: &Dx11Device, identifier: &str, bytecode: &Blob, partial_id: &str) -> anyhow::Result<()> {
        let context = || format!("loading shader {identifier}");
        let layout = self.partial.get(partial_id)
            .context("missing")
            .and_then(|partial| match partial.target.kind() {
                ShaderKind::Vertex => {
                    let desc = partial.input_layout_desc();
                    InputLayout::new_with_desc(device, desc, bytecode).map(Some).map_err(Into::into)
                },
                ShaderKind::Pixel => Ok(None),
            }).with_context(|| format!("template {partial_id:?}"));
        layout
            .and_then(|layout|
                self.insert(device, identifier.into(), bytecode, layout)
            ).with_context(context)
    }
...
impl ShaderDirectory {
    ...
    pub fn get_file<P: ?Sized + AsRef<Path>>(&self, path: &P) -> anyhow::Result<Option<fs::File>> {
        ...
        match fs::File::open(&resolved) {
            ...
            format!("missing shader data `{}`", Self::resolve_filename(path).display())
        )
    }
    pub fn open_file<P: ?Sized + AsRef<Path>>(&self, path: &P) -> anyhow::Result<Box<dyn io::BufRead>> {
        ...
        match res {
            Ok(Some(f)) => Ok(f),
            res =>
                match self.fallback.and_then(|f| f.get_file(path)) {
                    Some(f) => {
                        if let Err(e) = res {
    ...
    pub fn get_file_contents<P: ?Sized + AsRef<Path>>(&self, path: &P) -> anyhow::Result<Cow<'static, [u8]>> {
        let path = path.as_ref();
        let res = self.get_file(path);
            ...
            res =>
                match self.fallback.and_then(|f| f.get_file(path)) {
                    Some(f) => {
                        if let Err(e) = res {
                            let filename = Self::resolve_filename(path);
                            log::warn!("failed to retrieve shader {} from disk: {e:#}", filename.display());
                        }
    ...
    pub fn iter_contents_of<'e>(&self, ext: &'e str) -> impl Iterator<Item = (PathBuf, Cow<'static, [u8]>)> + 'e {
        let emb = self.fallback.map(|emb| emb.find(&format!("*.{ext}"))).transpose();
        let emb = rt::log::error_ok(emb).flatten()
            .into_iter()
            .flatten();
        ...
        let paths = dir.map(Either::Left)
            .chain(emb.map(Either::Right));
...
impl ID3DInclude_Impl for ShaderDirectory {
    fn Open(&self, _ty: d3d::D3D_INCLUDE_TYPE, fname: &PCSTR, _parent: *const c_void, out: *mut *mut c_void, out_len: *mut u32) -> WinResult<()> {
        let fname = unsafe {
            CStr::from_ptr(fname.as_ptr() as *const _)
        };
