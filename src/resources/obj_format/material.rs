...
impl ObjMaterials {
    pub fn load(&self, device: &Dx11Device, idx: usize) -> anyhow::Result<ObjMaterial> {
        ...
        let specular =
            if let (Some(texture), Some(value)) = (&material.specular_texture, &material.specular) {
                let texture_path = self.folder.join(PathBuf::from(&texture));
                let texture = Texture::load(device, &texture_path)?;
                let colour = Vec3::from_slice(value);
                Some(ColouredMaterialTexture { texture, colour })
        ...
        let shininess =
            if let (Some(texture), Some(attribute)) = (&material.shininess_texture, material.shininess) {
                let texture_path = self.folder.join(PathBuf::from(&texture));
                let texture = Texture::load(device, &texture_path)?;
        ...
        let dissolve =
            if let (Some(texture), Some(attribute)) = (&material.dissolve_texture, material.dissolve) {
                let texture_path = self.folder.join(PathBuf::from(&texture));
                let texture = Texture::load(device, &texture_path)?;
                Some(AttributedMaterialTexture { texture, attribute })
            } else {
