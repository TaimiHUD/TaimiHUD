use {
    crate::{
        ...
        exports::runtime::imgui::{
            self,
            Condition,
            TreeNode, TreeNodeFlags,
        },
        render::element::{
            addons::AddonHostSelection,
...
impl ArcRenderState {
    ...
    fn ui_options_keybinds(&mut self, ui: &imgui::Ui) {
        for &binding in ArcSettings::VK_WINDOWS {
            self.bindings.do_keybind(ui, binding, Some(|vk: &ArcVk| if let Some(window) = vk.window_name() {
        ...
        {
            ui.separator();
            self.bindings.do_keybind(ui, &ArcSettings::VK_RENDER_TOGGLE_PATHING, Some(|_vk: &ArcVk| PathingEvent::VISIBLE_TOGGLE_SPACE.try_send()));
            ui.separator();
            self.bindings.do_keybind(ui, &ArcSettings::VK_RENDER_TOGGLE_PATHING_MINIMAP, Some(|_vk: &ArcVk| PathingEvent::visible_toggle(MapContext::Minimap).try_send()));
            ui.separator();
            self.bindings.do_keybind(ui, &ArcSettings::VK_RENDER_TOGGLE_PATHING_MAP, Some(|_vk: &ArcVk| PathingEvent::visible_toggle(MapContext::Global).try_send()));
        }
        #[cfg(feature = "timers")]
        {
            ui.separator();
            for binding in &ArcSettings::VK_TIMER_TRIGGERS {
                self.bindings.do_keybind(ui, binding, Some(|vk: &ArcVk| TimersController::try_send(TimersEvent::TimerKeyTrigger(vk.id.into(), false))));
            }
            self.bindings.do_keybind(ui, &ArcSettings::VK_TIMER_RESET, Some(|_vk: &ArcVk| TimersController::try_send(TimersEvent::TimerReset)));
        }
    }
}
