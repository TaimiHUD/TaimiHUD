use {
    crate::{
        ...
        space::goggles::{
            self,
            lens::{LensClass, LENSES, LENS_PTR},
        },
    },
    anyhow::Context,
    ...
    if let Ok(lenses) = LENSES.read() {
        ...
        if let Some(selected_proj) = selected_proj {
            ...
            if let Some(combo) = ui.begin_combo("Projection", preview) {
                ...
                for (&key, &clss) in lenses.iter() {
                    if !matches!(
                        clss,
                        LensClass::World | LensClass::Test | LensClass::UI | LensClass::Dummy
                    ) {
                        continue
                    }
                ...
                for (key, info) in goggles::FerretResource::project_iter_ui(
                    goggles::project::ProjectClassification::DEFAULT_TARGET,
                ) {
                    let ty = match info.kind {
                        goggles::project::ProjectBufferKind::DepthView => "DV",
                        goggles::project::ProjectBufferKind::RenderTarget => "RT",
                    };
                    let mut selected = imgui::Selectable::new(format!(
                        "{ty}={:?} ({:#08x})",
                        info.classification,
                        key.as_ptr() as usize
                    ))
            ...
            if ui.is_item_clicked_with_button(imgui::MouseButton::Right) {
                new_proj = Some((
                    None,
                    Some(goggles::project::ProjectClassification::DEFAULT_TARGET),
                ));
            }
        ...
        if let Some(selected_shadowbox) = selected_shadowbox {
            ...
            if let Some(_combo) = ui.begin_combo("Shadowboxing", preview) {
                ...
                for (key, info) in goggles::FerretResource::project_iter_ui(
                    goggles::project::ProjectClassification::DEFAULT_SHADOWBOX,
                ) {
                    let selected = imgui::Selectable::new(format!(
                        "RT={:?} ({:#08x})",
                        info.classification,
                        key.as_ptr() as usize
                    ))
            ...
            if ui.is_item_clicked_with_button(imgui::MouseButton::Right) {
                new_shadowbox = Some((
                    None,
                    Some(goggles::project::ProjectClassification::DEFAULT_SHADOWBOX),
                ));
            }
        } else {
