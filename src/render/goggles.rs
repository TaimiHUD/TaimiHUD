        ...
        U: ?Sized + ImDrawWindow<'ui>,
    {
        ...
        if let Some(_list) = list.begin(ui) {
            ...
            for (key, info) in ClassShared::iter_ui() {
                ...
                let mut selected = imgui::Selectable::new(label).selected(is_selected).build(ui);
        ...
        if self.view_lens_info.is_empty() {
            ...
            let view = if let (Some(lens), Some(info)) = (&selected_lens, &selected_info) {
                let view: Option<&View> = match (&lens, info.kind) {
                    _ if !info.was_seen()
                        ...
                        None,
                    (p, goggles::class::BufferKind::DepthView) =>
                        Some({ &*dview.insert(unsafe { DepthView::from_d3d_raw_ref(p) }) }),
                    (p, goggles::class::BufferKind::RenderTarget) =>
                        Some({ &*rview.insert(unsafe { RenderTargetView::from_d3d_raw_ref(p) }) }),
                    (p, goggles::class::BufferKind::UnorderedAccessView) =>
                        Some({ &*uview.insert(unsafe { View::from_d3d_raw_ref(p) }) }),
                };
                view.map(|v| (v, info))
            } else {
                None
            ...
            if let Some(desc) = desc {
                ...
                let _ = write!(
                    out,
                    "format={:#x} usage={:#x} bind={:#x} misc={:#x}",
