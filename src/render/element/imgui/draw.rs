...
pub trait ImDrawWindowExt<'ui>: ImDrawWindow<'ui> {
        ...
        //S: taimi_ui::im::text::ImStr, I: taimi_ui::im::text::IntoImStrId,
    {
        ...
        let args = match self.imgui_version_num() {
            ...
            Some(im192::VERSION_NUM) => imw::DynArgsWindow::new(Some(
                ...
                im192::sys::ImGuiWindowFlags_NoNavFocus | im192::sys::ImGuiWindowFlags_NoFocusOnAppearing,
            )),
            _ => Default::default(),
        };
    ...
    /// TODO
    fn tooltip_text_wrapped<S>(&mut self, label: S)
    where
        S: ImStrExt,
    {
        if let Some(_tt) = self.begin_tooltip() {
            // TODO: maybe only needed on appearing?
            let _max_w = self.text_wrap_pos_push(Some(Some(384.0)));
            self.text_wrapped(label)
        }
    }
