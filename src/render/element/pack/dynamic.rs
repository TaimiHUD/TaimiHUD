...
impl<'a, 's, 'u, 'ui, U> DrawPe<'a, 's, 'u, U>
where
    U: ?Sized + ImDrawWindow<'ui> + 'u,
{
    ...
    pub fn draw(&mut self) {
        ...
        let noblank = match self.ui.imgui_version_num() {
            ...
            Some(im180::VERSION_NUM) =>
                Some(imw::InputText::IM180_FLAGS_PRESET | im180::sys::ImGuiInputTextFlags_CharsNoBlank),
            #[cfg(taimi_imgui = "192")]
            Some(im192::VERSION_NUM) =>
                Some(imw::InputText::IM192_FLAGS_PRESET | im192::sys::ImGuiInputTextFlags_CharsNoBlank),
            _ => None,
        };
        {
            ...
            self.ui.input_text_managed(
                c"##catid",
                &mut self.state.env.pack_cat_id,
                64,
                Some(c"id"),
                noblank,
            );
            if is_new_cat {
                ...
                self.ui.input_text_managed(
                    c"##catname",
                    &mut self.state.env.pack_cat_name,
                    64,
                    Some(c"name"),
                    None,
                );
        ...
        if let MarkerIndex::NS_UNK | MarkerIndex::NS_POI = mns {
            ...
            if let Some(_token) = texs {
                ...
                let manual_entry = self.ui.input_text_managed(
                    c"##tex",
                    &mut self.state.env.tex_selected,
                    96,
                    Some(c"image path"),
                    None,
                );
        ...
        if let MarkerIndex::NS_UNK | MarkerIndex::NS_TRAIL = mns {
            ...
            if let Some(_section_path) = section_path {
                ...
                self.ui.item_prepare_width(width);
                commit |= self.ui.input_text_managed(
                    c"##trl",
                    &mut self.state.env.trl_name,
                    96,
                    Some(c"trl"),
                    noblank,
                );
    ...
    pub fn draw_alloc(&mut self) {
        ...
        commit |= self.ui.input_text_managed(
            c"##dir",
            &mut self.state.env.pack_alloc_name,
            96,
            Some(c"dir"),
            noblank,
        );
    ...
    pub fn draw_alloc_open(&mut self) {
        let packs =
            self.ui
                .begin_tree_node_framed(ImCondition::appear(false), c"otherpacks", c"open", false);
        if let Some(packs) = packs {
            self.ui.indent();
            for pd in self.packs.values() {
                let _packid = self.ui.push_id(pd.state.ui_id());
                self.ui
                    .text_with_font(NexusLinkFont::Ui, im_fmt!("{}", &pd.state.info));
                self.ui.same_line();
...
impl super::PackElements {
    pub fn draw_dynamic<'ui, U>(&mut self, ui: &mut U)
    where
