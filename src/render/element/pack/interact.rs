use {
    crate::{
        controller::pathing::{
            registry::{PackInfo, PackMapPath, PackPath, PoiMapPath},
    ...
    glamour::Point3,
    std::iter,
    taimi_hoard::{loc::Locator, str_opt_ref},
    taimi_meta::packs::{
...
impl PathingWindowState {
    pub fn draw_pois(
        ...
        let map_packs = {
            let pack_info = self
                ...
                .unwrap_or_default();
            let Some(shared_map) = self.pack_gameplay.as_ref().map(|i| i.borrow()) else {
                return
            };
            pack_info
                .into_iter()
        ...
        let ipois = map_packs
            ...
            .map(|(map_path, _pack_info, map_info, map)| {
                map_info
                    ...
                    .enumerate()
                    .zip(
                        map.interactive_pois_nearby
                            .iter()
                            .map(|n| *n)
                            .chain(iter::repeat(false)),
                    )
                    .zip(
                        map.interactive_poi_pois
                            .iter()
                            .map(Some)
        ...
        }
        {
            ...
            if let Some(..) = &table_token {
                let mut ipois = ipois
                    .into_iter()
                    .zip(&map_packs)
                    .flat_map(|(ipois, map_packs @ (_, _, map_info, ..))| {
                        ipois
                            .into_iter()
                            .zip(map_info.interactive_pois.iter())
                ...
                if let Some(engine) = &engine {
                    ipois.sort_unstable_by_key(|(rpoi, ..)| {
                        ...
                        let dist = match prio {
                            ...
                            (false, false) => {
                                let id = RenderId::Poi {
                                    pack_idx: rpoi.pack_path().path,
                                    poi_idx: rpoi.loaded_index,
                                };
                                engine
                                    .packs
                                    .render_list
                ...
                for (rpoi, ipoi, (_map_path, pack_info, map_info, map)) in ipois.iter() {
                    if disabled_token.is_none() && rpoi.is_disabled() {
                        //ui.separator();
                        let disabled_token = disabled_token.get_or_insert_with(|| {
                            with_i18n!("disabled", |label| TreeNode::new("pois-disabled")
        ...
        if self.act_selected_poi.is_some() {
            ui.popup("poi-context", || self.menu_ipoi(ui, machine, engine))
        }
    ...
    pub fn menu_ipoi(&mut self, ui: &Ui, _machine: &mut RenderMachine, _engine: Option<&Engine>) {
    ...
    #[cfg(todo)]
    pub(super) fn lpoi_get_guid(
        map_info: &SharedMapPackLoaded,
        loaded_path: PoiPath<PackMapPath>,
    ) -> Option<Guid> {
        let guid_idx = map_info
            .info
            .poi_guid_mask()
    ...
    #[cfg(todo)]
    pub(super) fn lpoi_get_filtered(
        map: &SharedMapPackState,
        ipoi_idx: usize,
        lpoi_path: Locator<PackPath, usize>,
