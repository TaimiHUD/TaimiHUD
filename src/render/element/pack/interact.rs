    ...
    U: ?Sized + ImDrawWindow<'ui> + 'u,
{
    pub fn draw(&mut self) {
        ...
        match self.hidden {
            ...
            true =>
                self.act_untrigger = with_i18n!("trigger-untrigger", |label| ui.selectable(label, true)),
        }
        if self.guid {
        ...
        ui.separator();
        let action_dismiss_open = with_i18n!("trigger-behaviour", |label| ui
            .selectable_dismiss(label, false, false));
...
impl PoiInfoContext {
    ...
    pub fn finish_draw_menu<'ui, U>(&mut self, mut draw: DrawMenuPoi<'_, U>)
    where
...
impl<'s, 'a, 'u, 'ui, U> DrawPoiInfo<'s, 'a, 'u, U>
where
    U: ?Sized + ImDrawWindow<'ui> + 'u,
{
    pub fn new(
    ...
    pub fn draw(&mut self) {
        let ui = &mut *self.ui;
        self.bounds = Box2::new(ui.window_content_region_min(), ui.window_content_region_max());
        #[cfg(todo = "unused")]
    ...
    pub fn draw_nearby(&mut self) {
        ...
        for (mid, marker) in self.state.iter_markers() {
                        ...
                        if has_tooltip && self.ui.is_item_hovered() {
                            ...
                            if let Some(attrs) = poi_attrs() {
                                if let Some(_tt) = self.ui.begin_tooltip() {
                                    let copyable = attrs
                                        .get_attr_ref_of::<keys::CopyValue>()
            ...
            match named.or(marker.display_name()) {
                ...
                None => self
                    .ui
                    .text(mid.get_marker_path_pack_map().root.rel(marker.path).to_string()),
            }
    ...
    fn open_table(ui: &mut U, title_id: &str) -> Option<UiTokenDyn<'ui>> {
        let (table_flags, col_flags_title, col_flags_trailing) = match ui.imgui_version_num() {
            ...
            Some(im180::VERSION_NUM) => (
                ...
                Some(
                    im180::sys::ImGuiTableColumnFlags_NoReorder
            ...
            Some(im192::VERSION_NUM) => (
                ...
                Some(
                    im192::sys::ImGuiTableColumnFlags_NoReorder
        ...
        if let Some(..) = &table_token {
            let cols = [
                (Self::HEADER_TITLE, col_flags_title, 2.0),
                (
                    Self::HEADER_NEARBY,
...
impl super::PackElements {
    pub fn draw_interact<'ui, U>(&mut self, ui: &mut U) -> ActDrawInteract
    where
        U: ?Sized + ImDrawWindow<'ui>,
    {
