...
impl PackElements {
    ...
    fn pack_sort_key(filtering: bool, pack: &PackElement) -> Option<NonZero<u8>> {
        ...
        NonZero::new(match delay {
            None => SORT_KEY_HIDE,
            Some(true) if filtering && pack.categories.filter_state.any_visible() => SORT_KEY_PENDING,
...
impl PackElement {
    ...
    pub fn draw_pack_tooltip_contents<'ui, U>(&self, ui: &mut U, title_visible: bool, reason_visible: bool)
    where
        U: ?Sized + ImDrawWindow<'ui>,
    {
