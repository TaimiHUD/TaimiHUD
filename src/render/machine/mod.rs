...
use {
    ...
    arcloader_mumblelink::identity::MumbleIdentityTracker,
    std::borrow::Cow,
    taimi_meta::{
        coords::SignObtainer,
        map::{Map, MapCache},
...
pub struct RenderMachine {
    #[cfg(any(feature = "markers", feature = "space"))]
    pub identity: mumblelink::MumbleIdentityShare,
    #[cfg(any(feature = "markers", feature = "space"))]
...
impl RenderMachine {
        ...
        #[cfg(any(feature = "markers", feature = "space"))]
        {
            let map_left = match trans {
                GameplayTransition::Map { prev_map_id: Some(prev), .. }
                    if Some(prev) == gameplay.gameplay_map() =>
                    false,
                ...
                GameplayTransition::Intermission { .. }
                    if !self.is_cutscene() && !self.is_ingame_paused() =>
                    true,
                _ => true,
            };
            if map_left {
            ...
            #[cfg(feature = "space")]
            if let Some(map_id) = map_id {
                self.map_depth = MapCache
                    .lookup_map_projection(map_id.get())
                    .map(|proj| proj.depth.clone());
                ...
                Some(map_id) => match MapCache.lookup_map(map_id.get()) {
                    Some(map) => {
                        ...
                        if self.map_depth.is_none() {
                            ...
                            let map_aspect = map_aspect.min(Self::DEPTH_ASPECT_MAX);
                            let scale = 12.0 * MapLocalScale::METRES_PER_INCH;
                            let far = continent.height * map_aspect * scale;
    ...
    pub fn turn_render_entry() {
        ...
        if let Some(state) = state.as_mut() {
            ...
            let prev = state
                .machine
                .mumblelink_frames
                .get_latest_render_timestamp()
                .copied();
            let render_timestamp = render_timestamp.or_else(|| {
                let frametime = *state
                    .machine
                    .frame_duration
                    .insert(now.saturating_duration_since(prev?));
                let midpoint = (frametime <= Self::FRAMETIME_MAX_REASONABLE).then_some(frametime / 3);
                midpoint.and_then(|mid| now.checked_sub(mid))
            });
            let wants_frame_duration = state.machine.frame_duration.is_none()
                && state.machine.metrics_switch.contains(MetricsSwitch::COLLECT);
    ...
    pub fn turn_render(&mut self, _render_slot: RenderSlot<'_>) {
        ...
        let gameplay_change = gameplay_change.or(mumble_gameplay).or(frameskip_gameplay);
    ...
    pub(crate) fn is_cutscene(&self) -> bool {
        ...
        #[cfg(feature = "goggles2-camera")]
        if self.gameplay.gameplay_map().is_none()
            && self.goggles.camera.camera_enabled
    ...
    pub(crate) fn is_ingame_paused(&self) -> bool {
        #[cfg(feature = "extension-nexus")]
        if self.rtapi_state.is_loading_uncertain() {
            return true
        }
        if self.is_cutscene() {
            #[cfg(feature = "goggles2-camera")]
            if !self.goggles.camera.has_camera() {
                return true
            }
        }
        false
