...
impl RenderMachine {
    ...
    pub fn turn_render(&mut self, _render_slot: RenderSlot<'_>) {
        ...
        let mumble_gameplay = ml.and_then(|ml| self.act_mumblelink_tick(ml));
    ...
    fn post_render(&mut self) {
        ...
        if self.goggles.camera_enabled {
            ...
            match cam {
                ...
                Err(intermission) => self.goggles.camera_pause(intermission),
            }
        }
        #[cfg(feature = "goggles")]
        {
    ...
    fn is_cutscene(&self) -> bool {
        ...
        if self.gameplay.gameplay_map().is_none()
            && self.goggles.camera_enabled
            && self.goggles.is_camera_moving()
        {
            return true
        }
        false
