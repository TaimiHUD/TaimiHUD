...
#[cfg(feature = "space")]
use {
    taimi_meta::spatial::record::{FrameRecordEntry, FrameRecord},
...
impl RenderMachine {
    ...
    pub fn get_camera_mumblelink(&self) -> Option<RenderPosition> {
        ...
        Some(match (self.mumblelink_camera_frame.wrapping_sub(self.mumblelink_camera_prev_frame), self.mumblelink_frame_skip) {
    ...
    pub(crate) fn lastminute_mumblelink_update(&mut self) {
        ...
        if let Some(rtapi) = self.rtapi.as_ref().and_then(|rtapi| rtapi.is_active().then_some(rtapi)) {
            ...
            let rt_tick = match ml {
                ...
                #[cfg(todo)]
                _ => self.mumblelink_frame.wrapping_add(self.mumblelink_frame_skip.min(1)),
            ...
            if self.rtapi_state.has_camera() && rt_tick_missed() {
                ...
                if !vec32_eq(camera_rtapi.0, rtapi_prev.0) && !vec32_eq(camera_rtapi.1, rtapi_prev.1) {
                    let matches_ml = ml.map(|(ml, ..)| vec32_eq(camera.insert(Self::read_camera_mumblelink(ml)).0, camera_rtapi.0));
                    if matches_ml != Some(true) {
                        self.camera.record_mumblelink(rt_tick, (camera_rtapi.0, camera_rtapi.1, Vector3::ZERO));
                    }
                }
            }
        }
        #[cfg(deleteme)]
        let ml = ml.and_then(|(ml, tick, new_tick)| {
            new_tick.then_some((ml, tick))
        });
        ...
        #[cfg(deleteme)]
        let needs_ml_cam = ml.is_some() && self.camera.mumblelink.get_at(self.mumblelink_frames.latest_render_tick()).is_none();
        #[cfg(deleteme)]
    ...
    pub(super) fn post_render_mumblelink_check(&mut self) -> Option<u32> {
        ...
        self.mumblelink_frames.record_lastminute_tick_at(tick, Instant::now());
    ...
    pub fn get_latest_space_timestamp(&self) -> Option<Instant> {
        let ts = match &self.mumblelink_frames {
            #[cfg(feature = "space")]
            frames => frames.renders.get_at(frames.latest_render_tick())
                .copied().flatten(),
    ...
    pub fn latest_space_timestamp(&self) -> Instant {
        self.get_latest_space_timestamp().unwrap_or_else(|| Instant::now())
    }
}
...
impl MumblelinkFrames {
    pub fn latest_tick(&self) -> Option<(u32, &Instant)> {
        match () {
            ...
            #[cfg(not(feature = "space"))]
            _ => self.latest_tick_timestamp.as_ref().map(|i| (i, &self.latest_tick)),
        }
    }
    ...
    pub fn record_tick_at(&mut self, tick: u32, when: Instant) {
        let prev_tick = self.times.iter_populated().next().map(|(i, ..)| i).unwrap_or(self.times.position);
    ...
    pub fn duration_at(&self, tick: Option<u32>) -> Option<f32> {
        let (tick, ts) = tick.and_then(|tick| self.timestamp_at(tick)
            .map(|ts| (tick, ts))
    ...
    pub fn is_tick_overdue(&self) -> bool {
        let Some((tick, ts)) = self.last_seen_tick() else {
            return false
        };

        match self.render_to_uitick(self.latest_render_tick()).wrapping_sub(tick) {
            missed if missed < Self::UI_TICK_OVERDUE => false,
            _ if self.latest_render_timestamp().saturating_duration_since(*ts) < Self::UI_TICK_OVERDUE_DURATION =>
                false,
            _ => true,
        }
    }
    ...
    pub fn player_tick_overdue(&self) -> Option<f32> {
        ...
        match elapsed.as_secs_f32() - Self::PLAYER_TICK_DURATION {
            overdue if overdue >= Self::PLAYER_TICK_DURATION_OVERDUE_BY =>
                Some(overdue),
            _ => None,
        }
    }
    pub fn next_player_deadline(&self) -> Option<(u32, Instant)> {
        ...
        let estimate = self.duration_at(None)
            .map(|frame_time| tick.wrapping_add(((Self::PLAYER_TICK_DURATION / frame_time).ceil() as u32).max(1)))
            .unwrap_or_else(||
                tick.wrapping_add(Self::PLAYER_TICK_DEADLINE)
            );
    ...
    const PLAYER_TICK_DURATION_OVERDUE_BY: f32 = Self::PLAYER_TICK_DURATION_OVERDUE - Self::PLAYER_TICK_DURATION;
}
