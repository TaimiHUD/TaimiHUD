...
use {
    crate::{
        render::machine::{MumblelinkFrames, RenderMachine, RenderPosition, RenderPositioning},
    ...
    taimi_meta::{
        ...
        spatial::record::{frame_is_lt, FrameRecordEntry, FrameRecordOf},
        ui::{
...
#[cfg(feature = "goggles")]
use crate::{exports::runtime as rt, settings::goggles::GogglesMapDepth};

impl RenderMachine {
    pub fn get_camera_pos(
        &self,
        source: CameraSource,
    ) -> Option<(RenderPosition<DrawSpace>, CameraSource)> {
        ...
        let new = match () {
            #[cfg(feature = "goggles2-camera")]
            _ if self.goggles.camera.debug_interpolate => self.camera.debug_interpolate(
                new_frame,
                Some(source),
                &self.mumblelink_frames,
                self.goggles.camera.debug_interpolate_off,
            ),
            _ => self
                .camera
                .get_or_interpolate(new_frame, Some(source), &self.mumblelink_frames),
        };
        new.map(|cam| {
            let up = match () {
                #[cfg(feature = "goggles2-camera")]
    ...
    pub(super) fn is_camera_moving(&self) -> bool {
        self.camera.is_moving(
            self.mumblelink_frames.latest_render_tick(),
            None,
            &self.mumblelink_frames,
        )
    }
    ...
    pub(super) fn pre_render_space(&mut self) {
        #[cfg(feature = "goggles")]
        {
            ...
            self.goggles
                .act_pre_render(has_engine, self.display_size_ref().to_raw());
    ...
    pub fn get_camera_pos(
        ...
        if camsrc == CameraSource::MumbleLink && has_cam && self.mumblelink_frame_skip > 0 {
            ...
            if let Some((pos, front)) = interp {
                ...
                let factor =
                    (Self::CAMERA_SMOOTHING_PER_FRAME * self.mumblelink_frame_skip as f32).min(1.0);
    ...
    pub const DEFAULT_DEPTH_RANGE: Range<f32> = Self::DEFAULT_DEPTH_RANGE_IN.start
        * MapProjectionDepth::DEFAULT_METRES_PER_INCH
    ...
    pub fn depth_range(&self) -> Range<f32> {
        ...
        let Range { start: near, end: far } =
            self.get_depth_range().unwrap_or(Self::DEFAULT_DEPTH_RANGE_IN);
        let i2m = self.depth_scale_i2m();
    ...
    fn goggles_update_camera_unchecked(&mut self) {
        ...
        self.camera
            .record_goggles(snatch_frame, &GogglesShared::snatch_camera(), true);
    }
    ...
    fn goggles_update_camera_smooth_unchecked(&mut self) {
        ...
        if let Some(prev_frame) = prev_frame {
            ...
            self.camera
                .record_goggles(prev_frame, &GogglesShared::snatch_camera_smooth(), update_up);
        }
    }
    #[cfg(feature = "goggles")]
        ...
        match cam {
            Ok(cam) => {
                ...
                let update_mod: u32 = match self.mumblelink_state {
                    s if !s.contains(UiState::WINDOW_FOCUS) => 0x80,
                    s if s.contains(UiState::MAP_OPEN) => match self.get_map_open_state() {
                        MapOpen::Closed | MapOpen::Closing { .. } => 0x08,
                        _ => 0x80,
                    },
                    _ => 0x02,
                };
            ...
            Err(intermission) => self.goggles.camera.camera_pause(intermission),
        }
    }
    ...
    pub fn logo_object() -> Option<(&'static [tobj::Model], &'static [tobj::Material])> {
        static MAT: LazyLock<Option<(Vec<tobj::Model>, Vec<tobj::Material>)>> = LazyLock::new(|| {
            ...
            .and_then(|(o, m)| m.map(|m| (o, m)));
            match res {
                #[cfg(taimi_debug)]
                res => rt::log::debug_ok(anyhow::Context::context(res, "taimihud.obj")),
                #[cfg(not(taimi_debug))]
                res => res.ok(),
            }
        });
...
impl CameraState {
    ...
    pub fn is_moving(&self, frame: u32, prefer: Option<CameraSource>, frames: &MumblelinkFrames) -> bool {
        let Some((prior, next)) = self.frames_near_relaxed(frame) else { return false };
        let fallback = || {
            frame
                .checked_sub(next)
                .map(|behind| behind <= Self::MOVING_GRACE_FRAMES)
                .unwrap_or(true)
        };
        ...
        match (self.get_at(prior, prefer), self.get_at(next, prefer)) {
            (Some(prior), Some(next)) => {
                ...
                let frame_time = frames
                    .duration_at(Some(ui_tick))
                    .unwrap_or(Self::MOVING_THRESHOLD_TIME);
    ...
    pub fn get_at(&self, render_frame: u32, prefer: Option<CameraSource>) -> Option<&CameraPosition> {
        ...
        match ml {
            #[cfg(feature = "goggles2-camera")]
            _ if goggles.is_some()
                && (self.goggles_confident || prefer == Some(CameraSource::Goggles2)) =>
                goggles,
            #[cfg(feature = "goggles2-camera")]
            ml if (!self.mumblelink_confident && prefer != Some(CameraSource::MumbleLink))
                || ml.is_none() =>
                goggles.or(ml),
            ml => ml,
        }
    ...
    pub fn get_or_interpolate(
        ...
        if _frames.is_ui_tick_overdue() {
            match self.goggles.iter_populated().next() {
                Some((tick, cam))
                    if frame.wrapping_sub(tick) <= Self::SKIP_TICK_THRESHOLD
                        || frame_is_lt(frame, tick) =>
                    return Some(*cam),
                _ => (),
            }
        }
        ...
        Some(self.interpolate_between(frame, prior, next, Self::INTERP_PARAMS_DEFAULT, prefer))
    }
    pub fn debug_interpolate(
        &self,
        frame: u32,
        prefer: Option<CameraSource>,
        _frames: &MumblelinkFrames,
        ...
        let frame = match force_off {
            #[cfg(deleteme)]
            false =>
                if let Some((i, ..)) = _frames.latest_tick() {
        ...
        match force_off {
            true => {
                ...
                let cam_prior = || prior.and_then(|prior| self.get_at(prior, prefer));
                (!choose_next).then(cam_prior).flatten().or(cam_next).copied()
            },
            false => Some(self.interpolate_between(frame, prior, next, (0.05f32, true), prefer)),
        }
    }
    fn interpolate_between(
        ...
        let interp_frame = {
            ...
            let interp_pos = match frame.wrapping_sub(prior) {
                pos if frame_is_lt(next, frame) => pos as f32 + param_adj,
                pos => pos as f32,
            };
        ...
        let front = match dir_extrapolate {
            ...
            _ => cam_prior
                .front
                .slerp(cam_next.front, interp)
                .normalize_or(cam_next.front),
        };
        CameraPosition { pos, front }
    }
    ...
    fn frames_near(&self, frame: u32, strict: bool) -> Option<(Option<u32>, u32)> {
        ...
        let past = self
            .populated_frames()
            .filter(|&pos| frame_is_lt(pos, frame) && Some(pos) != next);
        let (prior, next) = match (past.min_by_key(sort_key), next) {
            (_, None) => return None,
            (prior, Some(next)) => (prior, next),
            #[cfg(todo = "unnecessary")]
            (Some(prior), None) => (
                self.populated_frames()
                    .filter(|&pos| frame_is_lt(pos, prior))
                    .min_by_key(sort_key),
                prior,
            ),
        };
    ...
    fn up_at_with(&self, frame: u32, thresh: u32) -> Option<Vector3<DrawSpace>> {
        self.up.get_at(frame).copied().or_else(|| {
            self.up.iter_populated().next().and_then(|(up_tick, up)| {
    ...
    pub(super) fn record_mumblelink(&mut self, frame: u32, (pos, front, up): RenderPosition) {
        ...
        if let Some(prev) = self.mumblelink.front().get_opt() {
            ...
            self.mumblelink_movement =
                self.mumblelink_movement * Self::MOVEMENT0 + movement * Self::MOVEMENT1;
        }
    ...
    pub fn reset(&mut self) {
        ...
        self.mumblelink
            .advance_to(self.mumblelink.position.wrapping_add(4));
        #[cfg(feature = "goggles2-camera")]
        {
...
impl From<RenderPositioning> for CameraPosition {
    fn from((pos, front, ..): RenderPositioning) -> Self {
        Self { pos: pos.into(), front: front.into() }
    }
}
    ...
    pub(crate) fn act_pre_render(&mut self, _visible: bool) {
        ...
        if self.camera_enabled && FerretResource::wants_camera() {
            ...
            if let Some((buf_dest, found)) = found {
                log::info!(
                    "found new cam at {buf_dest:p}@{:#x} out of {alternatives} choices",
                    found.offset
                );
    ...
    pub(crate) fn act_enable(&mut self, (en, en_cam, (en_proj, en_proj_shadowbox)): GogglesEnables) {
        ...
        if en_proj || en {
            ...
            match en_proj_shadowbox {
                true =>
                    FerretResource::project_set_shadowbox_request(Some(ProjectRequest::default_shadowbox())),
...
impl GogglesState {
    ...
    pub(super) fn camera_setup(
        &mut self,
        cam: Option<RenderPosition<DrawSpace>>,
        (fov_y, aspect): (f32, f32),
        update: u8,
    ) {
        self.camera_paused = false;
        let cam = match cam {
    ...
    pub(super) fn has_camera(&self) -> bool {
        ...
        match () {
            _ if self.has_camera_primary() => true,
