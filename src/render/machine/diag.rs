    ...
    fn metrics_post_ui_checkpoint(&mut self) {
        if let Some(checkpoint) = self.metrics_checkpoint_ui.take() {
            ...
            let interval = if let Some(frametime) = self.frame_duration {
                ...
                let prior_frame = self.mumblelink_frames.render_to_uitick(
                    self.mumblelink_frames.latest_render_tick().wrapping_sub(1)
                );
                self.mumblelink_frames.timestamp_at(prior_frame)
                    .map(|prev| frame_start.saturating_duration_since(*prev))
            };
            if let Some(interval) = interval {
                STATS_FRAME_TIME_INTERVAL.reset(interval.as_micros() as u64);
            }
            STATS_FRAME_TIME_LATENCY.reset_with(|| {
                checkpoint.saturating_duration_since(*frame_start).as_micros() as u64
            });
