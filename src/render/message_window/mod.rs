use {
    ...
    core::{mem, ops, fmt},
    std::{borrow::Cow, collections::{btree_map, BTreeMap}},
    taimi_pack::attributes::{cell::{pack_attr, PackValueSet, GetAttrDynExt, SetAttrDyn}, keys},
...
impl MessageBaseDesc {
    ...
    pub fn id(&self) -> Option<&str> {
        self.attrs.get_attr_dyn_ref_of::<keys::NameId>()
    ...
    pub fn title(&self) -> Option<&str> {
        self.attrs.get_attr_dyn_ref_of::<keys::DisplayName>()
            .and_then(str_opt_ref)
    }
    ...
    pub fn attribution(&self) -> Option<&str> {
        self.attrs.get_attr_dyn_ref_of::<keys::Title>()
            .and_then(str_opt_ref)
    }
    ...
    pub fn message(&self) -> Option<&str> {
        self.attrs.get_attr_dyn_ref_of::<keys::Info>()
            .and_then(str_opt_ref)
    }
    ...
    pub fn tooltip_title(&self) -> Option<&str> {
        self.attrs.get_attr_dyn_ref_of::<keys::TipName>()
            .and_then(str_opt_ref)
    }
    ...
    pub fn tooltip_desc(&self) -> Option<&str> {
        self.attrs.get_attr_dyn_ref_of::<keys::TipDescription>()
            .and_then(str_opt_ref)
    }
...
impl MessageItemDesc {
    ...
    #[inline]
    pub fn is_actionable(&self) -> bool { !self.actions.is_empty() }
    #[inline]
    pub fn is_empty(&self) -> bool {
...
impl MessageActionDesc {
    ...
    pub fn is_dismiss(&self) -> bool {
        self.attrs.attr_dyn_or_default::<keys::IsHidden>()
            .into()
    }
    ...
    pub fn is_context_menu(&self) -> bool {
        self.attrs.attr_dyn_or_default::<keys::IsSeparator>()
            .into()
    }
...
impl fmt::Debug for MessageActionDesc {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_tuple("MessageActionDesc").field(&self.base)
...
impl MessageItemStash {
    ...
    pub fn stash_str0(&mut self, key: usize, s: String) -> &Str0 {
        match self.strings.entry(key) {
            btree_map::Entry::Vacant(e) => e.insert(unsafe {
                String0::from_vec_unchecked(s.into())
            }),
    ...
    pub fn lookup_str0(&mut self, s: &str) -> &Str0 {
        ...
        self.strings.entry(key)
            .or_insert_with(|| unsafe {
                String0::from_vec_unchecked(s.to_owned().into())
            }).as_str0()
    }
    ...
    pub fn lookup_str0_with_id(&mut self, s: &str, id: &str) -> &Str0 {
        ...
        let stashed = match self.strings.entry(key) {
            ...
            btree_map::Entry::Occupied(s) =>
                return s.into_mut().as_str0(),
        };
        let translated = crate::render::i18n::LOADER.with_fluent_message_and_bundle(id, |m, b| {
        ...
        stashed.insert(translated.unwrap_or_else(||
            String0::format(s)
        )).as_str0()
    }
...
impl MessageWindowState {
    ...
    pub fn draw_window<'ui, U>(&mut self, ui: &mut U) where
        U: ?Sized + ImDrawWindow<'ui>,
    {
        ...
        let (pivot, pos) = match pos {
            ...
            None => {
                let pos = ui.with_io_dyn(|io| io.display_size().to_vector() * ImVec2::new(0.84, 0.028)).into();
    ...
    pub fn draw_body<'ui, U>(&mut self, ui: &mut U) where
        U: ?Sized + ImDrawWindow<'ui>,
    {
        ...
        let size_dirty = mem::take(&mut self.ui_size_dirty) || ui.window_is_appearing();
        let args = match ui.imgui_version_num() {
            #[cfg(taimi_imgui = "180")]
            Some(im180::VERSION_NUM) => imw::DynArgsChildWindow::new(Some(
                im180::sys::ImGuiWindowFlags_NoFocusOnAppearing
                | im180::sys::ImGuiWindowFlags_NoBringToFrontOnFocus
                ...
                | size_dirty.then_some(im180::sys::ImGuiWindowFlags_AlwaysAutoResize).unwrap_or(0)
                | (!interactive).then_some(
                    im180::sys::ImGuiWindowFlags_NoMouseInputs
                    | im180::sys::ImGuiWindowFlags_NoScrollWithMouse
        ...
        if size_dirty {
            ...
            ui.window_prepare_size(imw::ChildWindow::SIZE_NONE.with_height(line_height).cast(), ImCondition::Always);
        }
        ...
        let collapse = if self.explicit {
            ui.small_button(fl!("unhide"))
                .then_some(WindowOpen::Open)
        } else if collapsed {
        ...
        let collapse = collapse.or(if self.explicit {
            ui.same_line();
            ui.small_button(fl!("close"))
                .then_some(WindowOpen::Closed)
        } else { None });
        ...
        for (i, (key, item)) in self.items.iter().enumerate() {
            ...
            if has_footer {
                ...
                if let Some(pack_path) = pack_path {
                    let pkey = (pack_path.path.repr() as usize).rotate_left(16) ^ pack_path.root.path as usize;
                    let s = if let Some(s) = self.item_stash.try_lookup_str0(pkey) {
                        s
                    } else {
                        use taimi_hoard::loc::LocationRef;
                        let info = Controller::with_sender(|s| s.pathing.as_ref().and_then(|p|
                            p.shared.packs.packs.borrow().lookup_ref(&pack_path.root).map(|p| p.info.clone())
                        )).flatten();
    ...
    pub fn register_item_with_ui<'ui, U>(&mut self, _ui: &mut U, id: MessageKey, desc: MessageItemDesc) where
        U: ?Sized + ImDrawWindow<'ui>,
    {
    ...
    pub fn register_item(&mut self, id: MessageKey, desc: MessageItemDesc) {
        ...
        let item = MessageItemState {
            desc,
        };
        self.items.insert(id, item);
        self.ui_size_dirty = true;
        if was_empty && self.ui_state.get().window.open.is_closed() {
    ...
    #[inline]
    pub fn clear_items_matching<F>(&mut self, mut filter: F) where
