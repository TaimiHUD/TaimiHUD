...
impl PathingConfig {
        ...
        U: ?Sized + ImDrawWindow<'ui>,
    {
...

        {
            ...
            if let Some(true) = available {
                let tree_goggles = ui.begin_sidebar_tree_node(
                    ImCondition::initial(true),
                    c"pathing-config-goggles",
                    fl!("pathing-config-goggles"),
                );
        ...
        U: ?Sized + ImDrawWindow<'ui>,
    {
        ...
        match Self::slider_opt_setting_with_initial(ui, label, value, range, initial) {
            Some(Some(SpaceSettings::NONE_F32)) if range.0 > 0.0 => Some(Some(0.0)),
            res => res,
        }
    }
        ...
        U: ?Sized + ImDrawWindow<'ui>,
    {
        ...
        if ui.checkbox("arcrender", &mut arcrender) {
            ...
            Self::set_pathing(|s| {
                s.space
                    .goggles
                    .set_enables(arcrender.then_some(flag).unwrap_or_default(), flag);
        ...
        ui.indent();
        if let Some(value) =
            Self::slider_opt_alpha(ui, fl!("pathing-config-trail-alpha"), trail_alpha, None)
        {
        ...
        if let Some(value) = Self::slider_opt_alpha(ui, fl!("pathing-config-poi-alpha"), poi_alpha, None) {
        ...
        if poi_alpha > 0.0 {
            ...
            if arcrender {
                if let Some(value) =
                    Self::toggle_setting(ui, "pathing-config-poi-scale-limit", &mut scale_poi_limit)
                {
        ...
        if arcrender {
            if with_i18n!("pathing-config-distance-order", |label| ui
                .checkbox(&label, &mut distance_ordering))
            {
        ...
        if arcrender {
            if let Some(value) =
                Self::toggle_setting(ui, "pathing-config-distance-fade-range", &mut distance_fade_range)
            {
        ...
        if poi_alpha > 0.0 && arcrender {
            ui.indent();
            if let Some(value) =
                Self::toggle_setting(ui, "pathing-config-player-overlap-poi", &mut player_overlap_poi)
            {
        ...
        if let Some(value) = Self::slider_opt_mult(
            ...
            edge_scale.unwrap_or(0.0),
            (0.1f32, 5.0),
            Some(/*GogglesSettings::DEFAULT_EDGE_SCALE*/ 0.5),
        ) {
        ...
        if let Some(value) = Self::combo_setting(ui, fl!("pathing-config-camera-source"), camera_source) {
            ...
            if value == Some(CameraSource::Goggles2) {
                machine.goggles.enabled_config.insert(
        ...
        U: ?Sized + ImDrawWindow<'ui>,
    {
        ...
        if enables.contains(GogglesEnables::PROJECT_ENABLE) {
            ...
            let mut options_commit = Self::goggles_feature_opts(
                ui,
                &mut enables,
                GogglesEnables::PROJECT_ENABLE,
                GogglesEnables::OPTIONS_PROJECT.difference(GogglesEnables::OPTIONS_PROJECT_COMPAT),
            );
            // split for visual separation/grouping
            options_commit |= Self::goggles_feature_opts(
                ui,
                &mut enables,
                GogglesEnables::PROJECT_ENABLE,
        ...
        if enables.contains(GogglesEnables::CAMERA_ENABLE) {
            let options_commit =
                Self::goggles_feature_opts_all(ui, &mut enables, GogglesEnables::CAMERA_ENABLE);
            enables_commit.insert(options_commit);
        }
        if !machine
            .goggles
            .is_enabled(GogglesEnables::CAMERA_ENABLE | GogglesEnables::CAMERA_PERSPECTIVE)
        {
            ui.unindent();
            #[cfg(todo)]
            {
            ...
            ui.text_wrapped(c"Try sliding focus down until paths stop disappearing under the ground");
            ui.indent();
        }
        if enables.intersects(
            GogglesEnables::PROJECT_ENABLE
            ...
            if let Some(map_id) = map_id {
                ...
                let value = v2.clone().or_else(|| {
                    farz_set.and_then(|v| {
                        machine
                            .fov_y()
                            .and_then(|fovy| v.reinterpret_v1_as_v2(fovy, &map_scale))
                    })
                });
                ...
                let mut new_depth = {
                    let farz_set = value
                        .clone()
                        .or(farz)
                        .unwrap_or(MapProjectionDepth::DEFAULT_FALLBACK);
                    with_i18n!("pathing-config-goggles-depth", |label| Self::slider_setting(
                        ui,
                        &label,
                        farz_set.farz,
                        (
                            GogglesMapDepth::V2_FARZ_SLIDER_START,
                            GogglesMapDepth::V2_FARZ_SLIDER_END
                        )
                    ))
                };
                let remove_msg = match (farz_set, v2) {
                    ...
                    _ if has_farz
                        && machine.goggles.is_enabled(
                            GogglesEnables::CAMERA_ENABLE | GogglesEnables::CAMERA_PERSPECTIVE,
                        ) =>
                        Some("detected"),
                ...
                if let Some(v) = new_depth {
                    let value = v.map(MapProjectionDepth::with_farz).map(GogglesMapDepth::from);
                    Self::set_pathing(|s| match value {
                        Some(value) => {
                            ...
                            if farz_set.is_some() {
                                ...
                                if machine.map_depth.is_some() {
                                    ...
                                    machine
                                        .goggles
                                        .enabled_config
                                        .remove(GogglesEnables::CAMERA_PERSPECTIVE);
                                }
                            }
                        },
                    });
                ...
                if let Some((_, _, n, f)) = machine.goggles.camera.perspective_params() {
                    ...
                    ui.text(format!(
                        " (detected {:.02}: {n:.04}\"..{f:.02}\" / {:.06}m..{:.03}m)",
                        farz_det.farz,
                        n * map_scale_i2m,
                        f * map_scale_i2m
                    ));
        ...
        if !enables_commit.is_empty() {
            ...
            match enables_commit {
                #[cfg(todo)]
                commit => machine.goggles.enabled_config ^= enables_commit,
                _commit => machine.goggles.enabled_config = enables,
            }
        }
        ...
        U: ?Sized + ImDrawWindow<'ui>,
    {
        ...
        for (i, opt) in opts.iter().enumerate() {
            ...
            let tooltip_id = match opt {
                ...
                GogglesEnables::CAMERA_PERSPECTIVE => Some(format!("pathing-config-goggles-{opt}-tip")),
                _ => None,
            };
