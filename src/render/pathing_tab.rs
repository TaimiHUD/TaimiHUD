use {
    crate::{
        ...
        fl,
        render::{element::im::prelude::*, machine::RenderMachine, RenderEvent, RenderState},
        settings::{
...
impl PathingConfig {
    ...
    fn draw_festival_opts(&mut self, ui: &Ui) {
        ...
        for festival in Festival::all() {
            ...
            let selection = with_i18n!(festival.as_str(), |name| {
                ...
                Selectable::new(title).selected(selected.unwrap_or(active)).build(ui)
            });
            if selection {
    ...
    #[cfg(feature = "goggles")]
    fn draw_goggles_opts(&mut self, ui: &Ui, machine: &mut RenderMachine) -> Option<()> {
        ...
        if !machine.goggles.is_enabled(GogglesEnables::CAMERA_ENABLE | GogglesEnables::CAMERA_PERSPECTIVE) {
            ...
            ui.text_wrapped(
                "Try sliding focus down until paths stop disappearing under the ground"
            );
            ui.indent();
        }
