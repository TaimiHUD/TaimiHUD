...
impl PathingConfig {
    ...
    fn draw_goggles_opts(ui: &Ui, machine: &mut RenderMachine) -> Option<()> {
        ...
        if let Some(map_id) = map_id {
            ...
            let mut near = near * taimi_meta::coords::MapLocalScale::INCHES_PER_METRE;
            if let Some(Some(value)) = Self::slider_setting(ui, "near", near, /*(0.15, 1.2)*/(4.0, 32.0)) {
                Self::set_pathing(|s| {
                    #[cfg(deleteme)]
            ...
            if let Some(Some(value)) = Self::slider_setting(ui, "far", far, /*(500.0, 2500.0)*/(2114.0f32 * 8.0, 2114.0f32 * 48.0)) {
                Self::set_pathing(|s| {
                    #[cfg(deleteme)]
                    let map_depth_calibration = s.space.goggles.map_depth_calibration_mut();
                    #[cfg(deleteme)]
