...
impl FestivalState {
    pub const DEFAULT: Self = Self::new(Festivals::empty());

    pub const fn new(active: Festivals) -> Self {
        Self {
            active,
            on: Festivals::empty(),
            off: Festivals::empty(),
    ...
    pub fn set_preference(&mut self, festival: Festival, pref: Option<FestivalPreference>) {
        ...
        match pref {
            Some(true) => self.on.insert(festival),
            Some(false) => self.off.insert(festival),
            None => (),
        }
    }
