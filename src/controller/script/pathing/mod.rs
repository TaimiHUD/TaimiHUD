use {
    ...
    taimi_meta::packs::{CategoryIndex, MarkerId, MarkerIndex, MarkerPath, VisibilityFlags},
    taimi_pack::{
        attributes::{
            cell::{pack_attr, GetAttrDyn, PackKeyId, PackKeySet, PackValueCell, PackValueOf, SetAttrDyn},
            keys,
            RenderAttributes,
        },
        pack::Pack,
        script::{
            self,
            pathing::imp::{MarkerLoc, MarkerOverrides, MarkerType, PackOverrides, PackOverridesShared},
        },
    },
};
...
use {
    crate::{
        controller::script::{event::ScriptNotification, lua::LuaMessage, ScriptController},
        exports::runtime as rt,
    },
    taimi_meta::{map::MapID, packs::MapIndex},
...
impl PackPlugShared {
    ...
    pub fn new(path: PackPath) -> anyhow::Result<Self> {
        Controller::with_sender(|s| {
            s.pathing
                .as_ref()
                .and_then(|p| p.shared.packs.packs.borrow().lookup_ref(&path).cloned())
        })
        .flatten()
    ...
    pub fn with_shared_pack(load: SharedPackLoad) -> Self {
        let name = load
            ...
            .info
            .as_ref()
            .and_then(|i| i.primary_root())
            .and_then(|r| r.display_name.as_ref().map(|n| &n[..]))
    ...
    pub fn get_pack(&self) -> script::Result<Arc<Pack>> {
        self.load.loaded.borrow().pack.clone().context("pack unloaded")
    }
    pub fn get_loader(&self) -> script::Result<SharedLoaderBox> {
        self.load.loaded.borrow().loader.clone().context("pack unloaded")
    }
}
...
impl PackPlugStash {
    ...
    pub fn collect_changes_from(
        pending: &PackKeySet,
        shared: &PackPlugShared,
        marker_path: MarkerPath,
    ) -> Option<Vec<PackValueCell>> {
        let o = PackOverrides::shared_read(&shared.overrides)
            .overrides
            .get(&marker_index2loc(marker_path.path))
            .cloned();
        o.as_ref().map(MarkerOverrides::shared_read).map(|o| {
            pending
                .iter()
                .filter_map(|&key| {
                    o.get_dyn(key).and_then(|v| match v.map(|v| v.clone_dyn()) {
    ...
    #[inline]
    #[cfg(todo = "unused")]
    pub fn drain_all_changes<'a>(
        &'a mut self,
        shared: &'a PackPlugShared,
    ) -> impl Iterator<Item = (MarkerPath, Vec<PackValueCell>)> + 'a {
        Self::drain_all_changes_imp(&mut self.changes_dirty, &mut self.pending_changes, shared)
    }
    pub(super) fn drain_all_changes_imp<'a>(
        dirty: &'_ mut bool,
        pending: &'a mut BTreeMap<MarkerPath, PackKeySet>,
        shared: &'a PackPlugShared,
    ) -> impl Iterator<Item = (MarkerPath, Vec<PackValueCell>)> + 'a {
        *dirty = false;
    ...
    pub fn prune_changes(&mut self) {
        self.pending_changes.retain(|_, c| !c.is_empty());
    ...
    pub fn is_dirty(&self) -> bool {
        self.changes_dirty | !self.pending_start.is_empty()
    }
    ...
    pub fn process_changes_to_outbound(&mut self, pack_path: PackPath, shared: &PackPlugShared) {
        ...
        let changes = PackPlugStash::drain_all_changes_imp(
            &mut self.changes_dirty,
    ...
    pub fn process_outbound_pathing(&mut self) {
        ...
        let leftover = Controller::with_sender(|s| match s.pathing.as_ref() {
            Some(p) => match p.command.try_send(outgoing) {
                | Ok(()) | Err(TrySendError::Closed(..)) => PathingEvent::Nop,
...
pub fn marker_loc2index((ty, index): MarkerLoc) -> MarkerPath {
    MarkerPath::new_path(MarkerIndex::new(marker_ty2ns(ty), index as u32))
}
/// TODO: deleteme
...
impl ScriptMessage {
    ...
    pub fn marker_event(id: ScriptNotification, marker: MarkerPath, target: PackPath) -> Self {
        let args =
            vec![Box::new(Some(marker.path.repr())) as Box<dyn taimi_pack::script::lua::IntoLuaMut + Send>];
    ...
    pub fn marker_event_bool(
        ...
        let args = vec![
            Box::new(Some(marker.path.repr())) as Box<dyn taimi_pack::script::lua::IntoLuaMut + Send>,
    ...
    pub fn map_left_pack(target: PackPath, left: Option<MapIndex>) -> Self {
        ...
        let args = vec![Box::new(Some(map_id)) as Box<dyn taimi_pack::script::lua::IntoLuaMut + Send>];
        LuaMessage::NotifyScriptWith {
    ...
    pub(super) async fn do_refresh_packs() {
        ...
        let packs = packs.into_iter().map(|(path, l)| async move {
            let has = l
                .lock()
                .await
                .contains_asset(PACK_ENTRYPOINT)
