#[cfg(feature = "paths")]
use {
    crate::controller::pathing::registry::{PackIndex, PackPath, PackRegistryNs},
...
impl ScriptIndex {
    ...
    pub fn matches(self, rhs: Self) -> bool {
        match self {
            ...
            l => {
                ...
                match l.namespace() {
                    Self::NS_MISC if l.get_wildcard_namespace() == rns => true,
    ...
    pub fn is_wildcard(self) -> bool {
        matches!(
            self,
            Self::WILDCARD_MISC | Self::WILDCARD_PLUG | Self::WILDCARD_PACK
        )
    }
...
impl From<ScriptIndex> for u32 {
    #[inline(always)]
    fn from(v: ScriptIndex) -> Self {
        v.index
    }
}
impl<T> AsPrimitive<T> for ScriptIndex
where
    T: Copy + 'static,
    u32: AsPrimitive<T>,
{
...
impl LoadedScriptNs {
    ...
    const BITS_NAMESPACE: u32 = mem::size_of::<ScriptIndexNamespace>() as u32 * 8
        - ScriptIndex::SHIFT_NS
        - ScriptIndex::MASK_NS.leading_zeros();
    ...
    pub fn marker_notif_to_id(
        script: PackScriptPath,
        event: ScriptEventPath,
        arg: ScriptEventArg,
    ) -> ScriptEventId {
...
impl NamespaceTryConvTo<ScriptIndex, PackPath> for LoadedScriptNs {
    fn try_conv_to(path: Locator<Self, ScriptIndex>) -> Option<PackPath> {
...
impl NamespaceTryConvTo<ScriptIndex, PackScriptPath> for LoadedScriptNs {
    fn try_conv_to(path: Locator<Self, ScriptIndex>) -> Option<PackScriptPath> {
        match path.path.namespace() {
...
impl NamespaceTryConvTo<ScriptIndex, PlugPath> for LoadedScriptNs {
    fn try_conv_to(path: Locator<Self, ScriptIndex>) -> Option<PlugPath> {
        match path.path.namespace() {
...
impl PartialEq<PackRegistryNs> for PackScriptNs {
    #[inline(always)]
    fn eq(&self, _: &PackRegistryNs) -> bool {
        true
    }
}
#[cfg(feature = "paths")]
impl PartialEq<PackScriptNs> for PackRegistryNs {
    #[inline(always)]
    fn eq(&self, _: &PackScriptNs) -> bool {
        true
    }
}
#[cfg(feature = "paths")]
...
pub type MarkerDestination<R = ScriptPath, E = event::ScriptNotification, T = EventArgPath> =
    Locator<R, Locator<ScriptEventPath<E>, T>>;
