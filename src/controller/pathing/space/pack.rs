...
impl SpacePackCollection {
    ...
    pub fn rebuild_entities(
        ...
        if !full_rebuild {
            let has_bvh_static = !self.bvh_static.is_empty() && &self.bvh_static[..] == &[Self::BVH_STATIC_ALL];
            if has_bvh_static && removed_count > 0 {
                // (it's fine to leave removed items here but shrug)
                self.bvh_static.retain(|i| removed.get(*i as usize).map(|r| *r).unwrap_or(true));
            }
            #[cfg(todo)]
    ...
    pub(crate) fn needs_bvh_rebuild(&self) -> bool {
        if self.render_entities.entities.is_empty() { return false }
        match self.bvh.nodes.is_empty() {
            ...
            true if self.render_entities.entities.len() > Self::BVH_MAX_STATIC && self.bvh_static.len() <= 1 => true,
            true => self.render_entities.extra.iter().filter(|e| !e.is_invalid()).nth(Self::BVH_MIN).is_some(),
