...
#[cfg(feature = "space")]
use {
    crate::{resources::shader::ShaderDescription, space::pack::render},
...
impl PathingController {
    ...
    async fn load_all(&mut self) {
        ...
        let res = self.do_load_all().await.context("Loading all paths");
        if let Err(e) = res {
            log::error!("{e:#}");
        }
    }
    ...
    pub(crate) async fn set_visible_with(
        ...
        let (_dirty, _set) = if mask.is_empty() | matches!(granularity, ToggleGranularity::Individual) {
            let set = Self::set_visible_settings(pathing, control, set);
            (control, set)
        } else {
            ...
            for c in mask_write.into_iter() {
                Self::set_visible_settings(pathing, c, Some(new_state));
            }
    ...
    fn filter_press_gameplay(&mut self, control: GameControl) -> Option<MapIndex> {
        let res = self.gameplay_map().and_then(|map_id| {
            let keyboard_available =
                crate::render::machine::RenderMachine::ui_read_captured_keyboard().is_none();
    ...
    fn load_grace_period_update(&mut self, load_started: bool) {
        ...
        if let Some(state) = state {
            ...
            if load_started {
                let timeout =
                    (Self::LOAD_PERIOD_TIMEOUT_MULT * pending as u32).min(Self::LOAD_PERIOD_TIMEOUT_MAX);
