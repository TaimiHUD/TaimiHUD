...
use {
    ...
    taimi_hoard::{loc::{LocationRef, LocationMut}, time::Timestamp},
    taimi_meta::{
        packs::{
...
pub(crate) enum PathingEvent {
    ...
    /// remove and disable
    #[cfg(feature = "paths-dyn")]
    MaskMarker { marker: MarkerId },
    #[cfg(todo)]
    UnmaskMarker { marker: MarkerId },
...
impl PathingController {
    ...
    fn process_dirty_packs(
        &mut self,
        map_id: Option<MapIndex>,
        packs_dirty: &PackSet,
    ) {
        #[cfg(feature = "paths-filter")]
        let hidden_guids = LazyCell::new(|| Self::clone_hidden_guids()
        ...
        for path in packs_dirty {
            if let Some(map_path) = map_id.map(|map| path.rel(map)) {
                ...
                #[cfg(feature = "paths-filter")]
                let hidden_ctx = hidden_guids.as_ref()
                    .map(|(h, w)| (&**h, *w));
                let _ = self.prepare_for_pack_map(map_path, true, hidden_ctx);
            }
        }
    ...
    fn setup_stats() {
        ...
        {
            rt::statistics::StatsRef::new(&interact::STATS_POLL_INTERVAL, rt::statistics::StatsUnit::Time).register(
                rt::statistics::StatsDesc::new("stats-pathing-interact", "stats-pathing-interact-poll"),
            );
        }
    }
}
