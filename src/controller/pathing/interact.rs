use {
    crate::{
        controller::{
            pathing::{
                info::MapPackInfo,
                registry::{LoadedMarkerPath, LoadedPoiPath, PackMapPath, PackPath, PoiMapPath},
                shared::{
                    interact::{
                        empty_trigger_bvh,
                        NearbyMarkers,
        ...
        fl,
        render::{
            message_window::{MessageActionDesc, MessageItemDesc},
            RenderEvent,
            RenderState,
        },
        settings::{
...
impl SpaceInteraction {
    pub fn with_poi(path: PoiMapPath, poi: &super::state::LoadedPoi) -> Self {
        ...
        let (radius, auto) = {
            ...
            (
                interaction.attr_or_default_into::<keys::TriggerRange, _>(),
                interaction.attr_or_default_into::<keys::AutoTrigger, _>(),
            )
        };
    ...
    fn is_passive<A>(&self, attrs: &A) -> bool
    where
        A: GetAttr<keys::Info>
    ...
    pub const PASSIVE_NEARBY: TriggerKind = TriggerKind::from_bits_retain(
        TriggerKind::INFO.bits() | TriggerKind::COPY.bits() | TriggerKind::SCRIPT.bits(),
    );
    ...
    pub fn is_interactive(poi: &LoadedPoi) -> bool {
        ...
        let interactive = interactive
            || poi.info().marker_info.has_attr_of::<keys::ScriptTrigger>()
            || poi.info().marker_info.has_attr_of::<keys::ScriptFocus>();
        interactive
    }
...
impl MapInteractState {
    ...
    pub fn update_entities(&mut self, maps: &LoadedMaps, map_info: &LoadedMapInfo, map_id: MapIndex) {
        ...
        let mut pois = maps
            .iter_with_info(map_info, Some(map_id))
            .flat_map(|(map_path, map, _map_info)| {
                map.lpois().iter().filter_map(move |(lpath, poi)| {
        ...
        for (i, dest) in self.entities.iter_mut().enumerate() {
            ...
            let e = SpaceInteraction::with_poi(lpath, poi);
            let prev = mem::replace(&mut dest.value, e);
            entities_dirty |= (prev.path != lpath)
                | !vec32_eq(prev.bounds.position, dest.bounds.position)
                | (prev.bounds.radius.to_bits() != dest.bounds.radius.to_bits());
        }
        if let Some(trunc) = trunc {
            ...
            let additional = pois.map(|(lpath, poi, ..)| {
                entities_dirty = true;
                BvhShape::new(SpaceInteraction::with_poi(lpath, poi))
            });
    ...
    pub fn nearest_to(&self, pos: Point3<LocalSpace>) -> Option<(Point3<LocalSpace>, f32)> {
        ...
        trigger_bvh
            .nearest_to(LocalSpace::to2(pos).into_nalg(), &self.entities[..])
...
impl InteractReactor {
    ...
    pub(super) fn handle_map_leave(&mut self, rx: &mut PathingReceiver) {
        ...
        RenderEvent::MessageDismissMatching {
            filter: Box::new(move |id: &MarkerId| {
                id.marker_path::<PackPath>()
                    .map(|p| p.path.namespace() == MarkerIndex::NS_POI)
                    .unwrap_or(false)
            }) as Box<_>,
        }
        .try_send();
    ...
    fn process_interaction(
        ...
        let behaviour = match &action {
            ...
            _ => attrs
                .clone_attr_of::<keys::Behaviour>()
                .map(|b| (b, attrs.attr_or_default::<keys::ResetLength>())),
        };
        ...
        } else if matches!(action, InteractionEventAction::Interact)
            && took_action
                .map(|a| !a.is_empty() && a.contains(TriggerKind::COPY) && !a.contains(TriggerKind::SCRIPT))
                .unwrap_or(false)
        {
            ...
            for id in ids {
                let filter = filter::GroupConfig {
                    ...
                    inverted: lpoi
                        .info()
                        .filter_attrs()
                        .attr_or_default::<keys::InvertBehaviour>()
                        .into(),
                };
    ...
    fn process_interact<'a>(
        ...
        if let &InteractionEvent::Nearby { path, loaded_path } = &event {
            if let Some(item) = self.process_nearby_info((path, loaded_path), map_info, maps) {
                ...
                RenderEvent::MessageInfo { key, item }.try_send();
            }
        }
        match event {
            ...
            ref event @ (InteractionEvent::Nearby { .. } | InteractionEvent::Interact { .. }) => {
                ...
                #[cfg(feature = "paths-lua")]
                if !allowed.is_empty()
                    && matches!(action, InteractionEventAction::AutoTrigger)
                    && matches!(event, InteractionEvent::Nearby { .. })
                {
                    // TODO: && allowed.contains(TriggerType::SCRIPT)?
                    // it'll get back to us here momentarily don't worry...
                    let sent =
                        event_tx.send(InteractionEvent::Interact { action, path, loaded_path: lpath });
                    if sent.is_ok() {
                        return PathingEvent::Nop
                    }
                }
    ...
    pub(super) fn process_gone<'a>(
        ...
        let had_info = map.lpois().lookup_ref(&lpath.unscope()).map(|lpoi| {
            lpoi.interaction_attrs().has_attr_of::<keys::Info>()
                | lpoi.interaction_attrs().has_attr_of::<keys::CopyValue>()
        });
    ...
    fn process_nearby_info<'a>(
        ...
        let copy = (!(self.config.trigger_allow_auto | self.config.trigger_allow_interact).is_empty()
            & interest.contains(TriggerKind::COPY))
        ...
        let passive_info = ((self.config.trigger_allow_auto & TriggerKind::INFO).intersects(interest)
            | copy.is_some())
        ...
        if !act_auto {
            if marker_is_auto {
                let mut act =
                    new_action(InteractionEventAction::Manual(self.config.trigger_allow_interact));
        ...
        with_i18n!("poi-activate-override-notice", |notice| act
            .set_tooltip_desc(&notice[..]));
        info_item.actions.push(act);
    ...
    async fn process_movement<'a>(
        ...
        if !self.map_interactions.trigger_bvh_dirty {
            ...
            for entity in deferred {
                let id = entity.value.poi_path();
                if !outgoing.contains_loaded_poi(id) {
                    continue
                }
                let is_nearby = entity.value.point_query_postprocess_filter(pos);
                if is_nearby {
        ...
        for entity in inrange {
            ...
            let Some(dist_dist) = entity.value.dist_dist_inrange(&pos) else {
                if let Some(path) = nearby.remove_poi(id) {
                    ...
                    let _ = rx.event_tx.send(InteractionEvent::Gone { path, loaded_path: id });
                }
                continue
            };
            ...
            if !prev_nearby {
                ...
                let sent = rx
                    .event_tx
                    .send(InteractionEvent::Nearby { path, loaded_path: id });
        ...
        if let Some((auto, passive)) = self.interest_movement() {
            ...
            let (next_mul, next_range) = match (auto, passive) {
                (false, false) => {
                    ...
                    const SLOW_BOUND_LOWER32: u32 =
                        (InteractReactor::UPDATE_INTERVAL_PASSIVE_MS << 1) as u32;
                ...
                _ if self.map_interactions.needs_trigger_bvh_rebuild() => {
                    ...
                    (
                        20,
                        Self::UPDATE_INTERVAL_PASSIVE_MS as u32
                            ..(Self::UPDATE_INTERVAL_PASSIVE_MS << 3) as u32,
                    )
                },
                _ if self.map_interactions.nearby.any() => {
                    ...
                    (
                        2,
                        Self::UPDATE_INTERVAL_MIN_MS as u32
                            ..((Self::UPDATE_INTERVAL_PASSIVE_MS << 1) as u32),
                    )
                },
                ...
                _ => (
                    5,
                    (Self::UPDATE_INTERVAL_MIN_MS << 2) as u32
                        ..(Self::UPDATE_INTERVAL_PASSIVE_MS << 1) as u32,
                ),
            };
            const UPDATE_INTERVAL_SHIFT_AMT: u32 = InteractReactor::UPDATE_INTERVAL_PASSIVE_MS as u32 * 2;
            const UPDATE_INTERVAL_SHIFT_AMT_NEG: isize =
                -(UPDATE_INTERVAL_SHIFT_AMT.next_power_of_two() as isize);
            if let Poll::Ready(pos) = rx.player_pos.poll_next_update(cx) {
                ...
                let check_pacing = resync_distance
                    .then(|| self.map_interactions.nearest_to(pos))
                    .flatten();
                if let Some((_point, dist2)) = check_pacing {
                    // checking vicinity to determine how slow we can be...
                    let upper_limit = (next_interval_ms + UPDATE_INTERVAL_SHIFT_AMT as u64)
                        .min(SLOW_BOUND_UPPER32 as u64);
                ...
                rx.player_pos
                    .set_threshold_timeout(Duration::from_millis(next_interval_ms));
                #[cfg(taimi_debug)]
                {
            ...
            InteractMessage::RefreshInfo(lpath, marker_path) => {
                let poi_path = match lpath.path.namespace() {
                    MarkerIndex::NS_POI => {
                        ...
                        Some((lpath.map_path(|p| p.index_poi_unchecked()), poi_path))
                    },
                    _ => None,
                };
