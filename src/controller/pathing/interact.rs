...
impl InteractReactor {
            ...
            if allowed.contains(TriggerKind::BEHAVIOUR) && organic {
                ...
                let until = match behaviour {
                    ...
                    | Behaviour::Taco(TacoBehaviour::ResetDailyPerCharacter) =>
                        Some(Either::Left(Timestamp::with_timestamp({
                            ...
                            (SOME_DAY.timestamp() as i64)
                                .wrapping_sub(now().timestamp() as i64)
                                .wrapping_rem_euclid(Timestamp::DAY.as_secs() as i64)
                        } as u64))),
                    Behaviour::Blish(BlishBehaviour::ResetWeekly) =>
                        Some(Either::Left(Timestamp::with_timestamp({
                            const SOME_WEEK: Timestamp =
                                Timestamp::with_timestamp(1754265600 - MANY_WEEKS.as_secs() * 13);
                            (SOME_WEEK.timestamp() as i64)
                                .wrapping_sub(now().timestamp() as i64)
                                .wrapping_rem_euclid(Timestamp::WEEK.as_secs() as i64)
                        } as u64))),
    ...
    pub(super) async fn trigger_interact_action(
        ...
        for (entity, dist_dist) in inrange {
            ...
            let sort_id = (nearby_discrete, !auto_trigger, !auto_triggered);
            let data = CmpIgnore((id, lpoi, map, map_info));
    ...
    pub(super) async fn reload_config(&mut self, _rx: &mut PathingReceiver, settings: &SettingsLock) {
