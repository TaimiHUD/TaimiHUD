use {
    crate::{
        controller::pathing::{
            registry::{LoadedTrailPath, LoadedPoiPath, LoadedCategoryPath, LoadedMarkerPath},
            PathingController,
        },
    ...
    taimi_pack::{
        attributes::{
            cell::{pack_attr, SetAttrDyn, GetAttrDynExt, PackValueOf, PackValueCell},
            keys,
        },
        category::id::IdCmpRelaxed,
        trail::TrlPath,
    },
    taimi_meta::packs::{MarkerId, MarkerIndex, MarkerPath, CategoryIndex, CategoryPath, PoiPath, VisibilityFlags},
    taimi_sync::arcs::ArcPtrCmp,
    anyhow::Context,
    std::sync::{Arc, LazyLock},
    taimi_hoard::loc::{LocationMut, LocationRef, Locator},
    taimi_meta::packs::{MapIndex, PackMapPath, PackPath},
};
#[cfg(feature = "paths-interact")]
use crate::{
    controller::pathing::{
        interact::SpaceInteraction,
...
impl PathingController {
    pub(super) async fn commit_attr_changes(&mut self, marker: MarkerId, write_attrs: &mut (dyn Iterator<Item = PackValueCell> + Send)) {
        let lpath = if let Some(path) =  marker.marker_path::<PackPath>() {
            Ok(self.gameplay_map().and_then(|map_id|
                self.map_info.find_marker_index(path, Locator::new_path(map_id))
            ).map(|lp| (Some(path), lp)))
        } else {
            marker.marker_path::<PackMapPath>().with_context(|| format!("{marker} not a pack marker path")).map(|lp|
                Some((None, lp))
            )
        };
    ...
    async fn commit_attr_keys(&mut self, lpath: LoadedMarkerPath<PackMapPath>, path: Option<MarkerPath<PackPath>>, write_attrs: &mut (dyn Iterator<Item = PackValueCell> + Send)) -> (bool,) {
        let info = self.packs.lookup_info(lpath.root.root);
        let Some(map) = self.maps.maps.get_mut(&lpath.root) else { return Default::default() };
        ...
        let fallback_cat = LazyCell::new({
            ...
            move || {
                let root_idx: Option<CategoryPath> = info.and_then(|(i, ..)| i.primary_root().map(|r| CategoryPath::new_path(r.index)));
                root_idx
                    .or_else(|| map_info.and_then(|map_info| map_info.categories().next()))
            }
        });
        ...
        match lpath.path.namespace() {
            MarkerIndex::NS_POI => {
                let poi = poi.insert(match map.pois.get_mut(lpath.path.index_poi_unchecked() as usize) {
                    Some(p) => p,
                ...
                if poi.info.category_path.path == CategoryIndex::MAX {
                    ...
                    if let Some(root) = *fallback_cat {
                        ...
                        #[cfg(todo)] {
                        poi.visibility = map_info.and_then(|i| i.category_index(root).and_then(|r| map.categories.get(r.path as usize)))
            ...
            MarkerIndex::NS_TRAIL => {
                let trail = trail.insert(match map.trails.get_mut(lpath.path.trail_index_unchecked() as usize) {
                    Some(p) => p,
                    None => return Default::default(),
                });
                if trail.info.category_path.path == CategoryIndex::MAX {
                    ...
                    if let Some(root) = *fallback_cat {
                        ...
                        #[cfg(todo)] {
                        trail.visibility = map_info.and_then(|i| i.category_index(root).and_then(|r| map.categories.get(r.path as usize)))
                            .map(|cat| cat.visibility)
                            .unwrap_or(VisibilityFlags::all());
                                }
                    }
        ...
        let mut cat = LazyCell::new({
            ...
            move || match lpath.path.namespace() {
                MarkerIndex::NS_CAT if cats.len() > cat_idx => Some(unsafe {
                    Arc::make_mut(cats).get_unchecked_mut(cat_idx)
                }),
                _ => None,
            }
        });
        let cat_lookup = LazyCell::new({
            ...
            move || {
                let load = shared.packs.packs.borrow().lookup_ref(&lpath.root.root)
        ...
        let changes = write_attrs
            .map(|v| (v.id(), PackValueDyn::from_cell_dyn(v)));
        let changes = write_attrs
            .map(|v| (v.id(), v));
        ...
        for (key, value) in changes {
            ...
            let info = poi.as_mut().map(|poi|
                &mut poi.info as &mut dyn SetAttrDyn
            ).or(trail.as_mut().map(|trail|
                &mut trail.info as &mut dyn SetAttrDyn
            )).or(cat.as_mut().map(|cat|
                &mut cat.attrs as &mut dyn SetAttrDyn
            ));
            let Some(info) = info else {
                continue
            };
            #[cfg(todo = "unnecessary")]
            let value = value.map(|v| v.into_inner())
        ...
        if trail_invalidate {
            ...
            self.space.invalidate_trail_geometry(ltp, self.map_info.lookup_mut(&lpath.root));
            info_needs_update = true;
        }
        let guids = match lpath.path.namespace() {
            _ if new_guid.is_none() => None,
            MarkerIndex::NS_POI =>
                Some(&mut map.poi_guids),
            MarkerIndex::NS_TRAIL =>
                Some(&mut map.trail_guids),
            _ => None,
        };
        ...
        if dirty | info_needs_update {
            self.rx.shared.gameplay.send_if_modified(move |gp| {
                if gp.map_id != Some(lpath.root.path) { return false }
                ...
                match lpath.path.namespace() {
                    ...
                    MarkerIndex::NS_POI => {
                        let poi = unsafe {
                            map.pois.get_unchecked(lpath.path.index_poi_unchecked() as usize)
                        };
                        ...
                        let map_poi = shared_map.as_mut().map(|map| Arc::make_mut(&mut map.pois.data))
                            .and_then(|map_pois| map_pois.get_mut(lpath.path.index_poi_unchecked() as usize));
                    ...
                    MarkerIndex::NS_TRAIL => {
                        let trail = unsafe {
                            map.trails.get_unchecked(lpath.path.trail_index_unchecked() as usize)
                        };
                        ...
                        let map_trail = shared_map.as_mut().map(|map| Arc::make_mut(&mut map.trails.data))
                            .and_then(|map_trail| map_trail.get_mut(lpath.path.trail_index_unchecked() as usize));
                ...
                if info_needs_update {
                    ...
                    ArcPtrCmp::from_mut(&mut map_info.poi_guids).clone_from_arc(
                        &map.poi_guids
                    );
                }
    ...
    fn masked_str() -> &'static taimi_pack::attributes::AttrString {
        static MASKED_ATTR_STR: LazyLock<taimi_pack::attributes::AttrString> = LazyLock::new(||
            taimi_pack::attributes::string_into("__TAIMI_MASKED")
        );
        &*MASKED_ATTR_STR
    }
    ...
    fn masked_attrs_poi() -> impl Iterator<Item = PackValueCell> + Clone {
        ...
        Self::masked_attrs_marker().chain(
            MASKED_ATTRS_POI.iter().cloned()
        )
    }
    fn masked_attrs_trail() -> impl Iterator<Item = PackValueCell> + Clone {
        ...
        Self::masked_attrs_marker().chain(
            MASKED_ATTRS_TRAIL.iter().cloned()
        )
    }
    ...
    pub async fn commit_marker_masked(&mut self, marker: MarkerId) {
        let lpath = if let Some(path) =  marker.marker_path::<PackPath>() {
            ...
            marker.marker_path::<PackMapPath>().with_context(|| format!("{marker} not a pack marker path")).map(|lp| Some((None, lp)))
        };
        let Some(Some((path, lpath))) = rt::log::warn_ok(lpath) else { return };
        // mask lpath at runtime
        let loaded_map = self.maps.maps.get_mut(&lpath.root).and_then(|map| {
            let dirty = match lpath.path.namespace() {
                MarkerIndex::NS_POI => if let Some(poi) = map.pois.get_mut(lpath.path.index_poi_unchecked() as usize) {
                    #[cfg(feature = "paths-interact")]
                    if poi.info.marker_info.has_attr_of::<keys::Info>() || SpaceInteraction::is_interactive(poi) {
                    ...
                    poi.marker_position = glamour::Point3::splat(taimi_meta::spatial::IRRELEVANT_MID);
                ...
                MarkerIndex::NS_TRAIL => if let Some(trail) = map.trails.get_mut(lpath.path.trail_index_unchecked() as usize) {
                    #[cfg(todo)] {
                ...
                MarkerIndex::NS_CAT => if let Some(cat) = Arc::make_mut(&mut map.categories).get_mut(lpath.path.index_category_unchecked() as usize) {
                    #[cfg(todo)] {
        ...
        self.rx.shared.gameplay.send_if_modified(|gp| {
            if gp.map_id != Some(lpath.root.path) { return false }
            if let (Some(loaded_map), Some(Some(map_info))) = (loaded_map, gp.info.lookup_mut(&lpath.root.root)) {
                match lpath.path.namespace() {
                    MarkerIndex::NS_POI => {
            ...
            if let (Some(loaded_map), Some(Some(map_info))) = (loaded_map, gp.state.lookup_mut(&lpath.root.root)) {
                map_info.write_with_loaded(&loaded_map);
            }
            true
        });
    }
    pub async fn allocate_loaded_marker(&mut self, path: MarkerPath<PackPath>, map_id: MapIndex) -> MarkerPath<PackMapPath> {
        ...
        self.rx.shared.gameplay.send_if_modified(|gp| {
            if gp.map_id != Some(map_path.path) { return false }
