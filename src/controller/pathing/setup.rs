...
impl PathingController {
    ...
    pub(super) fn handle_map_leave(&mut self) {
        ...
        if let Some(Some(leaving)) = leaving_pack_plugs {
            ...
            let msgs = leaving
                .into_iter()
                .map(|path| ScriptMessage::map_left_pack(path, from_map));
            for msg in msgs {
                msg.try_send();
            }
        }
    ...
    pub(super) fn prepare_for_pack_map(
        ...
        let pack_scripting = crate::controller::Controller::with_sender(|s| {
            ...
            match sshared.packs.contains_key(&script_path) {
                true => Some(true),
                false => sshared.available_packs.contains(&map_path.root).then_some(false),
            }
        })
        .flatten();
        #[cfg(feature = "paths-lua")]
        ...
        match pack_scripting {
            Some(false) => {
                let entrypoint = match self.rx.shared.packs.pack_loader_if_loaded(map_path.root) {
                    Some(Ok(loader)) => loader.try_lock_owned().ok().and_then(|mut l| {
                        let entrypoint = crate::controller::script::pathing::PACK_ENTRYPOINT;
                        let res = l.load_asset_dyn(entrypoint).with_context(|| {
                            format!(
                                "loading {}/{entrypoint}",
                                taimi_hoard::lazyfmt::or_unavail(pack_data.as_ref().map(|(_, _, i, ..)| i))
                            )
                        });
                        rt::log::warn_ok(res)
                    }),
            ...
            Some(true) => {
                let markers = self
                    ...
                    .collect::<Vec<_>>();
                ScriptMessage::map_prepared_pack(map_path.root, map_path.path.get(), markers.into_iter())
                    .try_send();
            },
            None => (),
        }
