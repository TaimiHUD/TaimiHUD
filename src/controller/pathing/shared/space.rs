...
impl SpacePackShared {
    ...
    fn setup_texture(key: &mut Option<TextureKey>, slot: &mut Option<TextureSlot>, pack_info: &SharedPackInfo, texture: Option<&AttrString>) {
        let key = match key {
            Some(key) =>
...
impl SharedPackInfo {
    #[inline]
    pub fn setup_texture(&self, key: &mut Option<TextureKey>, slot: &mut Option<TextureSlot>, texture: Option<&AttrString>) {
