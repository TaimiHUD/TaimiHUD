...
impl SharedPackInfo {
    ...
    pub fn iter_subresource_keys(
        &self,
        drain: bool,
    ) -> impl IntoIterator<Item = (AttrString, Arc<str>)> + 'static {
        let mut keys = self.allocated_keys.write().unwrap_or_else(|e| e.into_inner());
        match drain {
...
impl SharedPackDynamics {
    pub fn reset_for_pack(&self, pack: &Pack) {
        self.paths
            .reset_with_static_range(SharedPackAllocation::static_ranges_for(pack));
    }
    #[cfg(todo)]
...
pub type StaticMarkerRanges = (
    ops::RangeTo<PoiPath>,
    ops::RangeTo<TrailPath>,
    ops::RangeTo<CategoryPath>,
);
impl SharedPackAllocation {
    ...
    pub fn reserve_trail(&self) -> TrailPath {
        ...
        if path >= Self::INDEX_MAX_TRAIL {
            ...
            self.next_index_trail
                .store(TrailIndex::MAX, Self::ORDERING_RESERVE);
    ...
    pub fn reserve_category(&self) -> CategoryPath {
        ...
        if path >= Self::INDEX_MAX_CATEGORY {
            ...
            self.next_index_category
                .store(Self::INDEX_MAX_CATEGORY, Self::ORDERING_RESERVE);
    ...
    pub fn reset_with_static_range(&self, ranges: StaticMarkerRanges) {
        ...
        self.next_index_trail
            .store(trail.end.path, Self::ORDERING_RESERVE);
        self.next_index_category
            .store(cat.end.path, Self::ORDERING_RESERVE);
    }
...
impl SharedPacks {
    ...
    pub(crate) fn update_load_period(&self, pending: usize, state: bool) -> bool {
        let value =
            pending.min(Self::GRACE_COUNT_MASK) | state.then_some(Self::GRACE_BIT_WAITING).unwrap_or(0);
    ...
    pub(crate) fn update_load_state(&self, state: bool) -> bool {
        let prev = match state {
            true => self
                .load_period
                .fetch_or(Self::GRACE_BIT_WAITING, Ordering::Relaxed),
            false => self
                .load_period
                .fetch_and(!Self::GRACE_BIT_WAITING, Ordering::Relaxed),
        };
        self.notify_load_if(state, prev)
    }
