...
impl LoadedMarkerInfo {
    ...
    pub(crate) fn marker_attrs_mut(&mut self) -> &mut Arc<MarkerAttributes> {
        let render_attrs = if let Either::Left(a) = &self.attrs { Some(a.clone()) } else { None };
...
impl<A> GetAttr<A> for LoadedMarkerInfo
where
...
impl<A> SetAttr<A> for LoadedMarkerInfo
where
    A: AttrKey + AttrKeyValue,
    MarkerAttributes: GetAttr<A> + SetAttr<A>,
{
    fn unset_attr(&mut self) {
        match &mut self.attrs {
            Either::Right(a) =>
                if GetAttr::<A>::has_attr(&**a) {
                    SetAttr::<A>::unset_attr(&mut *Arc::make_mut(a))
                },
            Either::Left(r) =>
                if RenderAttributes::holds_attr_dyn_of::<A>() {
    ...
    fn set_attr(&mut self, v: A) {
        let needs_upgrade = match &self.attrs {
            Either::Right(..) => None,
            Either::Left(r) if RenderAttributes::holds_attr_dyn_of::<A>() => None,
            Either::Left(r) => Some(r.clone()),
        };
...
impl GetAttrDyn for LoadedMarkerInfo {
    ...
    fn iter_attrs_dyn(&self) -> impl Iterator<Item = Cow<'_, dyn AttrKeyValue>> {
        ...
        m.into_iter().flatten().chain(r.into_iter().flatten())
    }
}
impl SetAttrDyn for LoadedMarkerInfo {
    fn set_attr_dyn(&mut self, value: PackValueCell) -> bool {
        ...
        let needs_upgrade = match &self.attrs {
            Either::Right(..) => None,
            Either::Left(r) if unset || RenderAttributes::holds_attr_dyn(value.id()) => None,
            Either::Left(r) => Some(r.clone()),
        };
        if let Some(r) = needs_upgrade {
            let mut attrs = MarkerAttributes::default();
        ...
        if unset {
            ...
            if !attrs.has_attr_dyn(value.id()) {
                return true
            }
        }
...
impl LoadedTrailInfo {
    ...
    pub fn trail_attrs(&self) -> &Box<TrailAttributes> {
        unsafe { self.marker_info.attrs().trail.as_ref().unwrap_unchecked() }
    }
}
...
impl GetAttrDyn for LoadedTrailInfo {
    fn has_attr_dyn(&self, key: PackKeyId) -> bool {
        key == keys::TrailDataFile::pack_key_of() || self.marker_info.has_attr_dyn(key)
    }
    fn get_attr_dyn(&self, key: PackKeyId) -> Option<Cow<'_, dyn AttrKeyValue>> {
    ...
    fn clone_attr_dyn(&self, key: PackKeyId) -> Option<PackValueDyn> {
        if key == keys::TrailDataFile::pack_key_of() {
            self.trl
                .as_ref()
                .map(|v| PackValueDyn::new_boxed_dyn(keys::TrailDataFile::from(v.path.clone())))
        } else {
    ...
    fn iter_attrs_dyn(&self) -> impl Iterator<Item = Cow<'_, dyn AttrKeyValue>> {
        self.trl
            .as_ref()
            .map(|v| Cow::Borrowed(keys::TrailDataFile::from_ref(&v.path) as &_))
            .into_iter()
            .chain(self.marker_info.iter_attrs_dyn())
    }
}
impl SetAttrDyn for LoadedTrailInfo {
    fn set_attr_dyn(&mut self, value: PackValueCell) -> bool {
        if value.id() == keys::TrailDataFile::pack_key_of() {
            let v = value
                .is_valid()
                .then(|| unsafe { PackValueOf::<keys::TrailDataFile>::new_unchecked(value).to_value() })
                .flatten();
            match (&mut self.trl, v) {
                (Some(trl), Some(v)) => trl.path = v.into(),
                (trl, v) => *trl = v.map(|v| TrlPath::new(v.into())),
            }
            true
        } else {
...
impl LoadedPoiInfo {
    ...
    pub fn poi_attrs(&self) -> &Box<PoiAttributes> {
        unsafe { self.marker_info.attrs().poi.as_ref().unwrap_unchecked() }
    }
    ...
    pub fn bounds_at<U>(&self, origin: Point3<U>) -> Box3<U>
    where
        U: FloatUnit,
        f32: AsPrimitive<U::Scalar>,
    {
        if num_traits::Float::is_infinite(origin.x) {
            ...
            return Box3::new(IRR.min.as_(), IRR.max.as_())
        }
        ...
        let max_diagonal_mid = match self.poi_attrs().clone_attr_of::<keys::IconSize>() {
            ...
            Some(edge_len) if !edge_len.is_default() => (edge_len.powi(2) * (2.0f32 / 4.0f32)).sqrt(),
