use {
    ...
    std::{collections::BTreeMap, iter, sync::Arc, time::Duration},
    taimi_hoard::{
...
impl PathingController {
    pub(super) fn filter_dismiss_poi(
        ...
        let id = guid
            .as_ref()
            .map(|g| MarkerId::from(g.0.clone()))
            .or_else(|| {
                let marker_path: Option<MarkerPath> = poi_path.map(|poi_path| poi_path.pivot_from());
                #[cfg(todo)]
                let hiding_local = || hide_contexts.iter().any(|c| matches!(c, HideContext::Local(..)));
                marker_path.map(|marker_path| match &hide_contexts {
                    #[cfg(todo)]
                    c if hiding_local() => MarkerId::for_marker(path),
    ...
    pub fn filter_state_next_schedule(&mut self, _now: Option<Timestamp>) -> Option<Timestamp> {
        ...
        #[cfg(todo = "unnecessary")]
        let next_expire = next_expire.and_then(|expiry| expiry.checked_duration_since(now));
        [
            #[cfg(feature = "paths-schedule")]
            next_schedule,
            next_expire,
    ...
    #[inline]
    pub(super) fn unexpire_at(
        filter_expiry: &mut BTreeMap<MarkerId, AbortHandle>,
        item: &MarkerId,
    ) -> bool {
