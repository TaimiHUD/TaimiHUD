...
impl GogglesSettings {
    ...
    pub fn enabled(&self) -> bool {
        self.enables().contains(GogglesEnables::LENS_ENABLE | GogglesEnables::ENABLE)
    }
    #[inline]
    ...
    pub fn set_enables(&mut self, enables: GogglesEnables, mask: GogglesEnables) {
        ...
        if mask.contains(GogglesEnables::LENS_ENABLE) {
            self.goggles_enabled = Some(enables.contains(GogglesEnables::LENS_ENABLE | GogglesEnables::ENABLE));
        }
    }
    ...
    pub fn get_map_depth_calibration(&self, map_id: u32) -> Option<GogglesMapDepth> {
        let get_seen = || self.map_proj_seen.get(&map_id)
            .map(MapProjectionDepth::from)
            .map(GogglesMapDepth::from);
        self.get_map_depth_setting(map_id)
            .or_else(get_seen)
    }
    #[cfg(feature = "goggles")]
...
impl GogglesEnables {
    ...
    pub const DEFAULTS_CAMERA: Self = Self::from_bits_retain(
        Self::CAMERA_PERSPECTIVE.bits()
        | Self::CAMERA_DIR.bits()
        | Self::CAMERA_CUTSCENE.bits()
    );
    pub const DEFAULTS_PROJECT: Self = Self::from_bits_retain(
        Self::PROJECT_MAP.bits()
    );
    pub const DEFAULTS_LENS: Self = Self::empty();
    pub const ENABLES: Self = Self::from_bits_retain(
        Self::ENABLE.bits()
        | Self::FEATURE_ENABLES.bits()
    );
    ...
    pub const FEATURE_ENABLES: Self = Self::from_bits_retain(
        Self::LENS_ENABLE.bits()
        | Self::PROJECT_ENABLE.bits()
        | Self::CAMERA_ENABLE.bits()
    );
    ...
    pub const OPTIONS_PROJECT_COMPAT: Self = Self::from_bits_retain(
        Self::PROJECT_COMPAT_FLUSH.bits()
        | Self::PROJECT_COMPAT_METHOD.bits()
    );
    ...
    pub const OPTIONS_MASK: Self = Self::from_bits_retain(
        Self::OPTIONS_LENS.bits()
        | Self::OPTIONS_PROJECT.bits()
        | Self::OPTIONS_CAMERA.bits()
        //| Self::OPTIONS_ARCRENDER.bits()
    );
    #[cfg(feature = "goggles")]
    ...
    const STICKY: Self = Self::from_bits_retain(
        !Self::all().bits()
        | !Self::ENABLES.bits()
        | Self::ARCRENDER_ENABLE.bits()
    );
...
impl GogglesMapDepth {
    ...
    pub fn as_v2_preset(&self) -> Option<MapProjectionDepth> {
        match (self.far_value(), self.near_value()) {
            (Some(far), None) if far < Self::V2_FARZ_MAX =>
                Some(MapProjectionDepth::with_farz(far)),
            _ => None,
        }
    }
    ...
    pub fn reinterpret_v1_as_v2(&self, fovy: Angle, scale: &MapLocalScale) -> Option<MapProjectionDepth> {
        ...
        if self.far_value().is_some() { return None }
