...
unsafe fn imgui_bind_context() -> Option<NonNull<imgui_sys::ImGuiContext>> {
    let aapi = match addon_api() {
        Some(aapi) if aapi.imgui_context.is_null() => return None,
        Some(aapi) if imgui_sys::igGetCurrentContext() != aapi.imgui_context as *mut _ => aapi,
        _ => return None,
    };
