...
fn add_self() -> RuntimeResult<bool> {
    let Some(mut own_handle) = ExitHandle::own_handle(true) else {
        return Err(ExitHandle::HMODULE_ERR)
    };
    let mut prev_host = None;
    BootstrapState::try_write_with(|s| {
        // temporary override :3
        prev_host = Some((s.addon_host_preference, s.update_host_preference));
        s.addon_host_preference = Some(AddonHostName::All);
        s.update_host_preference = Some(None);
        false
    });
    let res = match () {
        #[cfg(feature = "extension-arcdps-codegen")]
    ...
    if let Some((prev_host, prev_updater)) = prev_host {
        BootstrapState::try_write_with(|s| {
            ...
            s.update_host_preference = prev_updater;
            false
        });
    }
...
fn get_update_url() -> Option<String> {
    ...
    match AddonHostName::ArcDPS.is_preferred_update_host() {
        ...
        Err(pref) => {
            match pref {
                Some(pref) => log::info!("skipping get_update_url, {pref} is preferred"),
                None => log::info!("skipping get_update_url, updates disabled"),
            }
            return None
        },
    }
