...
impl TextureLoader {
    ...
    pub fn mark_used(&self, key: &str) {
        match self.garbage.read() {
            Ok(garbage) if garbage.get(key).map(|used| used.generation).unwrap_or(0) != 0 => (),
            _ => return,
        }
    ...
    #[cfg(feature = "texture-loader")]
    pub fn mark_used_many<'a, I>(&self, keys: I)
    where
    ...
    pub fn collect_garbage(&self) -> Vec<TextureKey> {
        ...
        if let Ok(textures) = self.textures.read() {
            unused.extend(textures.iter().filter_map(|(key, slot)| match slot {
                ...
                TextureSlot::Inactive(..) => Some((key.clone(), SlotStatus::Weak)),
                _ => None,
            }));
        }
        ...
        if let Ok(mut garbage) = self.garbage.write() {
            ...
            garbage.extend(unused.into_iter().filter_map(|(key, status)| match status {
                SlotStatus::Dead | SlotStatus::Weak =>
                    Some((key, RecentlyUsed { generation: Self::GC_MID_AGE })),
                SlotStatus::Unused => Some((key, RecentlyUsed { generation: 1 })),
            }));
        }
        deceased
    }
    #[cfg(feature = "texture-loader")]
    pub fn cleanup_garbage<'a, I>(&self, deceased: I)
    where
...
impl TextureSlot {
    ...
    pub fn imgui_texture(&self) -> Option<ImguiTexture> {
        ...
        self.im_size()
            .map(move |size| ImguiTexture { id, size: size.cast() })
    }
        ...
        match self {
            ...
            Self::Nexus(tex) => {
                ...
                let bytes_pixel = tex
                    .resource()
                    .as_ref()
                    .map(|srv| Texture::format_bpp(srv.get_desc().Format))
                    .unwrap_or(0) as u32;
                bytes_pixel.saturating_mul(size.width).saturating_mul(size.height) as usize
            },
            _ => 0,
        }
    }
