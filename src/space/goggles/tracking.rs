use {
    ...
    std::time::Instant,
    std::sync::atomic::{AtomicPtr, AtomicU32, Ordering},
...
impl GogglesShared {
    ...
    #[inline(always)]
    pub fn get() -> *mut Self {
        static FERRET: SyncUnsafeCell<GogglesShared> =
            SyncUnsafeCell::new(GogglesShared::DEFAULT);
        FERRET.get()
    }
    #[inline]
    ...
    pub fn is_game_dx11(ctx: &Dx11Context) -> bool {
        if Self::dx11_context() != ctx.as_raw() { return false }
...
impl GogglesShared {
    ...
    pub fn dx11_context_ref() -> &'static AtomicPtr<c_void> {
        unsafe {
            &*g2!(&raw const ferret.dx11_context)
        }
    }
    #[inline(always)]
    ...
    pub(super) fn flags_insert(flags: GogglesFlags) -> GogglesFlags {
        GogglesFlags::from_bits_retain(
            Self::flags_ref().fetch_or(flags.bits(), Self::FLAGS_ORDERING)
        )
    }
    #[inline]
    pub(super) fn flags_remove(flags: GogglesFlags) -> GogglesFlags {
        GogglesFlags::from_bits_retain(
            Self::flags_ref().fetch_and(!flags.bits(), Self::FLAGS_ORDERING)
        )
    }
    #[inline]
    pub(super) fn flags_toggle(flags: GogglesFlags) -> GogglesFlags {
        GogglesFlags::from_bits_retain(
            Self::flags_ref().fetch_xor(flags.bits(), Self::FLAGS_ORDERING)
        )
    }
...
impl GogglesState {
    ...
    pub fn refresh_enables(&mut self) {
        ...
        frame_log!("refresh enables: effective={:?}; avail={:?}; new(off)={new_disables:?}", self.enabled_config_effective(), self.available);
        frame_log!("refresh enables: active={:?}; enabled={:?}", self.active, self.enabled_config);
    ...
    pub(crate) fn settings_enables(&self, settings: &SpaceSettings) -> GogglesEnables {
        ...
        if settings.camera_source == Some(CameraSource::Goggles2) {
            enables.insert(GogglesEnables::ENABLE | GogglesEnables::CAMERA_ENABLE | GogglesEnables::CAMERA_DIR);
        }
        #[cfg(deleteme)]
    ...
    pub fn latest_frame_timestamp(&self) -> Option<Instant> {
        self.is_classifying().then(||
            self.class.latest_frame_timestamp()
        ).flatten()
    }
}
...
#[cfg(feature = "goggles2-camera")]
pub(super) fn search_ferret<F, M>(data: &[u32], granularity: usize, mut filter: F, mut matcher: M, matchlen: Option<usize>) -> Option<&[u32]>
where
    F: FnMut(&[u32]) -> bool,
