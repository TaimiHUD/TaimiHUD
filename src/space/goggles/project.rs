use {
    ...
    std::{
        sync::atomic::{AtomicPtr, AtomicU32, AtomicU8},
        time::Instant,
    },
...
impl ProjectShared {
    ...
    pub(super) fn on_set_targets_prior() -> Option<(D3dPtr, D3dPtr)> {
        if !ClassShared::frame_valid() || Self::drawing().is_empty() {
            return None
        }
        Some((
    ...
    pub(super) fn on_set_targets_pre(
        ...
        let render_unbound = prev_bound_render
            .as_ref()
            .and_then(|prev| rtvs.clone().all(|rtv| rtv.as_d3d_raw() != prev).then_some(*prev));
        let render_unbound = render_unbound.and_then(|key| {
            ClassShared::with_seen2(key, |buf| buf.winner.then_some((key, buf.classification))).flatten()
        });
        ...
        let render_view_interest = if views.len() > 1 {
            rtvs.clone()
                .filter_map(|&key| {
                    ClassShared::with_seen2(key, |buf| {
                        let score = match buf.classification {
                            _ if !buf.winner => 0xf,
                            BufferClass::World if ProjectAction::Draw.is_in(drawing) => 0,
                            BufferClass::Pretty | BufferClass::Fallback
                                if ProjectAction::Draw.is_in(drawing) =>
                                1,
                            BufferClass::Target => {
                                #[cfg(todo)]
        ...
        for ((cls, target), act, dv) in draws {
            let target = unsafe { RenderTargetView::from_d3d_raw_ref(&target) };
            let dv = dv.as_ref().map(|dv| unsafe { DepthView::from_d3d_raw_ref(dv) });
    ...
    fn log_view_of(name: &dyn core::fmt::Display, view_ptr: D3dNn, buf: &super::class::BufferInfo) {
        let winner = buf
            .winner
            .then(|| ClassShared::seen_class(buf.kind, buf.classification))
            .flatten();
        ...
        let bcount = lazyfmt::or_empty(
            winner.map(|(_, _, bcount, _)| lazyfmt::MaybeFmt::new(move |f| write!(f, "/{bcount}"))),
        );
        let bgen = lazyfmt::or_empty(
            winner.map(|(_, _, _, bgen)| lazyfmt::MaybeFmt::new(move |f| write!(f, "/{bgen}"))),
        );
    ...
    fn reach_for(reach: BufferClass) -> Option<(D3dNn, Option<D3dNn>)> {
        let mut dv = None;
        let newtarget = ClassShared::seen_class(BufferKind::RenderTarget, reach).map(|(key, ..)| key);
    ...
    pub(super) fn on_set_state_prior() -> Option<(
        ...
        rtv.and_then(|rtv| {
            ...
            let (empty, rt) = match &dv {
                None | Some((None, ..)) => {
                    let rt = ClassShared::with_seen2(rtv, |buf| {
                        buf.winner.then_some((
                            buf.classification,
                            buf.depth_binds_count_write(),
                            buf.depth_binds_count_disabled() + buf.depth_binds_null,
                        ))
                    })
                    .flatten();
                    (rt.is_none(), rt)
    ...
    pub(super) fn on_set_state_pre(
        ...
        let (drawing, method) = (Self::drawing(), Self::method());
        let dv = LazyCell::new(|| ClassShared::bound_depth_ptr());
        let after = LazyCell::new(|| {
            dv.and_then(|dv| {
                ClassShared::with_seen2(dv, |buf| {
                    (
                        buf.depth_binds_count_write(),
                        buf.depth_binds_count_disabled(),
                        buf.depth_binds_null,
                        buf.flags,
                    )
                })
            })
        ...
        let target_is_depthless = LazyCell::new(|| {
            let (has_written_depth, prev_disabled) = match (depth_prior, rt_prior) {
                ...
                (Some((_, writes, disabled)), _)
                    if disabled.saturating_sub(writes) >= Self::DEPTHLESS_ENOUGH =>
                    return true,
                (_, Some((_, writes, disabled)))
                    if disabled.saturating_sub(writes) >= Self::DEPTHLESS_ENOUGH =>
                    return true,
                // wait for at least one depth-enabled write...
                (Some((_, writes @ 0..=Self::DEPTHLESS_MAX_WRITE, disabled)), _) => (writes, disabled),
                #[cfg(todo = "unnecessary")]
                (_, Some((_, writes @ 0..=1, disabled))) => (writes, disabled),
                _ => return false,
            };
            let after = &*after;
            #[cfg(todo = "unnecessary")]
        ...
        let draws = ProjectAction::iter_bits(drawing).filter_map(|act| {
            method
                .actions_on_state(act, cls)
                .min()
                .and_then(|(score, action)| {
                    if let Some((_, _, _, flags)) = *after {
                        if !flags.contains(BufferStateFlags::CLEARED_DEPTH) {
                            return None
                        }
                    }
        ...
        for (target, dv, act, cls) in draws {
            let rtv = unsafe { RenderTargetView::from_d3d_raw_ref(&target) };
            let dv = dv.as_ref().map(|dv| unsafe { DepthView::from_d3d_raw_ref(dv) });
    ...
    pub(super) fn on_new_frame() {
        ...
        let state = match state_lock.as_mut().and_then(|s| s.as_mut()) {
            Some(state) =>
                if state.machine.goggles.is_enabled(GogglesEnables::PROJECT_ENABLE) {
                    Some(state)
                } else {
                    None
                },
    ...
    fn draw(
        ...
        let mut state = match state_lock.as_mut().and_then(|s| s.as_mut()) {
            Some(state) =>
                if state.machine.goggles_project_draw_start(what, cls) {
                    Some(state)
                } else {
                    None
                },
        ...
        let draw = match (what, state.as_mut()) {
            ...
            (ProjectAction::DrawMinimap, Some(state))
                ...
                None,
            (Drawing::REFLECT | Drawing::REFLECT_BELOW, Some(state))
                if cls == BufferClass::Reflection
                    && !state
                        .machine
                        .goggles
                        .is_enabled(GogglesEnables::PROJECT_REFLECTIONS) =>
                None,
        ...
        if let Some(engine) = engine {
            ...
            match (what, cls, target_dv) {
                ...
                (ProjectAction::DrawObscured, ..) => {
                    ...
                    if desc.goggles.target_depthview.is_none() {
                        desc.goggles.target_depthview = LensShared::read_selected().and_then(|l| {
                            ClassShared::with_seen2(l, |buf| {
            ...
            if frame_log!(::is_enabled()) {
                ...
                Self::log_view(
                    "env",
                    Self::bound_render().or(ClassShared::bound_render_primary_ptr()),
                );
    ...
    fn detect_clear(context: &DeviceContext0, target: &RenderTargetView) {
        target.clear_rgba(context, Self::DETECT_COLOUR);
    }

    #[inline(always)]
    pub fn bound_render_ref() -> &'static AtomicPtr<c_void> {
        unsafe { &*g2!(&raw const ferret.project2.bound_render) }
    }
    fn bound_render() -> D3dPtr {
        NonNull::new(Self::bound_render_ref().load(GogglesShared::FLAGS_ORDERING))
    }
    ...
    pub fn drawing_ref() -> &'static AtomicU32 {
        unsafe { &*g2!(&raw const ferret.project2.drawing) }
    }
    fn read_drawing() -> Drawing {
        Drawing::from_bits_retain(Self::drawing_ref().load(GogglesShared::ENABLED_ORDERING))
    }
    ...
    fn drawing() -> Drawing {
        Drawing::from_bits_retain(Self::drawing_ref().load(GogglesShared::FLAGS_ORDERING))
    }
    fn insert_drawing(v: Drawing) -> Drawing {
        Drawing::from_bits_retain(Self::drawing_ref().fetch_or(v.bits(), GogglesShared::FLAGS_ORDERING))
    }
    fn mask_drawing(v: Drawing) -> Drawing {
        Drawing::from_bits_retain(Self::drawing_ref().fetch_and(!v.bits(), GogglesShared::FLAGS_ORDERING))
    }
    ...
    pub fn map_open_ref() -> &'static AtomicU8 {
        unsafe { &*g2!(&raw const ferret.project2.map_open) }
    }
...
impl ProjectMethod {
    ...
    fn actions_on_unbind(
        self,
        act: Drawing,
        cls: BufferClass,
    ) -> impl Iterator<Item = (i32, MethodAction)> {
        let primary = match (act, cls, self) {
            (
                Drawing::OBSCURED_SHADOWED,
                BufferClass::Shadowbox,
                ProjectMethod::Early | ProjectMethod::Fuzzy,
            ) => Some((0, None)),
            (
                ProjectAction::DrawObscured,
                BufferClass::World,
                ProjectMethod::Early | ProjectMethod::Fuzzy,
            ) if ClassShared::with_seen_class(BufferKind::DepthView, BufferClass::World, |_k, buf| {
                buf.bind_count <= 1
            })
            .unwrap_or(true) =>
                None,
            ...
            (
                ProjectAction::DrawObscured,
                BufferClass::Pretty,
                ProjectMethod::Shiny | ProjectMethod::Late,
            ) => Some((0, None)),
            ...
            (ProjectAction::DrawMinimap, cls, method) => {
                ...
                let next = match method {
                    ...
                    ProjectMethodMinimap::Eager =>
                        ClassShared::with_current_dv(|_, buf| buf.winner.then_some(buf.classification))
                            .flatten(),
                    _ => None,
                }
                .unwrap_or(BufferClass::Unknown);
                return method
                    .minimap_action_on_unbind(cls, next)
                    .map(|a| (0, a))
                    .into_iter()
            },
            (ProjectAction::DrawMap, cls, method) => {
                ...
                return method
                    .map_action_on_unbind(cls, next, next_bind_count)
                    .map(|a| (0, a))
                    .into_iter()
            },
            ...
            (ProjectAction::Shadowbox, _, Self::Shiny)
                if ClassShared::with_current_dv(|_, buf| {
                    buf.bind_count > 1 && buf.winner && matches!(buf.classification, BufferClass::World)
                })
                .unwrap_or(false) =>
                Some((0x10, Some(BufferClass::Shadowbox))),
            (ProjectAction::Shadowbox, BufferClass::Shadowbox, Self::Pretty) => None,
            (ProjectAction::Draw, BufferClass::Pretty, Self::Pretty) => Some((0, None)),
            (ProjectAction::Draw, _, Self::Pretty) => None,
            (ProjectAction::Draw, _, Self::Conservative) => None,
            (ProjectAction::Draw, BufferClass::Pretty, Self::Fuzzy | Self::Compatibility)
                if ClassShared::with_current_dv(|_, buf| {
            ...
            (ProjectAction::Draw, BufferClass::World, Self::Compatibility)
                if ClassShared::with_seen_class(
                    ...
                    BufferClass::World,
                    |_k, buf| buf.depth_binds_count_write() <= ProjectShared::DEPTHLESS_MAX_WRITE,
                )
                .unwrap_or(true) =>
                None,
            (Drawing::ONION, BufferClass::World, Self::Early)
                if ClassShared::with_seen_class(
                    BufferKind::DepthView,
                    BufferClass::World,
                    |_k, buf| buf.depth_binds_count_write() <= ProjectShared::EARLY_MIN_WRITE,
                )
                .unwrap_or(true) =>
                None,
            (ProjectAction::Draw, BufferClass::World, Self::Fuzzy | Self::Shiny)
                if ClassShared::with_seen_class(
                    BufferKind::DepthView,
                    BufferClass::World,
                    |_k, buf| buf.depth_binds_count_write() <= 1,
                )
                .unwrap_or(true) =>
                None,
            (
                act,
                cls,
                Self::Fuzzy | Self::Conservative | Self::Compatibility | Self::Shiny | Self::Pretty,
            ) if matches!(
                (act, cls),
                (ProjectAction::Draw, BufferClass::World)
                    | (ProjectAction::DrawMinimap, BufferClass::Minimap)
            ) =>
                Some((0x20, None)),
            (ProjectAction::Draw, BufferClass::Fallback, _)
                if ClassShared::can_expect_upcoming(BufferKind::RenderTarget, BufferClass::World)
                    != Some(true) =>
                Some((0xff, None)),
            (ProjectAction::Shadowbox, BufferClass::Shadowbox, _) => Some((0x08, None)),
            _ => None,
        };
        primary
            .map(|(score, retarget)| (score, MethodAction { retarget, ..Default::default() }))
            .into_iter()
        //.chain(fallback)
    }
    fn actions_on_state(self, act: Drawing, cls: BufferClass) -> impl Iterator<Item = (i32, MethodAction)> {
        let primary = match (act, cls, self) {
            ...
            #[cfg(todo)]
            (ProjectAction::DrawMinimap, cls, method) =>
                return ProjectMethodMinimap::from(method)
                    .minimap_action_on_state(cls)
                    .map(|a| (0, a))
                    .into_iter(),
            #[cfg(todo)]
            (ProjectAction::DrawMap, cls, method) =>
                return ProjectMethodMap::from(method)
                    .map_action_on_state(cls)
                    .map(|a| (0, a))
                    .into_iter(),
            ...
            | (ProjectAction::DrawMinimap, BufferClass::Target, Self::Compatibility) => Some((0, None)),
            _ => None,
        };
        primary
            .map(move |(score, retarget)| {
                (score, MethodAction {
                    retarget,
...
impl ProjectMethodMinimap {
    fn minimap_action_on_unbind(self, prior: BufferClass, next: BufferClass) -> Option<MethodAction> {
        match self {
            ...
            Self::Slow if matches!(prior, BufferClass::Minimap) =>
                Some(MethodAction { ..Default::default() }),
            _ => None,
        }
    }
    #[cfg(todo)]
...
impl From<ProjectMethod> for ProjectMethodMinimap {
    fn from(method: ProjectMethod) -> Self {
        match method {
            ProjectMethod::Compatibility => Self::Slow,
            #[cfg(todo)]
            | ProjectMethod::Late => Self::SlowShiny,
            | ProjectMethod::Fuzzy
            | ProjectMethod::Shiny
            | ProjectMethod::Pretty
            | ProjectMethod::Early => Self::Eager,
            _ => Self::Slow,
        }
    }
}
...
impl From<ProjectMethod> for ProjectMethodMap {
    fn from(method: ProjectMethod) -> Self {
        match method {
            #[cfg(todo)]
            ProjectMethod::Conservative => Self::Conservative,
            ProjectMethod::Compatibility => Self::Reliable,
            | ProjectMethod::Early | ProjectMethod::Late => Self::Eager,
            #[cfg(todo)]
            | ProjectMethod::Fuzzy | ProjectMethod::Shiny | ProjectMethod::Pretty => Self::Shiny,
            _ => Self::Reliable,
        }
    }
}
    ...
    fn world_clear(&self) -> Option<[f32; 4]> {
        ClassShared::with_seen_class(BufferKind::RenderTarget, BufferClass::World, |_k, buf| {
            buf.state
                ...
                .then_some(buf.state.clear_colour)
        })
        .flatten()
    }
    fn bind_count(&self, kind: BufferKind, cls: BufferClass) -> (u32, bool) {
...
impl RenderMachine {
    fn goggles_project_draw_start(&mut self, what: Drawing, cls: BufferClass) -> bool {
        if !self.goggles.is_enabled(GogglesEnables::PROJECT_ENABLE) {
            return false
        }
        ...
        let time_passed = match cls {
            BufferClass::Target | BufferClass::Minimap | BufferClass::FrameBuffer => true,
            _ => false,
        };
        if time_passed {
