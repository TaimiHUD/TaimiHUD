    ...
    fn draw(
        ...
        if let Some(engine) = engine {
            let what = match (what, cls) {
                (Drawing::SPACE, BufferClass::Shadowbox) =>
                    Drawing::SHADOWBOX_OUTLINE,
                (what, _) => what,
            };
            let target_dv = match what {
                ...
                ProjectAction::DrawMap if matches!(cls, BufferClass::World) =>
                    dv.map(|dv| *dv.as_d3d_raw()),
                ProjectAction::DrawObscured if matches!(cls, BufferClass::Target) && engine.drawing.wants_onion(&engine.arcdata) =>
                    None,
                Drawing::REFLECT | Drawing::REFLECT_BELOW | ProjectAction::Shadowbox if matches!(cls, BufferClass::Shadowbox | BufferClass::Reflection) =>
                    dv.map(|dv| *dv.as_d3d_raw()),
                ProjectAction::Draw | Drawing::ONION if !state.machine.goggles.is_enabled(GogglesEnables::LENS_ENABLE) => None,
