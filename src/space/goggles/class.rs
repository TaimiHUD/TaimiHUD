use {
    super::{g2, GogglesFlags, GogglesShared},
    crate::{
        exports::runtime::{
            self as rt,
            statistics::{StatsCounter, StatsDesc, StatsRef, StatsUnit},
        },
...
pub(super) unsafe fn set_targets(
    ...
    let uaviews = _uavs.iter().filter_map(|uav| uav.as_ref());
    ...
    #[cfg(todo = "unnecessary")]
    let views = views.chain(uaviews.map(|v| (View::from_d3d_ref(v), BufferKind::UnorderedAccessView)));
    ...
    for (view, kind) in views.clone() {
        ...
        let buf = match seen.entry(view_ptr) {
            ...
            btree_map::Entry::Vacant(e) => {
                ...
                let buf_desc = resource
                    .as_ref()
                    .and_then(|r| r.as_texture2())
                    .map(|t2| t2.desc());
                e.insert(BufferInfo {
                    ...
                    size: buf_desc
                        ...
                        .unwrap_or(BufferInfo::EMPTY_SIZE),
                    format: buf_desc
                        .as_ref()
                        .map(|desc| desc.Format)
                        .unwrap_or(BufferInfo::EMPTY_FORMAT),
                    last_seen: now,
                    first_seen: now,
                    kind,
        ...
        match (buf.winner, buf.kind, buf.classification) {
            (_, _, BufferClass::Reflection | BufferClass::Shadowbox) => is_early_bind = buf.bind_count == 1,
            (true, kind @ BufferKind::RenderTarget, cls @ BufferClass::Target)
                if Some(buf.bind_count)
                    == ClassShared::seen_class(kind, cls).map(|(_, _, bcount, ..)| bcount) =>
            {
                // your days are numbered...
                ClassShared::bind_generation_limit_ref().store(
                    next_bind_generation.wrapping_add(ClassShared::FRAME_END_BREATHING_ROOM),
                    GogglesShared::FLAGS_ORDERING,
                );
            },
            _ => (),
        }
    ...
    ClassShared::bound_render_primary_ref().store(
        nn::nonnull_ptr_mut(render_primary_ptr),
        GogglesShared::FLAGS_ORDERING,
    );
...
impl GogglesFlags {
    ...
    fn class_cleared_inconsistent(self) -> bool {
        (self & (ClassShared::FLAGS_CLEARS | GogglesFlags::CLASS_CLEARED_INCONSISTENT))
            == GogglesFlags::CLASS_CLEARED_INCONSISTENT
    }
}
pub(super) unsafe fn set_state(_context: &Dx11Context, state: Option<&DepthState>, stencil_ref: u32) {
    if !ClassShared::classifying_depth() {
        return
    }
    ...
    if let Some(desc) = &desc {
        ...
        if let Some(depth) = depth {
            ...
            if render_ptr_primary.is_some() || !ClassShared::classifying_render() {
                let keys = IntoIterator::into_iter([render_ptr_primary, depth_ptr]).flatten();
                ClassShared::bufs_mut(keys, |buf, _key| {
                    if is_strong_state {
                        ...
                        } else if let (BufferKind::DepthView, Some(render_ptr)) =
                            (buf.kind, render_ptr_primary)
                        {
...
#[inline]
pub(super) unsafe fn clear_colour(_context: &Dx11Context, view: &RenderTargetView, colour: &[f32; 4]) {
    if !ClassShared::frame_valid_pre() {
        frame_log!(";; ignoring ({:?})", GogglesShared::flags());
        return
    }
...
impl ClassShared {
    ...
    fn expect_present_count_ref() -> &'static AtomicU32 {
        unsafe { &*g2!(&raw const ferret.class2.expect_present_count) }
    }

    #[inline(always)]
    fn frame_count_ref() -> &'static AtomicU32 {
        unsafe { &*g2!(&raw const ferret.class2.frame_count) }
    }
    #[inline(always)]
    ...
    fn bind_generation_ref() -> &'static AtomicU32 {
        unsafe { &*g2!(&raw const ferret.class2.bind_generation) }
    }
    #[inline(always)]
    ...
    fn bind_generation_limit_ref() -> &'static AtomicU32 {
        unsafe { &*g2!(&raw const ferret.class2.bind_generation_limit) }
    }
    #[inline(always)]
    ...
    fn bound_depth_ref() -> &'static AtomicPtr<c_void> {
        unsafe { &*g2!(&raw const ferret.class2.bound_depth) }
    }
    #[inline(always)]
    pub(super) fn bound_depth_ptr() -> D3dPtr {
        NonNull::new(Self::bound_depth_ref().load(GogglesShared::FLAGS_ORDERING))
    }

    #[inline(always)]
    fn bound_render_primary_ref() -> &'static AtomicPtr<c_void> {
        unsafe { &*g2!(&raw const ferret.class2.bound_render_primary) }
    }
    #[inline(always)]
    pub(super) fn bound_render_primary_ptr() -> D3dPtr {
        NonNull::new(Self::bound_render_primary_ref().load(GogglesShared::FLAGS_ORDERING))
    }

    #[inline(always)]
    fn inconsistent_clears_ref() -> &'static AtomicU32 {
        unsafe { &*g2!(&raw const ferret.class2.inconsistent_clears) }
    }
...
impl ClassShared {
    ...
    pub const ENABLES: GogglesEnables = GogglesEnables::from_bits_retain(
        GogglesEnables::LENS_ENABLE.bits() | GogglesEnables::PROJECT_ENABLE.bits(),
    );
    ...
    const FLAGS_CLEARS: GogglesFlags = GogglesFlags::from_bits_retain(
        GogglesFlags::CLASS_CLEARED_COLOUR.bits() | GogglesFlags::CLASS_CLEARED_DEPTH.bits(),
    );
    ...
    fn frame_valid_class() -> bool {
        match GogglesShared::flags() & Self::FLAGS_FRAME_VALID {
            ...
            flags =>
                Self::frame_valid_present().unwrap_or(flags.contains(GogglesFlags::CLASS_FRAME_ONGOING)),
        }
    }
    ...
    pub(super) fn reset_end() {
        ...
        let winners = unsafe { &mut *g2!(&raw mut ferret.class.winners) };
        ...
        let next_classify_tick = is_ingame && {
            ...
            if enabled.contains(GogglesEnables::PROJECT_ENABLE) {
                if is_ingame_world {
                    ...
                    classify_tick |=
                        !winners.contains_key(&(BufferKind::RenderTarget, BufferClass::Minimap));
                }
        ...
        let has_present_count =
            Self::expect_present_count_ref().load(order) != Self::PRESENT_COUNT_DISABLED;
        ...
        if flags.contains(GogglesFlags::CLASS_CLEARED_INCONSISTENT) {
            ...
            if !has_present_count
                && !winners.contains_key(&(BufferKind::DepthView, BufferClass::Shadowbox))
                    | !winners.contains_key(&(BufferKind::DepthView, BufferClass::Reflection))
            {
    ...
    pub(super) fn reset_frame(&mut self) {
        ...
        let assocs = mem::take(&mut self.assoc)
            .into_iter()
        ...
        for (key, assoc_key, assoc_cls, assoc_flags, assoc_colour) in assocs {
            ...
            if assoc_flags
                .difference(rt_flags)
                .contains(BufferStateFlags::CLEARED_COLOUR)
            {
    ...
    pub(crate) fn with_seen2<R, F: FnOnce(&BufferInfo) -> R>(key: D3dNn, f: F) -> Option<R> {
        if g2!(*&ferret.class.cleanup_time) {
            return None
        }
        let seen = unsafe { &*g2!(&raw const ferret.class.seen) };
        seen.get(&key).map(f)
    }
    #[inline]
    pub(crate) fn with_seen_class<R, F: FnOnce(D3dNn, &BufferInfo) -> R>(
        kind: BufferKind,
        cls: BufferClass,
        f: F,
    ) -> Option<R> {
        Self::seen_class(kind, cls).and_then(|(key, ..)| Self::with_seen2(key, |buf| f(key, buf)))
    }
    #[inline]
    ...
    pub(super) fn with_current_dv<R, F: FnOnce(D3dNn, &BufferInfo) -> R>(f: F) -> Option<R> {
        let seen = unsafe { &*g2!(&raw const ferret.class.seen) };
        Self::bound_depth_ptr().and_then(|key| seen.get(&key).map(|buf| f(key, buf)))
    }
    pub(super) fn seen_class(kind: BufferKind, cls: BufferClass) -> Option<WinnerInfo> {
        if g2!(*&ferret.class.cleanup_time) {
            return None
        }
        let winners = unsafe { &*g2!(&raw const ferret.class.winners) };
...
impl ClassShared {
    pub(crate) fn iter_ui() -> impl Iterator<Item = (D3dNn, BufferInfo)> {
        ...
        let clsok = |cls: BufferClass| match cls {
            ...
            BufferClass::New => false,
            _ => true,
        };
        let mut candidates = seen
            .iter()
            .filter(|(_k, buf)| {
                buf.age() >= BufferInfo::TIME_AGE_UI && !buf.is_lost(now) && clsok(buf.classification)
            })
    ...
    pub(super) unsafe fn query_buf<'a, F>(mut f: F) -> Option<(&'a D3dNn, &'a BufferInfo)>
    where
        F: FnMut(&BufferInfo) -> bool,
    {
        ...
        let now = Self::frame_count();
        seen.iter()
            .filter(|(_k, buf)| f(buf) && !buf.is_lost(now))
    ...
    pub fn query_dv<'a>(cls: BufferClass) -> Option<InterfaceRef<'a, d3d11::ID3D11DepthStencilView>> {
        ...
        .map(|(k, _)| unsafe { InterfaceRef::from_raw(*k) })
    }
    pub fn query_rt_by_dv<'a>(cls: BufferClass) -> Option<InterfaceRef<'a, d3d11::ID3D11RenderTargetView>> {
        let assoc = unsafe {
            Self::query_buf(|buf| {
                matches!(buf.kind, BufferKind::DepthView)
                    && buf.associated.is_some()
        ...
        match seen.get(&assoc) {
            Some(buf) if matches!(buf.kind, BufferKind::RenderTarget) && !buf.is_lost(now) =>
                Some(unsafe { InterfaceRef::from_raw(assoc) }),
            _ => None,
        }
    }
            ...
            let (wkey, won, winner) = if let Some(buf) = this.seen.get_mut(&key) {
                ...
                let w = match (set, cls) {
                    (
                        ...
                        BufferClass::World
                        | BufferClass::Target
                        | BufferClass::Shadowbox
                        | BufferClass::Reflection
                        | BufferClass::Pretty
                        | BufferClass::Fallback
                        | BufferClass::Minimap,
                    ) => {
                        buf.state.winner = true;
                        Some((
                            key,
                            buf.state.classification_score,
                            buf.state.bind_count,
                            buf.state.bind_generation,
                        ))
                    },
                    _ => {
                        buf.state.winner = false;
    ...
    #[inline(always)]
    fn bufs_mut<F, I>(keys: I, mut f: F)
    where
    ...
    #[inline(always)]
    #[cfg(todo)]
    fn bound_bufs_mut<F>(f: F)
    where
        F: FnMut(&mut BufferInfo, D3dNn),
    {
        Self::bufs_mut(Self::bound_views_keys(), f)
    }
    fn bound_bufs_all_mut<F>(mut f: F)
    where
        F: FnMut(&mut BufferInfo, D3dNn),
    {
        let seen = unsafe { &mut *g2!(&raw mut ferret.class.seen) };
        ...
        for (key, buf) in seen {
            if !buf.state.is_bound(bind_generation) {
                continue
            }
            f(buf, *key);
        }
...
#[derive(
    Debug, Copy, Clone, PartialEq, Eq, PartialOrd, Ord, Hash, strum::IntoStaticStr, strum::VariantArray,
)]
...
impl BufferInfo {
    ...
    fn reacquire<K>(seen: &BTreeMap<K, Self>, now: u32, key: &K, acquire: BufferClass) -> Option<K>
    where
        K: Clone + Eq + Ord,
    {
        ...
        let mut candidates = seen.iter().filter(|(_k, buf)| {
            *_k != key
                ...
                && buf.age() >= BufferInfo::TIME_AGE_MIN
                && !buf.is_gone(now)
        });
    ...
    fn prepare_state(&mut self, commit: bool, is_ingame: bool) {
        ...
        match self.classification {
            BufferClass::New if age >= Self::TIME_AGE_MIN => self.classification = BufferClass::Unknown,
            ...
            _ if commit
                && age > BufferInfo::TIME_AGE_RECLASSIFY
                && !self.winner
                && self.classification_score != BufferState::SCORE_MANUAL =>
                self.classify_commit(is_ingame),
            _ => (),
        }
    }
    ...
    fn record_association(&mut self, target: D3dNn, key: D3dNn, mut strong: Option<bool>) {
        match self.associated {
            ...
            Some(assoc) if assoc == target => strong = strong.or(Some(true)),
            Some(assoc) if self.state.associated != Some(assoc) => self.state.associated = Some(assoc),
            _ => (),
        }
    ...
    pub(crate) fn classify(&self, is_ingame: bool) -> Option<(BufferClass, i32)> {
        ...
        match cls {
            Some((cls, score)) if score > 0 => Some((cls, score)),
            _ => None,
        }
    }
    ...
    fn classify_score_target(&self) -> i32 {
        ...
        match self.depth_binds_count_disabled() {
            0 => confidence -= 2,
            disabled if disabled >= 5 || disabled >= self.depth_binds_count() / 2 => confidence += 24 + 12,
            _ => confidence -= 24,
        }
        ...
        match self.depth_binds_count_write() {
            0 => confidence -= 4,
            1 => confidence += 2,
            amt if amt >= self.depth_binds_count() / 4 => confidence -= 16,
            _ => confidence -= 6,
        }
    ...
    fn classify_score_space(&self) -> i32 {
        ...
        match (
            self.flags.contains(BufferStateFlags::CLEARED_COLOUR),
            self.clear_colour,
        ) {
        ...
        match self.depth_binds_count_disabled() {
            0..=1 => confidence -= 4,
            disabled_count if disabled_count <= self.depth_binds_count() / 2 => confidence += 6,
            _ => (),
        }
        if self.flags.contains(BufferStateFlags::DEPTH_WRITE_LE) {
            confidence += 6
    ...
    fn classify_score_fallback(&self) -> i32 {
        let mut confidence = match self.bind_count {
            2 if self.depth_binds_null == 4 && self.depth_binds_count == 4 => 12i32,
            _ => -48i32,
        };
        if self.flags.contains(BufferStateFlags::DEPTH_STENCIL_REF) {
            confidence -= 32
    ...
    fn classify_score_unsupported(&self) -> i32 {
        ...
        match ((expected.x / expected.y) - (w as f32 / h as f32)).abs() <= 2e-4f32 {
            true => -32,
            false => match self.classify_format_ok() {
                false => -28,
                true => 50,
            },
        }
    ...
    fn is_rt_format_ok(format: dxgi::DXGI_FORMAT) -> bool {
        match format {
            ...
            f if f.0 >= dxgi::DXGI_FORMAT_BC1_TYPELESS.0 && f.0 <= dxgi::DXGI_FORMAT_BC5_SNORM.0 => false,
            _ => true,
        }
    }
}
...
impl BufferState {
    ...
    pub fn mark_unbound(&mut self, bind_generation: u32) -> bool {
        if self.bind_generation < bind_generation {
            return false
        }
        #[cfg(todo)]
        {
    ...
    pub fn record_depth(&mut self, depth: DepthSummary, stencil_ref: u32) {
        ...
        match depth {
            DepthSummary::Discard => self.depth_binds_disabled += 1,
            DepthSummary::Disabled => self.depth_binds_disabled += 2,
            DepthSummary::ReadOnly(ComparisonFunc::Le) => self.depth_binds_readonly += 2,
            DepthSummary::ReadOnly(..) => self.depth_binds_readonly += 1,
            DepthSummary::ReadWrite(ComparisonFunc::Le) => self.depth_binds_write += 2,
            DepthSummary::ReadWrite(..) => self.depth_binds_write += 1,
            DepthSummary::ViewNull => self.depth_binds_null += 1,
...
impl GogglesClass {
    ...
    pub(super) fn enable(&mut self) {
        let counters = Self::STATS_COUNTERS.iter().zip(Self::stats_counters_ref());
    ...
    pub(super) fn disable(&mut self) {
        ...
        ClassShared::expect_present_count_ref().store(
            ClassShared::PRESENT_COUNT_DISABLED,
    ...
    pub(super) fn setup_engine(&mut self, engine: &Engine, _enables: GogglesEnables) {
        ...
        let buf_rt = BufferInfo {
            ...
            size: (
                engine.render_backend.display_size.width as _,
                engine.render_backend.display_size.height as _,
            ),
        ...
        ClassShared::with_mut(move |c| {
            match c.seen.entry(*rtv.as_d3d_raw()) {
                ...
                btree_map::Entry::Occupied(e) => copy_buf(e.into_mut(), &buf_rt),
            }
            if let (Some(dv), Some(buf_dv)) = (dv, buf_dv) {
                match c.seen.entry(*dv.as_d3d_raw()) {
                    ...
                    btree_map::Entry::Occupied(e) => copy_buf(e.into_mut(), &buf_dv),
                }
            }
        });
    }
...
impl super::GogglesState {
    ...
    pub fn class_offer_clear_inconsistent(&self) -> bool {
        ...
        if self.class.compat_clear_inconsistent() {
            return true
        }
