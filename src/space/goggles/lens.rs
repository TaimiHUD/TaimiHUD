...
pub(super) unsafe fn set_targets(_this: &Dx11Context, depth_view: Option<&DepthView>) {
            ...
            match known {
                ...
                Ok(None) => {
                    let size_ok = if let Some(dims) = get_view_dims(view) {
                        ...
                        if !format_ok {
                            log::warn!(
                                "DEBUG DELETEME(dv): fmt actually caught something? {}",
                                dims.Format.0
                            );
...
pub(super) unsafe fn set_depth_state(context: &Dx11Context, state: Option<&DepthState>, stencil_ref: u32) {
    ...
    let cls = {
        ...
        let viewport_ok = || {
            ...
            let expected = g2!(*&ferret.display_size);
            if viewport.viewport.TopLeftX.to_bits() != 0.0f32.to_bits()
                || viewport.viewport.TopLeftY.to_bits() != 0.0f32.to_bits()
            {
                return false
            }
            let expected_aspect = expected.x / expected.y;
        ...
        match desc_state.DepthEnable.0 != 0 {
            ...
            #[cfg(todo)]
            false if desc_state.DepthWriteMask != D3D11_DEPTH_WRITE_MASK_ZERO => Some(LensClass::UI),
            ...
            true if desc_state.DepthFunc == D3D11_COMPARISON_LESS => Some(match stencil_ref {
                ...
                0 if viewport_ok() => LensClass::World,
                #[cfg(todo = "unnecessary")]
                0 => LensClass::Unsupported,
                _ => LensClass::Dummy,
            }),
            true if desc_state.DepthFunc == D3D11_COMPARISON_LESS_EQUAL => Some(LensClass::Test),
...
pub(super) fn release_depth_view_pre(
    this: InterfaceRef<'static, IUnknown>,
    release: &GenericDetour<Release>,
) -> Option<usize> {
    let key = match this.cast::<ID3D11DepthStencilView>().ok() {
        ...
        Some(view) => {
            ...
            let _refcount =
                unsafe { release.call(InterfaceRef::from_raw(NonNull::new_unchecked(view_ref))) };
