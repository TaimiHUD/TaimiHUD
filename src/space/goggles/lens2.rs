...
impl LensShared {
    ...
    pub fn selected_ref() -> &'static AtomicPtr<c_void> {
        unsafe {
            &*g2!(&raw const ferret.lens.selected)
        }
    }
    #[inline]
    ...
    pub fn compatible_ref() -> &'static AtomicBool {
        unsafe {
            &*g2!(&raw const ferret.lens.compatible)
        }
    }
    #[inline]
    ...
    pub(super) fn buf_is_valid_ongoing(buf: &BufferInfo) -> bool {
        if !buf.winner || buf.classification != BufferClass::World { return false }
    ...
    fn buf_is_valid_inner(buf: &BufferInfo, is_bound: bool, strict: bool) -> bool {
        ...
        if write_min { return false }
        #[cfg(todo = "unnecessary")]
        if !ClassShared::buf_is_alive(buf) { return false }
...
impl GogglesLens {
    ...
    pub fn with_selected_lens<R, F: FnOnce(Option<&DepthView>) -> R>(&self, f: F) -> R {
        ...
        let dv = lens.as_ref().map(|p| unsafe {
            DepthView::from_d3d_raw_ref(p)
        });
        f(dv)
    }
