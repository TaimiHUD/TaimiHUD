...
impl DepthHandler {
    pub fn create(
        ...
        #[cfg(feature = "goggles")]
        let _ = DEPTH_STENCIL_VIEW_VTABLE.set(unsafe { &*(depth_stencil_view.as_d3d().vtable() as *const _) });
        ...
        Ok(Self {
            ...
            #[cfg(feature = "goggles2")]
            depth_stencil_state_readonly: OMDepthState::with_state(depth_stencil_state_readonly, Self::STENCIL_REF),
    ...
    pub fn depth_stencil_view(
        ...
        let (dsview, clear_depth) = match self.render_target_view.to_ref() {
            ...
            _ if self.inherit_render != 0 => unsafe {
                let view = InterfaceRef::from_raw(
                    core::ptr::NonNull::new_unchecked(self.inherit_render as *mut _)
                );
                let depth = core::ptr::NonNull::new(self.inherit_depth as *mut _).map(|p|
