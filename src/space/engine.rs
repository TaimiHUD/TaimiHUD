...
use {
    ...
    std::{collections::HashMap, fmt, mem, num::NonZeroU32, sync::Arc, time::Instant},
...
impl Engine {
    ...
    pub fn prepare(&mut self, machine: &mut RenderMachine) -> anyhow::Result<()> {
        ...
        if let Some(rx) = &mut self.interact_rx {
            for _ in 0..48 {
                let when = || self.drawing_start.as_ref().map(|s| s.elapsed().as_secs_f32());
                match rx.try_recv() {
    ...
    pub fn prepare_frame(
        ...
        self.packs
            .prepare_frame(machine, device_context, anim_timestamp, fresh)?;
        Ok(())
    }
    ...
    fn draw_carefully(
        ...
        let aspect =
            self.render_backend.viewport.viewport.Width / self.render_backend.viewport.viewport.Height;
        ...
        self.render_backend.depth_handler.inherit_depth = match machine.goggles.inherit_render {
            ...
            true => depth_view.as_ref().map(|v| v.as_d3d_raw().as_ptr() as usize),
            false => None,
        }
        .unwrap_or(0);
    ...
    pub fn draw(&mut self, machine: &mut RenderMachine, device_context: &Dx11Context, inherit: bool) {
        ...
        let (
            ...
            edge_feather_scale,
            arcrender,
            (_obscured_alpha, _obscured_dist),
        ) = self.map_settings(|s| {
            (
        ...
        let render_world = match visible_space {
            ...
            Some(distance_max) => {
                ...
                let fovy = machine.fov_y().unwrap_or(RenderMachine::DEFAULT_FOV_Y);
                let aspect = machine
                    .aspect_ratio()
                    .unwrap_or(RenderMachine::DEFAULT_ASPECT_RATIO);
                let cull = MapFrustum::from_camera_data(
                    camera,
                    (aspect, glamour::Angle::to_radians(fovy)),
        ...
        if let Some((camera, ref _depth, ref cull)) = render_world {
            ...
            let arcrender_settings = arcrender.then(|| {
                let mut settings = self.map_settings(|s| ArcrenderSettings {
                    ...
                    #[cfg(feature = "goggles2")]
                    poi_flags: if FerretResource::project_hack_shadowbox() {
                        pack::instance::MarkerInstanceData::FLAG_RESERVED_14
                    } else {
                        0
                    },
            ...
            if goggles_2pass {
                ...
                let cull = match _obscured_dist {
                    dist if dist <= _depth.end => cull,
                    dist => {
                        obscured_cull = MapFrustum::from_camera_data(camera, _depth.start..dist);
                        &obscured_cull
                    },
                };
