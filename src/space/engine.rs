...
use {
    ...
    std::{collections::HashMap, fmt, mem, num::NonZeroU32, ops, sync::Arc, time::Instant},
    taimi_d3d::{
...
#[cfg(feature = "goggles")]
use {
    crate::{settings::goggles::GogglesEnables, space::goggles},
    taimi_d3d::dx11,
};
...
impl Engine {
    ...
    {
        let (enabled, _goggles) = Settings::try_read()
            .map(|s| {
                (s.enable_katrender, match s.pathing.as_ref() {
                    #[cfg(feature = "goggles")]
            ...
            Ok(event) => {
                ...
                match event {
                    ...
                    ProcessShader(id, kind, bytecode, partial_id) => {
                        let res = self.render_backend.shaders.load_partial(
    ...
    fn process_gameplay_event(
        ...
        let device_context = || match device_context {
            Some(ctx) => Ok(ctx),
            ctx @ None => unsafe { self.render_backend.device.GetImmediateContext() }
    ...
    fn prepare(
        ...
        if let Some(rx) = &mut self.interact_rx {
            for _ in 0..48 {
                ...
                match rx.try_recv() {
                    Ok(InteractionEvent::Interact {
                        action: InteractionEventAction::Report(action),
                        loaded_path,
                        path: _,
                    }) =>
        ...
        #[cfg(feature = "goggles")]
        {
            let ctx = match device_context {
                None if machine.goggles.wants_d3d_context(&self.drawing) =>
                    unsafe { self.render_backend.device.GetImmediateContext() }
                        .ok()
                        .map(|c| &*device_context.insert(c)),
                ctx => ctx.as_ref(),
            };
            let view = &self.render_backend.depth_handler.render_target_view.views;
            machine
                .goggles
                .act_pre_render_frame(ctx, Some(view), &mut self.drawing);
        }
        Ok(())
    }
    pub fn setup_frame(&mut self, _machine: &mut RenderMachine, device_context: &Dx11Context) {
    ...
    fn pre_render(
        ...
        let is_proj = machine.goggles.is_enabled(GogglesEnables::PROJECT_ENABLE);
        let is_stale = match () {
            #[cfg(feature = "goggles2-project")]
            _ if is_proj => self.drawing.is_stale_frame(),
            _ => true,
        };
        if is_stale {
    ...
    pub fn render(&mut self, machine: &mut RenderMachine) -> anyhow::Result<()> {
        ...
        frame_log!(
            "engine; frame/render vis={:?} enabled={:?} drawing={:?} drawn={:?}",
    ...
    #[cfg(feature = "goggles2-project")]
    #[inline]
    pub(crate) fn project_proceed(&mut self, _machine: &mut RenderMachine, ctx: LocalContext) -> bool {
        if !self.drawing.prepared {
            return false
        }
    ...
    fn draw_carefully(
        ...
        let vp_rect = match desc.goggles.target_viewport {
            ...
            None => {
                ...
                let vp_size = vp.map(|vp| (vp, vp.size2())).and_then(|(vp, sz)| match sz {
                ...
                match desc.goggles.inherit {
                    #[cfg(todo = "unnecessary")]
                    false => {
                        let aspect = self.render_backend.viewport.viewport.Width
                            / self.render_backend.viewport.viewport.Height;
                        let vp_size_valid = vp_size.and_then(|(vp, size)| {
                            if vp.viewport.TopLeftX != 0.0 || vp.viewport.TopLeftY != 0.0 {
                                return None
                            }
    ...
    pub fn draw2(
        ...
        match ctx {
            LocalContext::World => {
                ...
                desc.draw_trails = draw_trails.then_some(DrawDescEntity { textured: textured_trails });
                desc.draw_pois = draw_pois.then_some(DrawDescEntity { textured: textured_pois });
    ...
    fn map_bounds(
        ...
        if !self.drawing.drawing.has(map_ctx) {
            return None
        }
        Some(self.get_map_bounds(machine, map_ctx))
    }
    fn get_map_bounds(&self, machine: &mut RenderMachine, map_ctx: MapContext) -> Box3<DrawSpace> {
    ...
    fn space_bounds(
        ...
        if !self.drawing.drawing.has(LocalContext::World) {
            return None
        }
    ...
    pub fn clear_scissor(&mut self, device_context: &Dx11Context, _desc: &DrawDescSpace) {
        #[cfg(feature = "goggles2-project")]
        #[cfg(todo)]
        if _desc.goggles.target_renderview.is_some() {
            return
    ...
    pub fn draw_map(
        ...
        let (fwoom, trail_textured, trail_scale, trail_alpha, poi_scale, poi_alpha) =
            self.map_settings(|s| {
                (
                    s.space.map_open(),
        ...
        if trail_alpha > 0.0 || poi_alpha > 0.0 {
            let backend = &mut self.render_backend;
            backend.perspective_handler.update_map(
                machine,
                &backend.viewport,
                render_bounds,
                local_bounds,
    ...
    pub fn draw_space(
        ...
        let arcrender = self
            ...
            .unwrap_or(false);
        #[cfg(feature = "goggles")]
        let goggles_2pass = desc
            .pass_is_obscured()
            .then_some(self.settings.as_ref())
            .flatten()
            .and_then(|s| {
        ...
        self.arcdata.apply_p(
            &mut self.packs.shared_p,
            self.render_backend.viewport.size2().cast(),
        );
        #[cfg(todo = "unnecessary")]
        #[cfg(feature = "goggles2-project")]
        match desc.pass {
            ...
            | DrawDescSpace::PASS_REFLECTING | DrawDescSpace::PASS_REFLECTING_BELOW => {
                self.packs.shared_p.render.edge_feather =
                    Vector2::<f32>::splat(ArcrenderSettings::FEATHER_SCALE_NONE).to_array();
                self.packs.shared_p.render.edge_feather_viewport =
                    Vector2::splat(ArcrenderSettings::VIEWPORT_NONE);
            },
            _ => (),
        }
        let legacy_alpha = match () {
            ...
            _ if desc.pass_is_obscured() => {
                if desc.depth_read {
                    state.set_depth_state(
                        device_context,
                        &self.render_backend,
                        DrawStateSpace::DEPTH_OBSCURED,
                    );
                } else {
        ...
        let (cam, cull) = match desc.pass.get_pass() {
            ...
            Drawing::REFLECT_BELOW => {
                ...
                self.packs.shared_v.trail.marker.flags |=
        ...
        let arcrender = match arcrender {
            Some(variant)
                if self.packs.resources.prepare_shaders_arc(
                    &self.render_backend.shaders,
                    &mut self.packs.draw_state,
                    variant,
                ) =>
                true,
        ...
        if arcrender {
            ...
            self.render_backend.perspective_handler.update_perspective(
                machine,
                *cam,
                Vec3::splat(self.arcdata.poi_expansion.scale()),
            );
            self.render_backend
                .perspective_handler
    ...
    pub fn draw0(&mut self, machine: &mut RenderMachine, device_context: &Dx11Context) {
        ...
        let minimap_bounds = self.minimap_bounds_screen(machine);

        let stencil_prev = mem::take(&mut self.drawing.stencil);
        #[cfg(feature = "goggles")]
        {
            self.drawing.stencil.set_edge_scale(
                self.settings
                    .as_ref()
        ...
        if clear_stencil {
            #[cfg(feature = "goggles")]
            let edge_scale = (stencil_prev.edge_scale != self.drawing.stencil.edge_scale).then_some(
                self.drawing
                    .stencil
                    .edge_scale()
                    .map(|s| (s, &machine.map.calibration)),
            );
            #[cfg(feature = "goggles")]
            ...
            self.drawing
                .stencil
                .apply_masks(device_context, &self.render_backend, &desc, &mut state);
            desc.stencil_write = false;
        ...
        if render_world.is_some() && machine.goggles.is_enabled(GogglesEnables::LENS_ENABLE) {
            machine.goggles.lens.with_selected_lens(|lens| {
    ...
    fn draw0_map(
        ...
        let (scissor, render_bounds) = match map_ctx {
            ...
            #[cfg(feature = "goggles2-project")]
            MapContext::Minimap
                if machine.goggles.project.is_projecting()
                    && self.render_backend.viewport.viewport.Width
                        == self.render_backend.viewport.viewport.Height =>
                (None, self.render_backend.viewport_rect()),
            MapContext::Minimap => match (
                self.drawing.stencil.minimap,
                self.drawing.stencil.minimap_bounds(),
            ) {
                (Some(screen_bounds), Some(render_bounds)) => (Some(render_bounds), screen_bounds),
                _ => {
                    #[cfg(taimi_debug)]
    ...
    pub fn gameplay_map_enter(
        ...
        #[cfg(feature = "goggles")]
        {
            let entry = self
                .settings
                .as_ref()
                .and_then(|s| s.space.goggles.get_map_depth_calibration(_map_id.get()));
    ...
    fn fill_settings(&mut self) -> Result<&mut PathingSettings, Option<anyhow::Error>> {
        match &mut self.settings {
            ...
            settings @ None => match Settings::read_with_blocking(|s| s.pathing.clone()) {
                Ok(Some(s)) => Ok(settings.insert(s)),
                Ok(None) => Err(None),
                Err(e) => Err(Some(e)),
            },
        }
    ...
    pub fn map_settings<R, F: FnOnce(&PathingSettings) -> R>(&mut self, f: F) -> R {
        let settings = match self.fill_settings() {
            Ok(s) => s,
            Err(None) => &*self.settings.get_or_insert_default(),
            Err(Some(e)) => {
...
impl DrawDescSpace {
    ...
    pub fn pass_is_obscured(&self) -> bool {
        self.pass
            .get_pass()
            .intersects(Drawing::OBSCURED | Drawing::OBSCURED_SHADOWED)
    }
...
impl DrawDescGoggles {
    ...
    pub fn render_view(&self) -> Option<&dx11::RenderTargetView> {
        self.target_renderview
            .as_ref()
            .map(|raw| unsafe { dx11::RenderTargetView::from_d3d_raw_ref(raw) })
    }
    #[inline(always)]
    ...
    pub fn depth_view(&self) -> Option<&dx11::DepthView> {
        self.target_depthview
            .as_ref()
            .map(|raw| unsafe { dx11::DepthView::from_d3d_raw_ref(raw) })
    }
...
impl FrameContext {
    ...
    pub fn new_scene(&mut self, when: Option<Instant>) -> Option<Instant> {
        let when = when.or(self.frame_start).unwrap_or_else(|| Instant::now());
...
impl FrameContext {
    ...
    pub fn draw_contexts() -> impl Iterator<Item = LocalContext> {
        IntoIterator::into_iter([LocalContext::MINIMAP, LocalContext::World, LocalContext::GLOBAL])
    }
    #[inline]
    pub fn draw_map_contexts() -> impl Iterator<Item = MapContext> {
        IntoIterator::into_iter([MapContext::Minimap, MapContext::Global])
    }
}
...
impl DrawStateSpace {
    ...
    pub fn set_minimap_scissor(
        ...
        match scissor {
            ...
            Some(bounds) => backend.depth_handler.setup_minimap_scissor(context, bounds),
            None => backend
                .depth_handler
    ...
    pub fn unset_scissor(&mut self, context: &Dx11Context) {
        if self.minimap_scissor.is_none() {
            return
        }
    ...
    pub fn unset_depth_state(&mut self, context: &Dx11Context, _backend: &RenderBackend) {
        if self.bound_depth == Self::ID_EMPTY {
            return
        }
    ...
    pub fn set_viewport(&mut self, context: &Dx11Context, backend: &RenderBackend) {
        backend.viewport.set(context);
    }
    /// TODO: unset would be polite
    pub fn set_raster(&mut self, context: &Dx11Context, backend: &RenderBackend) {
        backend.depth_handler.rasterizer_state.set(context);
    }
    /// TODO: unset would be polite
    pub fn set_sampler(&mut self, context: &Dx11Context, backend: &RenderBackend) {
        backend.sampler_state.set(context, Self::TEXTURE_TRAIL_SLOT);
    }
    const TEXTURE_TRAIL_SLOT: u32 = 0;

    pub fn setup(&mut self, context: &Dx11Context, backend: &RenderBackend) {
        if self.setup {
            return
        }
    ...
    pub fn unsetup(&mut self, context: &Dx11Context, _backend: &RenderBackend) {
        if !self.setup {
            return
        }
    ...
    pub fn unset_blend_state(&mut self, context: &Dx11Context, _backend: &RenderBackend) {
        if self.bound_blend == Self::ID_EMPTY {
            return
        }
...
impl FrameStencil {
    ...
    pub fn is_empty(&self) -> bool {
        ...
        self.minimap.is_none() && self.edge_scale.to_bits() == Self::EMPTY_EDGE_SCALE.to_bits()
    }
    ...
    fn eq_fields(&self, rhs: &Self) -> bool {
        if self.edge_scale.to_bits() != rhs.edge_scale.to_bits() {
            return false
        }
    ...
    pub fn apply_masks(
        ...
        let depth_mask_fallback = match desc.stencil_write {
            false if !desc.depth_write => return,
            #[cfg(feature = "goggles")]
