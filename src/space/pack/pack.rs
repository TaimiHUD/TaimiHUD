    ...
    pub fn prepare(&mut self, device: &Dx11Device, machine: &RenderMachine) -> anyhow::Result<bool> {
        ...
        if let Some(map_id) = map_id {
            if let Some(packs_map) = &packs_map_changed {
                ...
                rt::log::error_ok(self.resources.prepare(device, self.pack_data.map_ref_as_slice(), self.spacepacks.render_entities.entities.iter().map(|e| e.id)).context("RenderResources::prepare"));
                if !ibs_dirty {
                    if self
                        .pack_data
                        .values()
