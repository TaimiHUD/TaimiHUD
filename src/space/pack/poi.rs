...
impl PoiCommonRenderData {
    ...
    pub fn update_fallback(&mut self, device: &Dx11Device, _machine: &RenderMachine) {
        ...
        let fallback_obj = self.fallback_object.is_none().then(||
            RenderMachine::logo_object()
        );
        if let Some(Some((o, m))) = fallback_obj {
            ...
            let vertices = o.map(|o| o.load(false).0.into_iter()
                .map(|v| PoiVertex::new(
            ...
            self.fallback_object = vertices.and_then(|v| match PoiVertex::alloc(device, &v[..]) {
                #[cfg(taimi_debug)]
                vb => crate::exports::runtime::log::debug_ok(anyhow::Context::context(vb, "taimihud.obj")),
                #[cfg(not(taimi_debug))]
                vb => vb.ok(),
...
impl PoiRender {
    ...
    pub fn populate_rotation(&mut self, poi: &LoadedPoiRef) {
        ...
        self.icon_unset = !poi.lpoi_info().marker_info().has_attr_of::<keys::IconFile>() && !poi.lpoi_info().marker_info().has_attr_of::<keys::Occlude>();
        if self.icon_unset {
