...
impl DrawSpaceEntity for DrawSpaceArc<'_> {
    ...
    fn draw_poi(&mut self, _pack_data: &PackRenderData, space_idx: usize, poi: &PoiRender, _lpath: LoadedPoiPath) -> bool {
        if space_idx >= self.resources.len { return false }
        if self.bind_poi().is_none() { return false }
        //#[cfg(taimi_debug)]
        if poi.icon_unset {
            let binds = self.resources.poi_common.as_ref()
                .and_then(|c| c.fallback_object.as_ref().and_then(|o| c.fallback_textureo.as_ref()
                        .map(|t| (o, t))
                    ));
            if let Some(((vb, vc), tex)) = binds {
                ...
                unsafe {
                    ...
                    self.resources.poi_common.as_ref().unwrap_unchecked().set_primitive(self.context);
                }
            }
            self.last_quad = None;
            return true
...
impl ArcShaderVariant {
    ...
    pub fn all_ids() -> BTreeSet<&'static str> {
        let states = [
            Some(ShaderState::Trail),
            Some(ShaderState::Poi),
            None,
        ];
        let kinds = [ShaderKind::Vertex, ShaderKind::Pixel];
        let state_kinds = IntoIterator::into_iter(kinds).flat_map(|k|
            IntoIterator::into_iter(states).map(move |e| (k, e))
        );
        state_kinds.flat_map(|(k, e)|
            IntoIterator::into_iter(Self::VARIANTS).flat_map(move |v| v.id(k, e))
        ).collect()
    }
    #[cfg(todo)]
    pub fn all_template_ids() -> impl IntoIterator<Item = &'static str> {
        ...
        let kinds = [ShaderKind::Vertex, ShaderKind::Pixel];
        let state_kinds = IntoIterator::into_iter(kinds).flat_map(|k|
            IntoIterator::into_iter(states).map(|e| (k, e))
        );
        state_kinds.flat_map(|(k, e)|
            IntoIterator::into_iter(Self::VARIANTS).flat_map(|v| [
                v.template_id(k, e),
                v.id(k, e),
            ]).flatten()
        ).collect()
