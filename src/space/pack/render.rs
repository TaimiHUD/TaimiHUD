...
impl RenderOrderSort {
    ...
    pub fn cam_dist_order_biased(&self, position: Point3<DrawSpace>, bias: u32) -> i32 {
        self.cam_dist_order_for(position)
            .saturating_add((-(bias as i32)) % 0x800)
    }
    #[inline(always)]
    ...
    BvhIter: Iterator<Item = (Option<i32>, T)>,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        self.draw_order_heap
            .pop()
            .map(|he| (Some(he.cam_dist), he.value.0))
    }
}
...
impl Drawing {
    ...
    pub const PASSES_PRIMARY: Self = Self::from_bits_retain(Self::SPACE.bits() | Self::PASSES_MAP.bits());
    pub const PASSES_MAP: Self = Self::from_bits_retain(Self::MINIMAP.bits() | Self::GLOBALMAP.bits());
    ...
    pub const PASSES_INCOMPAT_LEGACY: Self = Self::from_bits_retain({
        let incompat_proj = match () {
            #[cfg(feature = "goggles2-project")]
            _ => Self::REFLECT_BELOW.bits() | Self::OBSCURED_SHADOWED.bits() | Self::SHADOWBOX.bits(),
            #[cfg(not(feature = "goggles2-project"))]
            _ => 0,
        };
