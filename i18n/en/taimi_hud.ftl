...
stats-render-time-project = Project { stats-render-time }
stats-render-time-latency = Overlay Latency
stats-render-time-interval = Frame Duration
stats-render-time-game = Game { stats-render-time }
...
marker-filetype-explanation = There are three kinds of markers file, there is the kind that comes with the BlishHUD Commander's Markers module (integrated), there is the kind that they use to ship Community Markers and then there is my own format, which takes the per marker set format and makes it a single file per marker set.
...
pathing-config-enable = {space} Pathing
