use {
    ...
    core::ops::Range,
    glam::{Vec3A, Vec4},
    glamour::{vec4, Box3, Intersection, Point3, Vector3, Vector4},
};
...
impl MapFrustum {
    pub fn from_camera_data(
        (pos, camera_dir, _camera_up): (Point3<DrawSpace>, Vector3<DrawSpace>, Vector3<DrawSpace>),
        (aspect_ratio, fovy): (f32, f32),
        Range { start: near, end: far }: Range<f32>,
    ) -> Self {
        let p = pos.to_vec3a();
        let d = camera_dir.to_vec3a().normalize();
        let right = d.cross(glam::Vec3A::Y).normalize();
        let up = right.cross(d).normalize();

        let tan_fov2 = (fovy / 2.0).tan();
    ...
    pub fn from_camera_data_sloppy(
        ...
        let pos = pos.to_vec3a();
        let camera_dir = camera_dir.to_vec3a();
        let camera_far = camera_dir * far;
        let camera_near = camera_dir * near;

        let camera_dir_right = camera_dir.cross(camera_up.into()).normalize();
