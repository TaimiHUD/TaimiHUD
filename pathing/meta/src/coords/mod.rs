...
impl LocalSpace {
    ...
    pub fn norm_to_game(local: Vector3<LocalSpace>) -> Vector3<GameSpace> {
        Vector3::new(local.x, local.z, -local.y)
    }
...
impl GameSpace {
    ...
    pub fn norm_to_local(game: Vector3<GameSpace>) -> Vector3<LocalSpace> {
        Vector3::new(game.x, -game.z, game.y)
    }
...
pub fn decompose_look_to32_rows(look: glam::Mat4) -> (glam::Mat3A, glam::Vec3A) {
    let eye = glam::Vec3A::new(look.x_axis.w, look.y_axis.w, look.z_axis.w);
...
pub fn decompose_look32_rows(
    look: glam::Mat3A,
    eye: glam::Vec3A,
) -> (glam::Vec3A, glam::Vec3A, glam::Vec3A) {
    let eye = match look {
        #[cfg(todo = "unnecessary")]
        look => look.mul_transpose_vec3(eye),
        look => {
            #[cfg(todo)]
            let eye = glam::Vec3A::new(eye.x, eye.y, -eye.z);
            look * eye
        },
    };
    let dir = match &look {
        #[cfg(todo)]
        look => Vector3::new(look.x_axis.z, look.y_axis.z, look.z_axis.z),
        look => look.z_axis,
    };
    let up = match &look {
        #[cfg(todo = "unnecessary")]
        look => {
            let side = match &look {
                #[cfg(todo)]
                look => Vector3::new(look.x_axis.x, look.y_axis.x, look.z_axis.x),
                look => look.x_axis,
            };
            dir.cross(side)
        },
