...
fn write_map_cache() {
    ...
    process_cache_data(&INC_MAP_CACHE_GZ, map_cache, &INC_MAP_CACHE, Some(MAP_SIGN_FILENAME.as_ref()));
}
...
fn process_cache_data<F>(id: &dyn fmt::Display, mut filename: F, src_id: &dyn fmt::Display, fallback: Option<&Path>) where
...
fn write_cache_data<F>(id: &dyn fmt::Display, mut filename: F, fallback: Option<&Path>) -> Result<Option<PathBuf>, Error> where
    F: FnMut(PathBuf) -> PathBuf,
{
