use {
    crate::{
        ...
        script::{
            format_err,
            lua::{to_lua_error, IColour, LuaProxyOf, ScriptApiTable, LuaStringlike},
...
impl FromLua for keys::Behaviour {
    fn from_lua(value: LuaValue, lua: &Lua) -> LuaResult<Self> {
        u8::from_lua(value, lua).and_then(|v| Self::try_from(v)
...
impl FromLua for keys::MapTypes {
    fn from_lua(value: LuaValue, lua: &Lua) -> LuaResult<Self> {
        Vec::<attributes::MapType>::from_lua(value, lua)
            .map(|v| keys::MapTypes::from(v))
    }
}
...
impl FromLua for attributes::MapType {
    fn from_lua(value: LuaValue, lua: &Lua) -> LuaResult<Self> {
        match value {
            LuaValue::Integer(v) => attributes::MapType::try_from(v as i32)
                .map_err(to_lua_error),
            v => LuaStringlike::from_lua(v, lua).and_then(|s|
                s[..].parse::<attributes::MapType>().map_err(to_lua_error)
            ),
        }
    }
}
...
impl FromLua for keys::Raids {
    fn from_lua(value: LuaValue, lua: &Lua) -> LuaResult<Self> {
        Vec::<keys::Raid>::from_lua(value, lua)
            .map(|v| keys::Raids::from(v))
    }
}
...
impl FromLua for keys::Specializations {
    fn from_lua(value: LuaValue, lua: &Lua) -> LuaResult<Self> {
        Vec::<keys::Specialization>::from_lua(value, lua)
            .map(|v| keys::Specializations::from(v))
    }
}
