use {
    crate::{
        attributes::{
            cell::{AttrKeyValue, GetAttrDyn, PackKeyId, PackValueDyn, PackValueOf, PackValueSet, PackKeySet},
...
impl PackOverrides {
    ...
    pub fn allocate_dynamic_cat(&mut self, id: CategoryId, pack: &'_ Pack, loc: Option<MarkerLoc>) -> Result<MarkerLoc> {
        ...
        let res = match loc {
            ...
            Some(loc) => {
                Self::allocate_dynamic_post_inner(
                    &mut self.dynamic,
                    &mut self.overrides,
                    loc,
                );
