...
pub trait AsFullId {
    type SegmentRef<'s>: AsRef<IdNameSeg> + 's where
        Self: 's;
    type SegmentIter<'s>: IntoIterator<Item = Self::SegmentRef<'s>> where
        Self: 's;
...
impl<'a, T: ?Sized + AsFullId> AsFullId for &'a T {
    type SegmentRef<'s> = <T as AsFullId>::SegmentRef<'s> where T: 's, 'a: 's;
    type SegmentIter<'s> = <T as AsFullId>::SegmentIter<'s> where T: 's, 'a: 's;
    fn segments(&self) -> Self::SegmentIter<'_> {
...
impl<T: Sized + AsRef<IdStr>> CategoryId<T> {
    pub fn new<S: Into<T>>(full_id: S, len: usize) -> Option<Self> {
        ...
        match len <= full_id.as_ref().len() {
            true if len > 0 => Some(unsafe {
                Self::new_unchecked(full_id, len)
            }),
            _ => None,
        }
    }
    ...
    pub fn get_parent(&self) -> Option<CategoryId<T>> where
        T: Clone,
    {
        match self.as_id().parent() {
            ...
            Some(id) => Some({
                ...
                unsafe {
                    Self::new_unchecked(self.full_id.clone(), len)
                }
            }),
        }
    }
...
impl<T: ?Sized + AsFullId + AsRef<IdNameBox> + AsRef<IdStr>> CategoryId<T> {
    pub fn to_id_box(&self) ->  Cow<'_, IdNameSeg> {
        match self.as_full_id() {
            ...
            None =>
                Cow::Borrowed(self.as_id()),
        }
    }
}
impl<T: ?Sized> fmt::Display for CategoryId<T> where
    Self: AsFullId,
{
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
...
impl<T: ?Sized + AsFullId> AsFullId for CategoryId<T> where
    ...
    for<'s> <CategoryIdIterator<<T as AsFullId>::SegmentIter<'s>> as Iterator>::Item: AsRef<IdNameSeg>,
{
    ...
    type SegmentRef<'s> = <Self::SegmentIter<'s> as Iterator>::Item where T: 's;
    type SegmentIter<'s> = CategoryIdIterator<<T as AsFullId>::SegmentIter<'s>> where T: 's;
    fn segments(&self) -> Self::SegmentIter<'_> {
        let len = self.len();
        CategoryIdIterator {
            inner: self.full_id.segments(),
            len,
        }
    }
    ...
    fn id_to_str(&self) -> Cow<'_, str> {
        ...
        match self.full_id.id_to_str() {
            Cow::Borrowed(s) =>
                Cow::Borrowed(s.get(..len).unwrap_or(s)),
            Cow::Owned(mut s) => {
                if s.len() > len {
                    s.truncate(len);
                }
...
impl<T: Iterator> Iterator for CategoryIdIterator<T> where
...
impl<T: iter::FusedIterator> iter::FusedIterator for CategoryIdIterator<T> where
    Self: Iterator,
{
}
...
impl CategoryId {
    ...
    pub fn invalid_singleton() -> &'static Self {
        ...
        static INVALID_CATEGORY_ID: LazyLock<CategoryId> = LazyLock::new(|| CategoryId::with_full_id(SEP_STR));
        &*INVALID_CATEGORY_ID
    }
}
...
impl<T> From<CategoryId<T>> for String where
...
impl<T: ?Sized> From<&'_ CategoryId<T>> for String where
    CategoryId<T>: fmt::Display,
{
    fn from(value: &CategoryId<T>) -> Self {
        value.to_string()
...
impl<'a, T: ?Sized> From<&'a CategoryId<T>> for &'a str where
...
impl<'i> IdRef<'i> {
    ...
    pub fn into_owned(self) -> FullIdOf<'static> {
        match self {
            Self::FullIdRef(full_id) =>
                IdRef::with_full_id(full_id.into()),
            Self::FullId(full_id) =>
                IdRef::with_full_id(full_id),
        }
    }
}
...
impl<T: ?Sized + AsFullId> FullIdOf<T> {
    ...
    pub fn cmp_with<R: ?Sized + AsFullId>(&self, rhs: &R) -> cmp::Ordering {
        ...
        loop {
            let seg = segs.next();
            let rhs = rhs.next();
            let ord = FullIdOf::name_iter_cmp(seg.as_ref().map(AsRef::as_ref), rhs.as_ref().map(AsRef::as_ref));
            if let Some(ord) = ord {
                break ord
            }
        }
    ...
    pub fn eq_with<R: ?Sized + AsFullId>(&self, rhs: &R) -> bool {
        ...
        loop {
            let seg = segs.next();
            let rhs = rhs.next();
            let eq = FullIdOf::name_iter_eq(seg.as_ref().map(AsRef::as_ref), rhs.as_ref().map(AsRef::as_ref));
            if let Some(eq) = eq {
                break eq
            }
        }
...
impl FullIdOf<FullIdRef> {
    ...
    fn name_iter_cmp(seg: Option<&IdNameSeg>, rhs: Option<&IdNameSeg>) -> Option<cmp::Ordering> {
        match (seg, rhs) {
            ...
            (seg, rhs) =>
                Some(seg.is_some().cmp(&rhs.is_some())),
        }
    }
}
impl<T: ?Sized + AsFullId> AsFullId for FullIdOf<T> {
    type SegmentRef<'s> = <T as AsFullId>::SegmentRef<'s> where T: 's;
    type SegmentIter<'s> = <T as AsFullId>::SegmentIter<'s> where T: 's;
    fn segments(&self) -> Self::SegmentIter<'_> {
        self.id.segments()
    }
    fn id_len(&self) -> usize {
