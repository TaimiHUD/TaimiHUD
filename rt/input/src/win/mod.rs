...
pub unsafe fn window_message(hwnd: HWND, msg: u32, w: usize, l: isize) -> anyhow::Result<()> {
    WindowsAndMessaging::PostMessageA(Some(hwnd), msg, WPARAM(w), LPARAM(l))
        .context("PostMessageA")
}

pub fn window_send_inputs<I: Into<KeyboardAndMouse::INPUT>>(hwnd: HWND, inputs: impl IntoIterator<Item = I>) -> anyhow::Result<()> {
