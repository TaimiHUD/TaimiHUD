...
impl<T> Watcher<T> {
    ...
    pub fn init(&self, value: T) -> &watch::Receiver<T> {
        ...
        unsafe { self.init_receiver(watch) }
    }
...
impl<T: Clone> Watched<T> {
    ...
    pub async fn watch<F: FnMut(&T, &mut Option<T>) -> bool>(&mut self, mut cond: F) -> &mut T {
        let value = {
            ...
            self.watch.watch(move |v| cond(v, cached)).await
        };
        self.cached.insert(value.clone())
    }

    pub fn write_with<F: FnOnce(&mut T)>(&mut self, f: F) {
...
pub mod serde_imp {
    ...
    impl<T> Sender<T> {
        ...
        where
            T: Serialize,
        {
            watch.borrow().serialize(serializer)
        }
        pub fn deserialize<'de, D: de::Deserializer<'de>>(
            deserializer: D,
        ) -> Result<watch::Sender<T>, D::Error>
        where
            T: Deserialize<'de>,
        {
            T::deserialize(deserializer).map(watch::Sender::new)
        }
    }
    ...
    impl<T> Receiver<T> {
        ...
        /// while this can maybe be recovered into a sender again, it's usually not what you want...
        #[cfg(todo)]
        pub fn deserialize<'de, D: de::Deserializer<'de>>(
            deserializer: D,
        ) -> Result<watch::Receiver<T>, D::Error>
        where
            T: Deserialize,
        {
            T::deserialize(deserializer).map(|v| watch::Sender::new(v).subscribe())
        }
    }
