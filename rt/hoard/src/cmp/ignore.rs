...
impl<K, V: ?Sized> SetPair<K, V> {
    #[inline]
    pub const fn new(key: K, value: V) -> Self where
        V: Sized,
    {
        Self {
            key,
            value: CmpIgnore(value),
        }
    }

    #[inline(always)]
    pub const fn key(&self) -> &K { &self.key }
    /// mutations that affect equality or ordering is usually a very bad idea
    #[inline(always)]
    pub fn key_mut(&mut self) -> &mut K { &mut self.key }
    #[inline(always)]
    pub const fn value(&self) -> &V { &self.value.0 }
    #[inline(always)]
    pub fn value_mut(&mut self) -> &mut V { &mut self.value.0 }
    ...
    #[inline(always)]
    pub fn into_key(self) -> K where
        V: Sized,
    {
        self.key
    }
    #[inline(always)]
    pub fn into_value(self) -> V where
        V: Sized,
    {
        self.value.0
    }
    #[inline(always)]
    pub fn into_tuple(self) -> (K, V) where
        V: Sized,
    {
...
impl<K, V: ?Sized> ops::DerefMut for SetPair<K, V> {
    #[inline(always)]
    fn deref_mut(&mut self) -> &mut Self::Target { self.value_mut() }
}
...
impl<K, V: ?Sized> PartialEq<K> for SetPair<K, V> where
    K: PartialEq,
{
    #[inline]
...
impl<K, V: ?Sized> PartialOrd<K> for SetPair<K, V> where
    K: PartialOrd,
{
    #[inline]
