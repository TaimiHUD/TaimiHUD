use {crate::collections::traits::TaimiCollection, core::ops, num_traits::AsPrimitive, std::sync::Arc};
...
pub trait TaimiSeqIndex<V: ?Sized, K: TaimiSeqKey = usize>: TaimiSeq<V, K> {
    fn seq_index<'a>(&'a self, index: K) -> Option<&'a V> {
        match self.seq_range() {
            Some(range) if range.contains(&index) => Some(unsafe { self.seq_index_unchecked(index) }),
            _ => None,
        }
    }
...
pub trait TaimiSeqIndexMut<V, K: TaimiSeqKey = usize>: TaimiSeqMut<V, K> {
    fn seq_index_mut<'a>(&'a mut self, index: K) -> Option<&'a mut V> {
        match self.seq_range() {
            Some(range) if range.contains(&index) => Some(unsafe { self.seq_index_mut_unchecked(index) }),
            _ => None,
        }
    }
    unsafe fn seq_index_mut_unchecked<'a>(&'a mut self, index: K) -> &'a mut V;
}
pub trait TaimiSeqStorage:
    TaimiSeq<<Self as TaimiSeqStorage>::Value, <Self as TaimiSeqStorage>::Index>
{
...
impl<T: ?Sized + TaimiSeqStorage> TaimiSeqStorage for Arc<T>
where
...
impl<V: ?Sized, K: TaimiSeqKey, T: ?Sized + TaimiSeq<V, K>> TaimiSeq<V, K> for Arc<T>
where
    Self: TaimiCollection,
{
    #[inline(always)]
...
impl<V: ?Sized, K: TaimiSeqKey, T: ?Sized + TaimiSeqIndex<V, K>> TaimiSeqIndex<V, K> for Arc<T>
where
    Self: TaimiCollection,
{
    #[inline(always)]
...
impl<'a, V, K: TaimiSeqKey, T: ?Sized> TaimiSeqGet<V, K> for &'a Arc<T>
where
    &'a T: TaimiSeqGet<V, K>,
    Self: TaimiSeq<V, K>,
{
