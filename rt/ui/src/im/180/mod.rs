//! [imgui180_sys](self::sys) and [imgui180](self::imgui)
...
pub use self::text::{FontId, StyleColor};
...
impl<'ui> Ui<'ui> {
    ...
    pub unsafe fn borrow_ctx_mut<'a>(context: &'a mut ImGuiContext) -> &'a mut &'a Self {
        let ui = Self::from_ctx_mut(context);
        mem::transmute(ui as *mut _ as *mut ())
    }
    pub unsafe fn from_ctx_mut(context: &mut ImGuiContext) -> &mut Self {
...
impl imw::Table {
    pub const IM180_FLAGS_PRESET: sys::ImGuiTableFlags_ =
        sys::ImGuiTableFlags_Resizable | sys::ImGuiTableFlags_Borders | sys::ImGuiTableFlags_RowBg;
    pub const IM180_ARGS_PRESET: imw::DynArgsTable = imw::DynArgsTable::new(Some(Self::IM180_FLAGS_PRESET));
}
impl imw::TableColumn {
    pub const IM180_WIDTH_STRETCH: sys::ImGuiTableColumnFlags_ = sys::ImGuiTableColumnFlags_WidthStretch;
    pub const IM180_WIDTH_FIXED: sys::ImGuiTableColumnFlags_ = sys::ImGuiTableColumnFlags_WidthFixed;
}
impl imw::TreeNode {
    pub const IM180_ARGS_FRAMED: imw::DynArgsWidget =
        imw::DynArgsWidget::new(Some(sys::ImGuiTreeNodeFlags_Framed));
    pub const IM180_ARGS_FRAMED_NOPUSH: imw::DynArgsWidget = imw::DynArgsWidget::new(Some(
        sys::ImGuiTreeNodeFlags_Framed | sys::ImGuiTreeNodeFlags_NoTreePushOnOpen,
    ));
}
impl imw::ProgressBar {
    pub const IM180_DEFAULT_SIZE: ImSize2 = ImSize2::new(f32::MIN, 0.0);
}
impl imw::InputPassword {
    pub const IM180_FLAGS_PRESET: sys::ImGuiInputTextFlags_ =
        imw::InputText::IM180_FLAGS_PRESET | sys::ImGuiInputTextFlags_Password;
}
impl imw::InputText {
    const IM180_MASK_ENTER: sys::ImGuiInputTextFlags =
    ...
    pub const IM180_ARGS_READ_ONLY: imw::DynArgsInputText =
        imw::DynArgsWidget::new(Some(Self::IM180_FLAGS_PRESET | Self::IM180_FLAGS_READ_ONLY));
}
impl imw::InputTextMultiline {
    pub const IM180_FLAGS_PRESET: sys::ImGuiInputTextFlags_ = 0;
    pub const IM180_DEFAULT_SIZE: ImSize2 = ImSize2::ZERO;
}
impl imw::Selectable {
    pub const IM180_ARGS_NO_DISMISS_POPUP: imw::DynArgsWidgetSized =
        imw::DynArgsWidgetSized::new(Some(sys::ImGuiSelectableFlags_DontClosePopups), None);
}
...
impl ImIo for Io {
    ...
    fn display_size(&self) -> ImSize2<f32> {
        match self {
            #[cfg(feature = "imgui180-rs")]
            io => io.display_size.into(),
            #[cfg(not(feature = "imgui180-rs"))]
            io => ImSpaces(io.DisplaySize).into(),
        }
    }
    #[inline]
    fn display_framebuffer_scale(&self) -> ImVec2<f32> {
        match self {
            #[cfg(feature = "imgui180-rs")]
            io => io.display_framebuffer_scale.into(),
            #[cfg(not(feature = "imgui180-rs"))]
            io => ImSpaces(io.DisplayFramebufferScale).into(),
        }
    }
    #[inline]
    ...
    fn key_is_down_untyped(&self, idx: usize) -> bool {
        match self {
            #[cfg(feature = "imgui180-rs")]
            io => io.keys_down.get(idx),
            #[cfg(not(feature = "imgui180-rs"))]
            io => io.KeysDown.get(idx),
        }
        .map(|&v| v as bool)
        .unwrap_or(false)
    }
    ...
    fn key_from_alphanum(&self, c: u8) -> usize {
        ...
        let idx = match c.to_ascii_uppercase() {
            b'A' => key_idx(sys::ImGuiKey_A),
            b'C' => key_idx(sys::ImGuiKey_C),
            b'V' => key_idx(sys::ImGuiKey_V),
...
impl ImDrawTarget for ImDrawList {
    ...
    fn add_rect_untyped(
        ...
        let flags = flags_untyped.unwrap_or(sys::ImDrawCornerFlags_All as u32) as sys::ImDrawCornerFlags;
...
impl<'ui> ImContext for Ui<'ui> {
    ...
    fn get_pio_ptr(&self) -> NonNull<Self::PlatformIo> {
        match () {
            #[cfg(todo)]
            _ => unsafe { NonNull::<()>::dangling() },
            _ => self.get_io_ptr().cast::<ImGuiIO>(),
        }
    }
}
unsafe impl<'ui> ImUiContext for ImGuiContext {
    #[inline(always)]
        ...
        unsafe {
            match () {
                ...
                _ => NonNull::new_unchecked(
                    &raw const self.Style as *const Style as *mut Style as *mut dyn ImStyle,
                ),
    ...
    fn get_draw_ptr_dyn(&self) -> NonNull<dyn ImDrawIo> {
        unsafe {
            match () {
                #[cfg(feature = "imgui180-imp")]
                _ =>
                    NonNull::new_unchecked(&raw const self.DrawData as *mut ImDrawData as *mut dyn ImDrawIo),
                #[cfg(not(feature = "imgui180-imp"))]
    ...
    fn get_io_ptr_dyn(&self) -> NonNull<dyn ImIo> {
        unsafe {
            match () {
                #[cfg(feature = "imgui180-imp")]
                _ => NonNull::new_unchecked(&raw const self.IO as *const Io as *mut Io as *mut dyn ImIo),
                #[cfg(not(feature = "imgui180-imp"))]
    ...
    fn get_pio_ptr_dyn(&self) -> NonNull<dyn ImPlatformIo> {
        unsafe {
            NonNull::new_unchecked(match () {
                #[cfg(feature = "imgui180-imp")]
                _ => &raw const self.IO as *mut ImGuiIO as *mut dyn ImPlatformIo,
                #[cfg(not(feature = "imgui180-imp"))]
                _ => Ui::materialize().get_io_ptr().as_ptr() as *mut dyn ImPlatformIo,
            })
        }
    }
    #[inline(always)]
    ...
    unsafe fn add_hook_boxed(
        ...
        unsafe {
            ...
            let request = ImGuiContextHook {
                ...
                Callback: Some(im180_context_hook_callback),
                UserData: Box::into_raw(hook).cast::<c_void>(),
                ..ImGuiContextHook::default()
            };
            let () = sys::igAddContextHook(ctx, &request);
            Some(NonZero::new_unchecked(request.UserData as *mut () as usize))
        }
    }
    /// there is no remove in 1.80, but we still can!
    ///
    /// TODO: replace callback with stub that will outlive dll for shared contexts
    #[inline]
    unsafe fn remove_hook_by_id(&mut self, id: NonZero<usize>) {
        let user_data = NonNull::new_unchecked(id.get() as *mut c_void);
        let ctx = self.get_ptr().cast::<ImGuiContext>().as_ptr();
        let removed = {
            let hooks = &mut *&raw mut (*ctx).Hooks;
            let hook = hooks
                .data_mut()
                .iter_mut()
                .find(|hook| hook.UserData == user_data.as_ptr());
            match hook {
                Some(hook) => Some({
                    mem::replace(&mut hook.UserData, ptr::null_mut()).cast::<Box<dyn ContextHookCallback>>()
                }),
                None => {
                    log::debug!("imgui context hook {user_data:p} expected for removal, leaking");
                    None
                },
            }
        };
...
unsafe extern "C" fn im180_context_hook_callback(ctx: *mut ImGuiContext, hook: *mut ImGuiContextHook) {
    #[cfg(debug_assertions)]
    if hook.is_null() {
        return
    }
    ...
    let hook_id = user_data.as_ptr() as usize;
    let ctx = &mut *ctx;
...
impl<'ui> ImUiWindow for Ui<'ui> {
    fn cursor_window_pos(&self) -> ImPos2<WindowSpace> {
        Ui::cursor_pos(self).into()
    }
    fn cursor_start_pos(&self) -> ImPos2<WindowSpace> {
        Ui::cursor_start_pos(self).into()
    }
    fn cursor_screen_pos(&self) -> ImPos2<ImSpace> {
        Ui::cursor_screen_pos(self).into()
    }
    fn scroll_offset(&self) -> ImVec2<ImSpace> {
        ImVec2::new(Ui::scroll_x(self), Ui::scroll_y(self))
    }
    fn window_pos(&self) -> ImPos2<ImSpace> {
        Ui::window_pos(self).into()
    }
    fn window_size(&self) -> ImSize2<ImSpace> {
        Ui::window_size(self).into()
    }
    fn item_rect_min(&self) -> ImPos2<ImSpace> {
        Ui::item_rect_min(self).into()
    }
    fn item_rect_max(&self) -> ImPos2<ImSpace> {
        Ui::item_rect_max(self).into()
    }
    fn item_rect_size(&self) -> ImSize2<ImSpace> {
        Ui::item_rect_size(self).into()
    }
    fn content_region_max(&self) -> ImPos2<WindowSpace> {
        Ui::content_region_max(self).into()
    }
    fn content_region_avail(&self) -> ImSize2<WindowSpace> {
        Ui::content_region_avail(self).into()
    }
    fn window_content_region_min(&self) -> ImPos2<WindowSpace> {
        Ui::window_content_region_min(self).into()
    }
    fn window_content_region_max(&self) -> ImPos2<WindowSpace> {
        Ui::window_content_region_max(self).into()
    }
    fn window_content_region_width(&self) -> f32 {
        Ui::window_content_region_width(self)
    }
}
fn im180_font_scale_window(_ui: &Ui<'_>) -> f32 {
    unsafe {
        match () {
            #[cfg(feature = "imgui180-imp")]
            _ => *&raw const (*sys::igGetCurrentWindow()).FontWindowScale,
            #[cfg(not(feature = "imgui180-imp"))]
            _ => {
                let size = match () {
                    #[cfg(todo = "unnecessary")]
                    _ => *&raw const (*sys::igGetDrawListSharedData()).FontSize,
                    _ => sys::igGetFontSize(),
                };
                let size_base = {
                    let size_font =
                        NonNull::new(sys::igGetFont()).map(|font| *(&raw const (*font.as_ptr()).FontSize));
                    let base_size = _ui.viewport_font_scale() * size_font.unwrap_or(1.0);
                };
                let scale = size / size_base;
                // oh well...
                #[cfg(todo)]
                let scale = scale / parent_window_font_scale;
                scale
            },
        }
    }
...
impl<'ui> ImUiWindow for Ui<'ui> {
    ...
    fn viewport_framebuffer_scale(&self) -> ImVec2<f32> {
        ImSpaces(unsafe {
            match () {
                #[cfg(feature = "imgui180-imp")]
                _ => (&*sys::igGetIO()).DisplayFramebufferScale,
                #[cfg(not(feature = "imgui180-imp"))]
                _ => *(&raw const (*Ui::materialize().get_io_ptr().as_ptr()).display_framebuffer_scale),
            }
        })
        .into()
    }
    fn viewport_font_scale(&self) -> f32 {
        unsafe {
            ...
            scale_global * scale_font.unwrap_or(1.0)
        }
    }
    fn font_scale(&self) -> f32 {
        let window_scale = im180_font_scale_window(self);
    ...
    fn item_is_hovered_untyped(&self, flags: Option<u32>) -> bool {
        const WINDOW_EXCLUSIVE_FLAGS: u32 = (sys::ImGuiHoveredFlags_AnyWindow
...
impl<'ui> ImDraw for &'_ Ui<'ui> {
    ...
    fn same_line_with(&mut self, offset: Option<f32>, spacing: Option<f32>) {
        Ui::same_line_with_spacing(offset.unwrap_or(0.0), spacing.unwrap_or(-1.0))
    }
}
impl<'ui> ImDraw for &'_ Ui<'ui> {
    #[inline]
    ...
    fn item_prepare_open(&mut self, open: bool, cond: ImCondition) {
        let ((imw::Window, ImVersion18000), cond) = cond.into();
    ...
    fn begin_table_dyn_untyped(
        ...
        <dyn ImStr>::with_cstr(ident, move |ident| unsafe {
            ...
            sys::igBeginTable(ident.as_ptr(), cols, flags, size, width)
                .then(|| UiTokenFn::new_fn_item(&mut im180_container_end_table))
        })
    }
}
...
impl<'ui> ImDrawItemStack<'ui> for &'_ Ui<'ui> {
    #[cfg(todo)]
    type StyleTokenItemSpacing = imgui::StyleStackToken<'ui>;
    ...
    fn push_style_item_spacing(&mut self, spacing: ImVec2) -> Self::StyleTokenItemSpacing {
        let token = unsafe {
            ...
            //<imgui::StyleStackToken<'ui> as UiTokenZst>::materialize_push()
            UiTokenFn::new_fn_item(&mut im180_pop_style_var)
        };
        token.into()
    }
    #[cfg(todo = "unnecessary")]
    fn push_item_spacing(&mut self, spacing: ImVec2) -> Self::StyleTokenItemSpacing {
        Ui::push_style_var(imgui::StyleVar::ItemSpacing(spacing.into()))
    }
}
...
impl<'ui> ImDrawWindowStack<'ui> for &'_ Ui<'ui> {
    ...
    fn begin_group_dyn(&mut self) -> UiTokenDyn<'ui> {
        unsafe {
            ...
            UiTokenFn::new_fn_item(&mut im180_container_end_group)
        }
    }
    #[inline]
    fn begin_tabs_dyn(&mut self, ident: &mut dyn ImStr, flags: Option<u32>) -> Option<UiTokenDyn<'ui>> {
        ...
        unsafe {
            <dyn ImStr>::with_cstr(ident, move |ident| sys::igBeginTabBar(ident.as_ptr(), flags))
                .then(|| UiTokenFn::new_fn_item(&mut im180_container_end_tabs))
        }
    }
    #[inline]
    fn begin_tab_dyn(
        ...
        <dyn ImStr>::with_cstr(label, move |label| unsafe {
            ...
            sys::igBeginTabItem(label.as_ptr(), open, flags)
                .then(|| UiTokenFn::new_fn_item(&mut im180_container_end_tab))
        })
    }
    #[inline]
    fn begin_tooltip_dyn(&mut self) -> Option<UiTokenDyn<'ui>> {
        unsafe {
            ...
            Some(UiTokenFn::new_fn_item(&mut im180_container_end_tooltip))
        }
    }
    #[inline]
    fn push_id32_dyn(&mut self, id: u32) -> UiTokenDyn<'ui> {
        unsafe {
            ...
            UiTokenFn::new_fn_item(&mut im180_pop_id)
        }
    }
    #[inline]
    ...
    fn open_popup_by_ident_dyn(&mut self, ident: &mut dyn ImStr, untyped_flags: Option<u32>) {
        let flags = untyped_flags.unwrap_or(0);
        <dyn ImStr>::with_cstr(ident, move |ident| unsafe {
            let () = sys::igOpenPopup(ident.as_ptr(), flags as c_int);
        })
    }
    #[inline]
    fn item_prepare_push_width_dyn(&mut self, width: f32) -> UiTokenDyn<'ui> {
        unsafe {
            ...
            UiTokenFn::new_fn_item(&mut im180_pop_item_width)
        }
    }
    #[inline]
    ...
    fn window_prepare_size(&mut self, size: ImSize2<ImSpace>, cond: ImCondition) {
        unsafe {
            let cond = cond.to_im180_sys() as sys::ImGuiCond;
...
impl<'ui> ImDrawWindowStack<'ui> for Ui<'ui> {
    ...
    where
        'ui: 'a,
    {
        ImDrawWindowStack::window_prepare_size_constraints_dyn(&mut &*self, min, max, cb)
    }
...
impl<T> ImSpaces<T> {
    #[inline(always)]
    pub fn to_sys2_180(self) -> sys::ImVec2
    where
        T: mint::IntoMint,
    ...
    #[inline(always)]
    pub fn to_sys4_180(self) -> sys::ImVec4
    where
        T: mint::IntoMint,
...
#[cfg(todo)]
impl<T> From<ImSpaces<T>> for sys::ImVec4
where
    T: mint::IntoMint<MintType = mint::Vector4<f32>>,
{
    #[inline(always)]
    fn from(v: ImSpaces<ImVec4<U>>) -> Self {
        v.to_sys4_180()
    }
}
...
impl ImCondition {
    pub fn to_im180_sys(self) -> sys::ImGuiCond_ {
        match self {
...
impl TextureId {
    ...
    pub fn id(&self) -> usize {
        self.tex_id as usize
    }
    #[inline]
    pub fn raw(&self) -> sys::ImTextureID {
        self.tex_id
    }
}
impl ImTexture for TextureId {
    fn im180_texture_id(&self) -> Option<sys::ImTextureID> {
        Some(self.id() as _)
    }
    #[cfg(feature = "imgui192")]
    fn im192_texture_ref(&self) -> Option<sys192::ImTextureRef> {
        Some(sys192::ImTextureRef::from_id(self.id() as _))
    }
}
#[cfg(todo)]
...
#[cfg(todo)]
impl UiToken for imgui::ItemWidthStackToken {
    #[inline]
    fn token_pop(self) {
        let is_empty = match () {
            #[cfg(debug_assertions)]
...
fn im180_pop_style_var() {
    unsafe { sys::igPopStyleVar(1) }
}
fn im180_pop_item_width() {
    unsafe { sys::igPopItemWidth() }
}
fn im180_container_end_window() {
    unsafe { sys::igEnd() }
}
fn im180_container_end_child_window() {
    unsafe { sys::igEndChild() }
}
fn im180_container_end_popup() {
    unsafe { sys::igEndPopup() }
}
fn im180_container_end_group() {
    unsafe { sys::igEndGroup() }
}
fn im180_container_end_tooltip() {
    unsafe { sys::igEndTooltip() }
}
fn im180_container_end_menu() {
    unsafe { sys::igEndMenu() }
}
fn im180_container_end_tabs() {
    unsafe { sys::igEndTabBar() }
}
fn im180_container_end_tab() {
    unsafe { sys::igEndTabItem() }
}
fn im180_container_end_table() {
    unsafe { sys::igEndTable() }
}
fn im180_container_end_tree_node() {
...
impl<W: Default> Into<((W, ImVersion18000), sys::ImGuiCond)> for ImCondition {
    #[inline(always)]
    fn into(self) -> ((W, ImVersion18000), sys::ImGuiCond) {
        let cond = self.to_im180_sys() as sys::ImGuiCond;
        (Default::default(), cond)
    }
}
impl Into<((imw::Window, ImVersion18000), sys::ImGuiWindowFlags)> for imw::DynArgsWindow {
    #[inline(always)]
    fn into(self) -> ((imw::Window, ImVersion18000), sys::ImGuiWindowFlags) {
        let flags = self.untyped_flags().unwrap_or(0);
        (Default::default(), flags as sys::ImGuiWindowFlags)
    }
}
impl<'ui, U: ?Sized, S, I, F> ImWidget<U> for (&'ui imw::Window, S, Option<I>, F)
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    S: ImStrExt,
    I: BorrowMut<bool>,
    F: Into<((imw::Window, ImVersion18000), sys::ImGuiWindowFlags)>,
{
    /// for mostly legacy reasons, token is unconditional
    type Output = (Option<imw::BeginVisible>, UiTokenDyn<'ui>);
    #[inline]
    fn draw_widget(self, _: &mut U) -> Self::Output {
        ...
        let token = unsafe { UiTokenFn::new_fn_item(&mut im180_container_end_window) };
        (vis, token)
    }
}
impl
    Into<(
        (imw::ChildWindow, ImVersion18000),
    ...
    )> for imw::DynArgsChildWindow
{
    ...
    fn into(
        ...
        let flags = self.untyped_flags().unwrap_or(0);
        let border = flags & imw::ChildWindow::IM180_BORDER_FLAG != 0;
        (
            Default::default(),
            (flags & !imw::ChildWindow::IM180_BORDER_FLAG) as sys::ImGuiWindowFlags,
            ImSpaces(self.size().unwrap_or(ImSize2::ZERO)).into(),
            border,
        )
    }
}
impl<'ui, U: ?Sized, S, F> ImWidget<U> for (&'ui imw::ChildWindow, S, (), F)
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    S: ImStrExt,
    F: Into<(
    ...
    )>,
{
    ...
    fn draw_widget(self, _: &mut U) -> Self::Output {
        ...
        let (_, flags, size, border) = flags.into();
        let id32 = label.with_imstr_dyn(|label| label.im_as_id32());
        let vis = imw::BeginVisible::new(match id32 {
            Some(id) => unsafe { sys::igBeginChild_ID(id as c_uint, size, border, flags) },
            None => <dyn ImStr>::with_cstr(label, move |label| unsafe {
                sys::igBeginChild_Str(label.as_ptr(), size, border, flags)
            }),
        });
        let token = unsafe { UiTokenFn::new_fn_item(&mut im180_container_end_child_window) };
        (vis, token)
    }
}
impl Into<((imw::Popup, ImVersion18000), sys::ImGuiWindowFlags)> for imw::DynArgsWindow {
    #[inline(always)]
    fn into(self) -> ((imw::Popup, ImVersion18000), sys::ImGuiWindowFlags) {
        let flags = self.untyped_flags().unwrap_or(0);
        (Default::default(), flags as sys::ImGuiWindowFlags)
    }
}
impl<'ui, U: ?Sized, S, F> ImWidget<U> for (&'ui imw::Popup, S, (), F)
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    S: ImStrExt,
    F: Into<((imw::Popup, ImVersion18000), sys::ImGuiWindowFlags)>,
{
    ...
    fn draw_widget(self, _: &mut U) -> Self::Output {
        ...
        let token = vis.then(|| unsafe { UiTokenFn::new_fn_item(&mut im180_container_end_popup) });
        token
    }
}
impl Into<((imw::PopupModal, ImVersion18000), sys::ImGuiWindowFlags)> for imw::DynArgsWindow {
    #[inline(always)]
    fn into(self) -> ((imw::PopupModal, ImVersion18000), sys::ImGuiWindowFlags) {
        let flags = self
            .untyped_flags()
            .unwrap_or(imw::PopupModal::IM180_FLAGS_PRESET);
        (Default::default(), flags as sys::ImGuiWindowFlags)
    }
}
impl<'ui, U: ?Sized, S, O, F> ImWidget<U> for (&'ui imw::PopupModal, S, Option<O>, F)
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    S: ImStrExt,
    O: BorrowMut<bool>,
    F: Into<((imw::PopupModal, ImVersion18000), sys::ImGuiWindowFlags)>,
{
    type Output = Option<UiTokenDyn<'ui>>;
    #[inline]
    fn draw_widget(self, _: &mut U) -> Self::Output {
...
impl<'ui, U: ?Sized, F, P> ImWidget<U> for (&'ui imw::Tooltip, P, F)
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    //F: Into<((imw::Tooltip, ImVersion18000), ())>,
{
    ...
    fn draw_widget(self, _ui: &mut U) -> Self::Output {
        ...
        unsafe {
            let () = sys::igBeginTooltip();
            UiTokenFn::new_fn_item(&mut im180_container_end_tooltip)
        }
        _ui.ui().begin_tooltip_dyn()
    }
}

impl<'ui, U: ?Sized, F, P> ImWidget<U> for (&'ui imw::Group, P, F)
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    //F: Into<((imw::Group, ImVersion18000), ())>,
{
    //type Output = Option<imw::ContainerOpen<Combo>>;
    ...
    fn draw_widget(self, _ui: &mut U) -> Self::Output {
        ...
        _ui.ui().begin_group_dyn()
    }
}

impl Into<((imw::Image, ImVersion18000), sys::ImVec2)> for imw::DynArgsWidgetSized {
    #[inline(always)]
    fn into(self) -> ((imw::Image, ImVersion18000), sys::ImVec2) {
        #[cfg(todo)]
...
impl<'ui, U: ?Sized, T, F> ImWidget<U> for (&'ui imw::Image, T, F)
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    T: ImTexture,
    F: Into<((imw::Image, ImVersion18000), sys::ImVec2)>,
{
    type Output = ();
    #[inline]
    fn draw_widget(self, _ui: &mut U) -> Self::Output {
        ...
        let tex_id = texture.im180_texture_id().unwrap_or(ptr::null_mut());
        let tint = texture.tint();
        let uv = texture.uv_bounds();
        // TODO?
        let border_colour = ImColour::splat(0.0);
        unsafe {
            sys::igImage(
                tex_id,
                size,
                ...
                ImSpaces(tint).into(),
                ImSpaces(border_colour).into(),
            )
        }
    }
}

impl Into<((imw::ProgressBar, ImVersion18000), sys::ImVec2)> for imw::DynArgsWidgetSized {
    #[inline(always)]
    fn into(self) -> ((imw::ProgressBar, ImVersion18000), sys::ImVec2) {
        #[cfg(todo)]
        let _flags = self.untyped_flags().unwrap_or(0);
        (
            Default::default(),
            ImSpaces(self.size().unwrap_or(imw::ProgressBar::IM180_DEFAULT_SIZE)).into(),
        )
    }
}
impl<'ui, U: ?Sized, O, S, F> ImWidget<U> for (&'ui imw::ProgressBar, (S, Option<O>), F)
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    S: Into<f32>,
    O: ImStr,
    F: Into<((imw::ProgressBar, ImVersion18000), sys::ImVec2)>,
{
    type Output = ();
    #[inline]
    fn draw_widget(self, _ui: &mut U) -> Self::Output {
...
impl<T: ?Sized + ImPrimitiveData>
    Into<(
        PhantomData<(imw::Slider<T>, ImVersion18000)>,
...
impl<'ui, T, U: ?Sized, L, D, S, F> ImWidget<U> for (&'ui imw::Slider<T>, L, (S, Option<D>), F)
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    ...
    )>,
{
    ...
    fn draw_widget(self, _ui: &mut U) -> Self::Output {
        ...
        let data_type = T::im180_data_type() as sys::ImGuiDataType;
        <dyn ImStr>::with_cstr(label, move |label| unsafe {
...
impl<T: ?Sized + ImPrimitiveData>
    Into<(
        PhantomData<(imw::VSlider<T>, ImVersion18000)>,
        sys::ImGuiSliderFlags,
        RangeInclusive<T::Primitive>,
        sys::ImVec2,
...
impl<'ui, T, U: ?Sized, L, D, S, F> ImWidget<U> for (&'ui imw::VSlider<T>, L, (S, Option<D>), F)
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    T: ImPrimitive,
    S: BorrowMut<T>,
...
impl<T: ?Sized + ImPrimitiveData>
    Into<(
        PhantomData<(imw::InputScalar<T>, ImVersion18000)>,
...
impl<'ui, T, U: ?Sized, L, D, S, F> ImWidget<U> for (&'ui imw::InputScalar<T>, L, (S, Option<D>), F)
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    T: ?Sized + ImPrimitiveData,
    S: BorrowMut<T>,
...
#[cfg(todo)]
impl<'ui, U: ?Sized, S> ImWidget<U> for imgui::Selectable<S>
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    S: AsRef<str>,
{
...
impl<'ui, U: ?Sized, S, F> ImWidget<U> for (&'ui imw::Selectable, S, bool, F)
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    S: ImStrExt,
    F: Into<(
...
impl Into<((imw::MenuItem, ImVersion18000), bool)> for imw::DynArgsWidget {
    #[inline(always)]
    fn into(self) -> ((imw::MenuItem, ImVersion18000), bool) {
        let flags = self.untyped_flags().unwrap_or(0);
        (Default::default(), flags & imw::MenuItem::FLAGS_DISABLED == 0)
    }
}
impl<'ui, U: ?Sized, L, S, F> ImWidget<U> for (&'ui imw::MenuItem, L, (bool, Option<S>), F)
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    L: ImStrExt,
    S: ImStr,
    F: Into<((imw::MenuItem, ImVersion18000), bool)>,
{
    type Output = bool;
    #[inline]
    fn draw_widget(self, _ui: &mut U) -> Self::Output {
...
#[cfg(todo)]
impl<'ui, U: ?Sized, L, B, S, F> ImWidget<U> for (&'ui imw::MenuItem, L, (&'_ mut B, Option<S>), F)
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    L: ImStrExt,
    S: ImStr,
    ...
    F: Into<((imw::MenuItem, ImVersion18000), bool)>,
{
    ...
    fn draw_widget(self, _ui: &mut U) -> Self::Output {
        ...
        <dyn ImStr>::with_cstr(label, move |label| unsafe {
            let shortcut = shortcut.as_ref().map(|p| p.as_ptr()).unwrap_or(ptr::null());
            let state = state as *mut bool;
            sys::igMenuItem_BoolPtr(label.as_ptr(), shortcut, state, enabled)
        })
    }
}
impl Into<((imw::Menu, ImVersion18000), bool)> for imw::DynArgsWidget {
    #[inline(always)]
    fn into(self) -> ((imw::Menu, ImVersion18000), bool) {
        let flags = self.untyped_flags().unwrap_or(0);
        (Default::default(), flags & imw::MenuItem::FLAGS_DISABLED == 0)
    }
}
impl<'ui, U: ?Sized, L, F> ImWidget<U> for (&'ui imw::Menu, L, (), F)
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    L: ImStrExt,
    F: Into<((imw::Menu, ImVersion18000), bool)>,
{
    type Output = Option<UiTokenDyn<'ui>>;
    #[inline]
    fn draw_widget(self, _ui: &mut U) -> Self::Output {
        ...
        <dyn ImStr>::with_cstr(label, move |label| unsafe {
            sys::igBeginMenu(label.as_ptr(), enabled)
                .then(|| UiTokenFn::new_fn_item(&mut im180_container_end_menu))
        })
    }
}

impl Into<((imw::TreeNode, ImVersion18000), sys::ImGuiTreeNodeFlags)> for imw::DynArgsWidget {
    #[inline(always)]
    fn into(self) -> ((imw::TreeNode, ImVersion18000), sys::ImGuiTreeNodeFlags) {
        let flags = self.untyped_flags().unwrap_or(0);
        (Default::default(), flags as sys::ImGuiTreeNodeFlags)
    }
}
impl<'ui, U: ?Sized, S, I, F> ImWidget<U> for (&'ui imw::TreeNode, S, I, F)
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    F: Into<((imw::TreeNode, ImVersion18000), sys::ImGuiTreeNodeFlags)>,
    S: ImStrExt,
    I: ImStr,
{
    type Output = Option<UiTokenDyn<'ui>>;
    ...
    S: ImStrExt,
{
    ...
    fn draw_widget(self, _ui: &mut U) -> Self::Output {
        ...
        let (_, size) = flags.into();
        <dyn ImStr>::with_cstr(label, move |label| unsafe {
            sys::igListBoxHeader_Vec2(label.as_ptr(), size)
        })
...
impl Into<((imw::Combo, ImVersion18000), sys::ImGuiComboFlags)> for imw::DynArgsWidget {
    #[inline(always)]
    fn into(self) -> ((imw::Combo, ImVersion18000), sys::ImGuiComboFlags) {
        let flags = self.untyped_flags().unwrap_or(0);
        (Default::default(), flags as sys::ImGuiComboFlags)
    }
}
impl<'ui, U: ?Sized, S, F, P> ImWidget<U> for (&'ui imw::Combo, S, Option<P>, F)
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    F: Into<((imw::Combo, ImVersion18000), sys::ImGuiComboFlags)>,
    S: ImStrExt,
    P: ImStr,
{
    //type Output = Option<imw::ContainerOpen<Combo>>;
    ...
    fn draw_widget(self, _ui: &mut U) -> Self::Output {
        ...
        .then(|| unsafe { UiTokenFn::new_fn_item(&mut im180_container_end_combo) })
    }
}
impl Into<((imw::InputText, ImVersion18000), sys::ImGuiInputTextFlags)> for imw::DynArgsInputText {
    #[inline(always)]
    fn into(self) -> ((imw::InputText, ImVersion18000), sys::ImGuiInputTextFlags) {
        let flags = self.untyped_flags().unwrap_or(imw::InputText::IM180_FLAGS_PRESET);
        (Default::default(), flags as sys::ImGuiInputTextFlags)
    }
}
/// TODO: callback
impl<'ui, U: ?Sized, S, F, I, H> ImWidget<U> for (&'ui imw::InputText, S, (I, Option<H>), F)
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    F: Into<((imw::InputText, ImVersion18000), sys::ImGuiInputTextFlags)>,
    S: ImStrExt,
    H: ImStr,
    I: BorrowMut<String>,
{
    ...
    fn draw_widget(self, _ui: &mut U) -> Self::Output {
        ...
        let enter_true = flags & imw::InputText::IM180_MASK_ENTER != 0;
        // XXX: without a callback, use return value to tell if buffer was modified
        let flags = flags & !imw::InputText::IM180_MASK_ENTER;
        ...
        <dyn ImStr>::with_cstr(label, move |label| unsafe {
            ...
            if buf_modified {
                ...
                let mut new_len = {
                    ...
                    match prev_end.read() {
                        0u8 => 0,
                        _ => {
                            // seems like it was extended, so skip ahead
                            // TODO: there's likely no guarantee the dead space can't be clobbered when the buffer is truncated, so...
                            // bad check tbh
                            ptr = prev_end.add(1);
                            prev_len + 1
                        },
            ...
            match enter_true {
                true => {
                    // TODO: IsKeyDown should be fine probably...
                    let is_key_pressed = |k: sys::ImGuiKey_| {
                        sys::igIsKeyPressed(
                            (&*sys::igGetIO())
                                .KeyMap
                                .get(k as usize)
                    ...
                    sys::igIsItemDeactivatedAfterEdit()
                        && (is_key_pressed(sys::ImGuiKey_Enter)
                            || is_key_pressed(sys::ImGuiKey_KeyPadEnter))
                },
                false => buf_modified,
            }
        })
...
impl Into<((imw::InputPassword, ImVersion18000), sys::ImGuiInputTextFlags)> for imw::DynArgsInputText {
    #[inline(always)]
    fn into(self) -> ((imw::InputPassword, ImVersion18000), sys::ImGuiInputTextFlags) {
        let flags = self
            .untyped_flags()
            .unwrap_or(imw::InputPassword::IM180_FLAGS_PRESET);
        (Default::default(), flags as sys::ImGuiInputTextFlags)
    }
}
impl<'ui, U: ?Sized, S, I, H, F> ImWidget<U> for (&'ui imw::InputPassword, S, (I, Option<H>), F)
where
    F: Into<((imw::InputPassword, ImVersion18000), sys::ImGuiInputTextFlags)>,
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    S: ImStrExt,
    H: ImStr,
    I: BorrowMut<String>,
...
impl
    Into<(
        (imw::InputTextMultiline, ImVersion18000),
        sys::ImGuiInputTextFlags,
        sys::ImVec2,
    )> for imw::DynArgsInputMultiline
{
    ...
    fn into(
        ...
        let flags = self
            .untyped_flags()
            .unwrap_or(imw::InputTextMultiline::IM180_FLAGS_PRESET);
        let size = self.size().unwrap_or(imw::InputTextMultiline::IM180_DEFAULT_SIZE);
        (
            Default::default(),
...
/// TODO: callback
impl<'ui, U: ?Sized, S, F, I> ImWidget<U> for (&'ui imw::InputTextMultiline, S, I, F)
where
    U: ImDraw + AsUi<'ui, Ui<'ui>>,
    F: Into<(
        (imw::InputTextMultiline, ImVersion18000),
...
/// TODO...
impl Into<((imw::Table, ImVersion18000), sys::ImGuiTableFlags)> for imw::DynArgsTable {
    #[inline(always)]
    fn into(self) -> ((imw::Table, ImVersion18000), sys::ImGuiTableFlags) {
        let flags = self.untyped_flags().unwrap_or(0);
        (Default::default(), flags as sys::ImGuiWindowFlags)
    }
}
/// TODO...
impl Into<((imw::TableRow, ImVersion18000), sys::ImGuiTableRowFlags)> for imw::DynArgsTableRow {
    #[inline(always)]
    fn into(self) -> ((imw::TableRow, ImVersion18000), sys::ImGuiTableRowFlags) {
        let flags = self.untyped_flags().unwrap_or(0);
        (Default::default(), flags as sys::ImGuiWindowFlags)
    }
}
/// TODO...
impl Into<((imw::TableColumn, ImVersion18000), sys::ImGuiTableColumnFlags)> for imw::DynArgsTableColumn {
    #[inline(always)]
    fn into(self) -> ((imw::TableColumn, ImVersion18000), sys::ImGuiTableColumnFlags) {
        let flags = self.untyped_flags().unwrap_or(0);
        (Default::default(), flags as sys::ImGuiWindowFlags)
    }
}
