pub use crate::dx11::d3d11::{ID3D11Resource, D3D11_RESOURCE_DIMENSION};
use crate::{
    dx::{self, DXGI_RESOURCE_PRIORITY},
    dx11::{
        buffer::{Buffer, ID3D11Buffer, ID3D11Texture2D, Texture2},
        prelude::*,
    },
};
...
impl Resource {
    ...
    pub fn get_type_d3d(&self) -> D3D11_RESOURCE_DIMENSION {
        unsafe { self.as_d3d().GetType() }
    }

    pub fn get_type(&self) -> ResourceDimension {
        ResourceDimension::try_from_d3d(self.get_type_d3d()).unwrap_or(ResourceDimension::Unknown)
    }
    ...
    pub fn get_eviction_prio(&self) -> DXGI_RESOURCE_PRIORITY {
        DXGI_RESOURCE_PRIORITY(unsafe { self.as_d3d().GetEvictionPriority() })
    }

    pub fn set_eviction_prio(&self, priority: DXGI_RESOURCE_PRIORITY) {
        unsafe { self.as_d3d().SetEvictionPriority(priority.0) }
    }
}
