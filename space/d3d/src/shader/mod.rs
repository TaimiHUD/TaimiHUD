...
pub fn compile(
    ...
    let error_context = || {
        anyhow!(
            "D3DCompile({target:?}, {}:{})",
            CStrRef::with_cstr(filename),
            CStrRef::with_cstr(entry_point)
        )
    };
...
mod defs {
    use {
        crate::{prelude::*, shader::D3D_SHADER_MACRO},
    ...
    impl<N, D> From<(N, D)> for ShaderDefinition
    where
        N: Into<CStrBox>,
        D: Into<CStrBox>,
    {
    ...
    impl Into<(String, String)> for ShaderDefinition {
        fn into(self) -> (String, String) {
            ...
            (
                name.map(|n| n.into_cstring().to_string_lossy().into_owned())
                    .unwrap_or_default(),
                definition
                    .map(|d| d.into_cstring().to_string_lossy().into_owned())
                    .unwrap_or_default(),
            )
        }
    }
    ...
    impl<D> FromIterator<D> for ShaderDefinitions
    where
    ...
    impl<D> Extend<D> for ShaderDefinitions
    where
        D: Into<ShaderDefinition>,
    {
    ...
    impl<N, D> From<BTreeMap<N, D>> for ShaderDefinitions
    where
    ...
    impl<N, D> From<HashMap<N, D>> for ShaderDefinitions
    where
        (N, D): Into<ShaderDefinition>,
    {
    ...
    impl<'de> serde::Deserialize<'de> for ShaderDefinitions {
            ...
            D: serde::Deserializer<'de>,
        {
            ...
            Self::try_from_str(defs).map_err(|_| {
