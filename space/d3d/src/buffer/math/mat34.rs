use core::mem;
use glam::f32::{Affine3A, Vec3A, Mat3, Mat4};
use glamour::{Vector4, Vector3, Matrix3, Matrix4, Unit, Scalar, FloatScalar};
...
impl<U: Unit> Mat43<U> {
    ...
    #[inline]
    pub fn from_cols<V: Into<Vector4<U>>>(
        x_axis: V,
        y_axis: V,
        z_axis: V,
    ) -> Self {
    ...
    const fn vec4to3(axis: Vector4<U>) -> Vector3<U> {
        unsafe {
            mem::transmute_copy(&axis)
        }
    }
}
impl<U: Unit + Scalar> Mat43<U> {
    pub fn try_from_mat4(
        m4: Matrix4<U>,
    ) -> Option<Self> {
        match m4.w_axis {
            w if w != Vector4::W =>
    ...
    pub fn try_from_mat4_translation(
        m4: Matrix4<U>,
    ) -> Option<Self> {
        let w_row = Vector4::<U>::new(
            m4.x_axis.w,
            m4.y_axis.w,
            m4.z_axis.w,
            m4.w_axis.w,
        ...
        match w_row {
            w if w != Vector4::<U>::W =>
                None,
            _ => Some(
                Self::from_mat4_translation_unchecked(m4)
            ),
        }
    }

    #[inline]
    pub fn from_mat3<M: Into<Matrix3<U>>>(
        m3: M,
    ) -> Self {
    ...
    pub fn to_mat4_translation(self) -> Matrix4<U> where
        U: FloatScalar,
    {
    ...
    #[inline]
    pub const fn from_mat4_ref_unchecked(
        m4: &Matrix4<U>,
    ) -> &Self {
        unsafe {
            mem::transmute(m4)
    ...
    #[inline]
    pub const fn from_mat4_unchecked(
        m4: Matrix4<U>,
    ) -> Self {
        unsafe {
            mem::transmute_copy(&m4)
        }
    }
    #[inline]
    pub fn from_mat4_translation_unchecked(
        mut m4: Matrix4<U>,
    ) -> Self {
    ...
    pub fn transpose(self) -> Self where
        U::Scalar: FloatScalar,
    {
        Self::from_mat3_translation(
            self.to_mat3().transpose(),
            self.w_row(),
        )
    }
}
impl Mat43 {
    ...
    pub fn from_affine3(
        ...
        Self::from_mat4_translation_unchecked(
            Mat4::from(a3)
        )
    }
    /// TODO: transpose?
    #[inline]
    pub fn from_affine3a(
        a3: Affine3A,
    ) -> Self {
        Self::from_mat4_translation_unchecked(
            Matrix4::from(Mat4::from(a3))
        )
    }
